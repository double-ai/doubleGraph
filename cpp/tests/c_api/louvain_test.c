                         ...
                         bool_t store_transposed)
{
  ...
  if (test_ret_value == 0) {
    ...
    // Verify partition equivalence (same grouping, labels may differ).
    // Two vertices should be in the same cluster iff they have the same expected label.
    for (int i = 0; (i < num_vertices) && (test_ret_value == 0); ++i) {
      for (int j = i + 1; (j < num_vertices) && (test_ret_value == 0); ++j) {
        int expected_same = (h_result[h_vertices[i]] == h_result[h_vertices[j]]);
        int actual_same   = (h_clusters[i] == h_clusters[j]);
        TEST_ASSERT(test_ret_value, expected_same == actual_same, "cluster partition doesn't match");
      }
    }
