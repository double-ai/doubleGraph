                                        ...
                                        bool_t store_transposed)
{
  ...
  // Validate path is a valid shortest path rather than checking exact vertices,
  // since BFS predecessor tie-breaking is non-deterministic.
  TEST_ASSERT(test_ret_value, paths_size == expected_max_path_length, "path size mismatch");
  TEST_ASSERT(test_ret_value, h_paths[0] == h_seeds[0], "path must start at source");
  TEST_ASSERT(test_ret_value, h_paths[paths_size - 1] == h_destinations[0], "path must end at destination");

  for (int i = 0; (i < (int)paths_size - 1) && (test_ret_value == 0); ++i) {
    TEST_ASSERT(test_ret_value,
                edge_exists(h_src, h_dst, num_edges, h_paths[i], h_paths[i + 1]),
                "path contains non-existent edge");
  }
