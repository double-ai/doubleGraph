...
{
  // For weighted cosine on this graph, all top-scoring pairs have score 1.0
  // (single common neighbor => intersection-restricted cosine is always 1.0).
  // There are 8 such pairs but topk=6, so tie-breaking is implementation-defined.
  // We only check that the correct count is returned and every score is ~1.0.
  int test_ret_value = 0;
  ...
  cugraph_graph_t* graph = NULL;
  ret_code = create_test_graph(
    handle, h_src, h_dst, h_wgt, num_edges, FALSE, FALSE, TRUE, &graph, &ret_error);
  TEST_ASSERT(test_ret_value, ret_code == CUGRAPH_SUCCESS, "create_test_graph failed.");

  cugraph_similarity_result_t* result = NULL;
  ret_code = cugraph_all_pairs_cosine_similarity_coefficients(
    handle, graph, NULL, TRUE, topk, FALSE, &result, &ret_error);
  TEST_ASSERT(test_ret_value, ret_code == CUGRAPH_SUCCESS, cugraph_error_message(ret_error));

  cugraph_type_erased_device_array_view_t* similarity_coefficient =
    cugraph_similarity_result_get_similarity(result);
  size_t result_num_pairs = cugraph_type_erased_device_array_view_size(similarity_coefficient);

  TEST_ASSERT(test_ret_value, result_num_pairs == topk, "Incorrect number of results");

  weight_t h_scores[result_num_pairs];
  ret_code = cugraph_type_erased_device_array_view_copy_to_host(
    handle, (byte_t*)h_scores, similarity_coefficient, &ret_error);
  TEST_ASSERT(test_ret_value, ret_code == CUGRAPH_SUCCESS, "copy_to_host failed.");

  for (size_t i = 0; (i < result_num_pairs) && (test_ret_value == 0); ++i) {
    TEST_ASSERT(test_ret_value, nearlyEqual(h_scores[i], 1.0f, 0.001),
                "similarity score not 1.0");
  }
...
{
  // For unweighted cosine, all pairs with shared neighbors score 1.0,
  // so topk tie-breaking is implementation-defined.  We only check that
  // the correct count is returned and every score ≈ 1.0.
  int test_ret_value = 0;
  ...
  cugraph_similarity_result_t* result = NULL;
  ret_code = cugraph_all_pairs_cosine_similarity_coefficients(
    handle, graph, NULL, FALSE, topk, FALSE, &result, &ret_error);
  TEST_ASSERT(test_ret_value, ret_code == CUGRAPH_SUCCESS, cugraph_error_message(ret_error));
