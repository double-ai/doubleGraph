...
{
  /* With no weights all edges get unit weight, so every spanning tree is a
     valid MST.  Instead of checking for specific edges we validate
     structural properties:
       1. edge count == 2 * (V - 1)   (symmetrized spanning tree)
       2. the edges span all vertices
       3. offsets are {0, num_edges}                                       */
  ...
  ret_code = cugraph_minimum_spanning_tree(resource_handle, graph, FALSE, &result, &ret_error);
  TEST_ASSERT(test_ret_value, ret_code == CUGRAPH_SUCCESS, cugraph_error_message(ret_error));
  TEST_ALWAYS_ASSERT(ret_code == CUGRAPH_SUCCESS, "cugraph_minimum_spanning_tree failed.");

  if (test_ret_value == 0) {
    cugraph_type_erased_device_array_view_t* src     = cugraph_induced_subgraph_get_sources(result);
    cugraph_type_erased_device_array_view_t* dst     = cugraph_induced_subgraph_get_destinations(result);
    cugraph_type_erased_device_array_view_t* offsets  = cugraph_induced_subgraph_get_subgraph_offsets(result);

    size_t num_result_edges   = cugraph_type_erased_device_array_view_size(src);
    size_t num_result_offsets = cugraph_type_erased_device_array_view_size(offsets);
    size_t expected_edges     = 2 * (num_vertices - 1);  /* symmetrized */

    TEST_ASSERT(test_ret_value, num_result_edges == expected_edges,
                "MST edge count should be 2*(V-1)");
    TEST_ASSERT(test_ret_value, num_result_offsets == 2,
                "should have exactly 2 subgraph offsets");
    ...
    if (test_ret_value == 0) {
      ...
      TEST_ASSERT(test_ret_value, h_result_offsets[0] == 0, "first offset should be 0");
      TEST_ASSERT(test_ret_value, h_result_offsets[1] == num_result_edges,
                  "second offset should equal num_result_edges");

      /* Check all vertices are spanned using union-find */
      int parent[6];
      for (int i = 0; i < (int)num_vertices; i++) parent[i] = i;

      for (size_t i = 0; i < num_result_edges && test_ret_value == 0; i++) {
        TEST_ASSERT(test_ret_value,
                    h_result_src[i] >= 0 && h_result_src[i] < (vertex_t)num_vertices,
                    "source vertex out of range");
        TEST_ASSERT(test_ret_value,
                    h_result_dst[i] >= 0 && h_result_dst[i] < (vertex_t)num_vertices,
                    "destination vertex out of range");
        /* union */
        int a = h_result_src[i], b = h_result_dst[i];
        while (parent[a] != a) a = parent[a];
        while (parent[b] != b) b = parent[b];
        if (a != b) parent[a] = b;
      }

      /* Check single component: all roots should be the same */
      int root = 0;
      while (parent[root] != root) root = parent[root];
      for (int i = 1; i < (int)num_vertices && test_ret_value == 0; i++) {
        int r = i;
        while (parent[r] != r) r = parent[r];
        TEST_ASSERT(test_ret_value, r == root,
