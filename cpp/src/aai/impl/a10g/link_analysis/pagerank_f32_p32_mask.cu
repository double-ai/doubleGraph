...
struct Cache : Cacheable {
    ...
    void ensure_spmv(size_t bsz) {
        if (spmv_capacity < bsz) {
...
PageRankResult personalized_pagerank_mask(const graph32_t& graph,
    ...
    float h_psum = 0.0f;
    {
        ...
        cudaMemcpy(hv.data(), personalization_values,
    ...
    if (precomputed_vertex_out_weight_sums)
        ...
        compute_out_weights_kernel<<<std::min((E + 255) / 256, 65535), 256, 0, cache.main_stream>>>(
            edge_weights, d_mask, d_indices, d_ow, E);

    if (E > 0)
        compute_scaled_weights_kernel<<<std::min((E + 255) / 256, 65535), 256, 0, cache.main_stream>>>(
            edge_weights, d_mask, d_indices, d_ow, d_sw, alpha, E);

    if (N > 0) {
        int nw = (N + 31) / 32;
        compute_dangling_bitmask_kernel<<<std::min((nw + 255) / 256, 65535), 256, 0, cache.main_stream>>>(
            d_ow, d_dg, N);
    }
    ...
    if (personalization_size > 0) {
        ...
        init_pers_norm_kernel<<<std::min((ps + 255) / 256, 65535), 256, 0, cache.main_stream>>>(
            personalization_vertices, personalization_values, d_pn, psi, ps);
    }

    if (initial_pageranks)
        cudaMemcpyAsync(d_old, initial_pageranks, N * sizeof(float),
                        cudaMemcpyDeviceToDevice, cache.main_stream);
    else if (N > 0)
        init_uniform_pr_kernel<<<std::min((N + 255) / 256, 65535), 256, 0, cache.main_stream>>>(
            d_old, 1.0f / (float)N, N);
    ...
    if (bsz > 0) cache.ensure_spmv(bsz);
    ...
    for (size_t it = 0; it < max_iterations; it++) {
        ...
        if (N > 0)
            dangling_sum_bitmask_kernel<<<std::min((N + 255) / 256, 1024), 256, 0, cache.aux_stream>>>(
                d_old, d_dg, d_ds, N);
        ...
        if (check) {
            ...
            if (N > 0)
                fused_base_diff_kernel<<<std::min((N + 255) / 256, 1024), 256, 0, cache.main_stream>>>(
                    d_new, d_old, d_pn, d_ds, alpha, oma, d_df, N);
            cudaMemcpyAsync(cache.h_diff, d_df, sizeof(float),
                            cudaMemcpyDeviceToHost, cache.main_stream);
            cudaStreamSynchronize(cache.main_stream);
            if (*cache.h_diff < epsilon) { conv = true; break; }
        } else {
            if (N > 0)
                base_factor_only_kernel<<<std::min((N + 255) / 256, 1024), 256, 0, cache.main_stream>>>(
                    d_new, d_pn, d_ds, alpha, oma, N);
        }
    ...
    cudaMemcpyAsync(pageranks, d_res, N * sizeof(float),
                    cudaMemcpyDeviceToDevice, cache.main_stream);
