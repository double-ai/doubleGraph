...
std::size_t minimum_spanning_tree(const graph32_t& graph,
    ...
    for (int iter = 0; iter < 40; iter++) {
        ...
        k_claim_vertex<<<(num_vertices + 255) / 256, 256, 0, stream>>>(
            cache.d_component, cache.d_comp_min_key, cache.d_v_min_wt, cache.d_v_min_dst,
            cache.d_comp_min_vertex, num_vertices);

        cudaMemsetAsync(cache.d_changed, 0, sizeof(int32_t), stream);

        k_add_and_merge<<<(num_vertices + 255) / 256, 256, 0, stream>>>(
            cache.d_component, cache.d_comp_min_vertex, cache.d_v_min_dst, cache.d_v_min_wt,
            cache.d_mst_src_temp, cache.d_mst_dst_temp, cache.d_mst_wt_temp,
            cache.d_mst_count, cache.d_changed, num_vertices);

        k_path_compress<<<(num_vertices + 255) / 256, 256, 0, stream>>>(cache.d_component, num_vertices);

        if ((iter + 1) % CHECK_INTERVAL == 0 || iter >= 38) {
            cudaMemcpyAsync(cache.h_changed_pinned, cache.d_changed, sizeof(int32_t), cudaMemcpyDeviceToHost, stream);
