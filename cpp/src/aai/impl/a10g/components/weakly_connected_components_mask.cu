...
void weakly_connected_components_mask(const graph32_t& graph,
    ...
    sample_hook_kernel<<<grid_v, block>>>(offsets, indices, edge_mask, components, num_vertices, 2);
    compress_kernel<<<grid_v, block>>>(components, num_vertices);
    edge_kernel<<<grid_e, block>>>(offsets, indices, edge_mask, components, num_vertices, num_edges);
    compress_kernel<<<grid_v, block>>>(components, num_vertices);
}
