...
similarity_result_float_t cosine_all_pairs_similarity_seg(const graph32_t& graph,
    ...
    int32_t bitmap_words = (graph_nv + 31) >> 5;
    ...
    if (topk.has_value()) {
        ...
        if (use_gmem) {
            count_kernel_gmem<512><<<num_seeds, 512, 0, stream>>>(
                d_offsets, d_indices, d_seeds, num_seeds,
                bitmap_words, cache.d_counts, cache.d_bitmaps);
        } else {
            int sm = get_smem(bitmap_words);
        ...
        cub::DeviceScan::ExclusiveSum(cache.d_temp_storage, temp_bytes,
                                      cache.d_counts, cache.d_off, num_seeds, stream);
        ...
        if (use_gmem) {
            output_kernel_gmem<512><<<num_seeds, 512, 0, stream>>>(
                ...
                cache.d_off, out_first, out_second, out_scores, total,
                cache.d_bitmaps);
        } else {
            int sm = get_smem(bitmap_words);
