...
struct Cache : Cacheable {
    ...
    void ensure_off(int64_t n) {
        if (off_cap < n) {
            if (off) cudaFree(off);
            cudaMalloc(&off, n * sizeof(int64_t));
            off_cap = n;
        }
    }
    ...
    void ensure_idx_buf(int64_t n) {
        if (idx_buf_cap < n) {
            if (idx_buf) cudaFree(idx_buf);
            cudaMalloc(&idx_buf, n * sizeof(int32_t));
            idx_buf_cap = n;
        }
    }
    ...
    void ensure_scores_sorted(int64_t n) {
        if (scores_sorted_cap < n) {
            if (scores_sorted) cudaFree(scores_sorted);
            cudaMalloc(&scores_sorted, n * sizeof(double));
            scores_sorted_cap = n;
        }
    }
    ...
    void ensure_idx_sorted(int64_t n) {
        if (idx_sorted_cap < n) {
            if (idx_sorted) cudaFree(idx_sorted);
            cudaMalloc(&idx_sorted, n * sizeof(int32_t));
            idx_sorted_cap = n;
        }
    }
...
static int get_smem(int32_t bw) { return bw * 4 + 1024; }
    ...
    std::optional<std::size_t> topk)
{
    ...
    cache.ensure_weight_sums(nv);
    {
        ...
        compute_weight_sums<<<grid, block, 0, stream>>>(
            d_offsets, edge_weights, cache.weight_sums, nv);
    }
    ...
    if (use_gmem) {
        count_kernel_gmem<512><<<num_seeds, 512, 0, stream>>>(
            ...
            cache.counts, cache.bitmaps);
    } else {
        int sm = get_smem(bitmap_words);
        cudaFuncSetAttribute(count_kernel<512>,
    ...
    cub::DeviceScan::ExclusiveSum(
        nullptr, temp_bytes, cache.counts, cache.off, num_seeds, stream);
    cache.ensure_prefix_temp(temp_bytes);
    cub::DeviceScan::ExclusiveSum(
        cache.d_prefix_temp, temp_bytes, cache.counts, cache.off,
        num_seeds, stream);

    
    cudaMemcpyAsync(&cache.h_pinned[0], cache.off + (num_seeds - 1),
                    sizeof(int64_t), cudaMemcpyDeviceToHost, stream);
    cudaMemcpyAsync(&cache.h_pinned[1], cache.counts + (num_seeds - 1),
                    sizeof(int64_t), cudaMemcpyDeviceToHost, stream);
    ...
    if (use_gmem) {
        output_kernel_gmem<512><<<num_seeds, 512, 0, stream>>>(
            d_offsets, d_indices, d_seeds, num_seeds, bitmap_words,
            cache.off, pair_first, pair_second, total, cache.bitmaps);
    } else {
        int sm = get_smem(bitmap_words);
        cudaFuncSetAttribute(output_kernel<512>,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, sm);
        output_kernel<512><<<num_seeds, 512, sm, stream>>>(
            d_offsets, d_indices, d_seeds, num_seeds, bitmap_words,
            cache.off, pair_first, pair_second, total);
    }
    ...
    bool use_topk = topk.has_value() &&
                    (static_cast<int64_t>(topk.value()) < num_pairs);

    if (use_topk) {
        int64_t k = static_cast<int64_t>(topk.value());

        cache.ensure_idx_buf(num_pairs);
        {
            int block = 256;
            int grid = (int)((num_pairs + block - 1) / block);
            iota_kernel<<<grid, block, 0, stream>>>(cache.idx_buf, num_pairs);
        }
        ...
        cub::DeviceRadixSort::SortPairsDescending(
            nullptr, sort_bytes, scores, cache.scores_sorted,
            cache.idx_buf, cache.idx_sorted, num_pairs, 0, 64, stream);
        cache.ensure_sort_temp(sort_bytes);
        cub::DeviceRadixSort::SortPairsDescending(
            cache.sort_temp, sort_bytes, scores, cache.scores_sorted,
            cache.idx_buf, cache.idx_sorted, num_pairs, 0, 64, stream);
...

        {
            ...
            gather_results<<<grid, block, 0, stream>>>(
                pair_first, pair_second, scores, cache.idx_sorted,
        ...
        return {final_first, final_second, final_scores,
                static_cast<std::size_t>(k)};
    }

    
    cudaStreamSynchronize(stream);
    return {pair_first, pair_second, scores,
            static_cast<std::size_t>(num_pairs)};
}
