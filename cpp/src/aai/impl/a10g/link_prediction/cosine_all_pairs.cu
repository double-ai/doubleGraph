...
similarity_result_float_t cosine_all_pairs_similarity(const graph32_t& graph,
    ...
    if (use_topk) {
        ...
        if (use_gmem) {
            launch_compute_topk_gmem(
                d_offsets, d_indices, num_verts, d_seeds, num_seeds,
                out_first, out_second, out_scores,
    ...
    if (use_gmem) {
        launch_count_gmem(d_offsets, d_indices, num_verts, d_seeds, num_seeds,
                         cache.base_offsets,
                         (unsigned long long*)cache.counter,
                         d_bitmaps, bitmap_words,
                         block_size, stream);
    } else {
        launch_count(d_offsets, d_indices, num_verts, d_seeds, num_seeds,
                     cache.base_offsets,
                     (unsigned long long*)cache.counter,
                     block_size, stream);
    }
    ...
    if (use_gmem) {
        launch_compute_gmem(d_offsets, d_indices, num_verts, d_seeds, num_seeds,
                           cache.base_offsets,
                           out_first, out_second,
                           d_bitmaps, bitmap_words,
                           block_size, stream);
    } else {
        launch_compute(d_offsets, d_indices, num_verts, d_seeds, num_seeds,
                       cache.base_offsets,
                       out_first, out_second,
                       block_size, stream);
    }
