...
void launch_set_source(int32_t* dist, float* sigma, int32_t* level_vertices,
                      int32_t source, cudaStream_t stream) {
    set_source_kernel<<<1, 1, 0, stream>>>(dist, sigma, level_vertices, source);
}
...
void launch_bfs_expand(
    ...
    int32_t* dist, float* sigma, int32_t current_level,
    cudaStream_t stream
) {
    if (frontier_size <= 0) return;
    const int block = 256;
    const int grid = ((int64_t)frontier_size * 32 + block - 1) / block;
    bfs_expand_kernel<<<grid, block, 0, stream>>>(
        offsets, indices, frontier, frontier_size,
        next_frontier, next_count, dist, sigma, current_level
    );
}

void launch_backward(
    const int32_t* offsets, const int32_t* indices,
    const int32_t* level_verts, int32_t level_size,
    ...
    const int block = 256;
    const int grid = ((int64_t)level_size * 32 + block - 1) / block;
    backward_kernel<<<grid, block, 0, stream>>>(
        offsets, indices, level_verts, level_size,
        dist, sigma, delta, edge_bc, current_level
    );
}
...
void edge_betweenness_centrality(const graph32_t& graph,
    ...
    cudaMemset(d_edge_bc, 0, num_edges * sizeof(float));
    ...
    for (std::size_t batch_start = 0; batch_start < num_samples; batch_start += actual_concurrent) {
        ...
        for (int i = 0; i < batch_size; i++) {
            ...
            launch_set_source(cache.d_dist[i], cache.d_sigma[i], cache.d_level_vertices[i], source, cache.streams[i]);
        ...
        while (any_active) {
            ...
            for (int i = 0; i < batch_size; i++) {
                ...
                launch_bfs_expand(
                    ...
                    cache.d_dist[i], cache.d_sigma[i], level, cache.streams[i]
                );
        ...
        for (int i = 0; i < batch_size; i++) {
            for (int d = (int)all_levels[i].size() - 1; d >= 0; d--) {
                ...
                launch_backward(
                    ...
                    cache.d_dist[i], cache.d_sigma[i], cache.d_delta[i], d_edge_bc, d, cache.streams[i]
                );
            }
        }
    ...
    if (has_scale && num_vertices > 1) {
        ...
        launch_scale(d_edge_bc, num_edges, 1.0f / scale_factor, 0);
