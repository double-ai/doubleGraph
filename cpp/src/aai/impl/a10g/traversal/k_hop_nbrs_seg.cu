...
void launch_compute_degrees_start(const int32_t* start_vertices, int64_t num_start,
    ...
    compute_degrees_start_kernel<<<calc_grid(num_start, 256), 256, 0, s>>>(
...
void launch_init_packed(const int32_t* sv, uint64_t* p, int64_t n, cudaStream_t s) {
    if (n == 0) return;
    init_packed_kernel<<<calc_grid(n, 256), 256, 0, s>>>(sv, p, n);
}

void launch_compute_degrees_packed(const uint64_t* p, int64_t n, const int32_t* csr_offsets, int64_t* d, cudaStream_t s) {
    if (n == 0) return;
    compute_degrees_packed_kernel<<<calc_grid(n, 256), 256, 0, s>>>(p, n, csr_offsets, d);
}
...
void launch_expand_packed(const uint64_t* p, const int64_t* wp, const int32_t* co, const int32_t* ci, uint64_t* np, int64_t n, cudaStream_t s) {
    if (n == 0) return;
    expand_packed_kernel<<<calc_grid(n, 256), 256, 0, s>>>(p, wp, co, ci, np, n);
}
...
void launch_bitmap_to_neighbors(const uint32_t* bm, int32_t bw, int64_t ns, const int64_t* offsets,
    ...
    if (ns == 0) return;
    bitmap_to_neighbors_kernel<<<(int)ns, 256, 0, s>>>(bm, bw, ns, offsets, neighbors);
}
...
static void ensure_buf(void*& buf, size_t& cur_size, size_t needed) {
    if (needed > cur_size) {
        if (buf) cudaFree(buf);
        cur_size = needed;
        cudaMalloc(&buf, cur_size);
    }
}
...
static k_hop_nbrs_result_t path_simple_k1(
    Cache& cache,
    const int32_t* csr_offsets, const int32_t* csr_indices,
    const int32_t* start_vertices, int64_t num_start,
    cudaStream_t stream)
{
    ...
    ensure_buf(cache.scan_temp, cache.scan_temp_size, get_scan_temp_size(num_start));
    ...
    int64_t total;
    cudaMemcpyAsync(&total, d_offsets + num_start, sizeof(int64_t), cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);

    int32_t* neighbors = nullptr;
    if (total > 0) {
        cudaMalloc(&neighbors, total * sizeof(int32_t));
        launch_expand_balanced(start_vertices, d_offsets,
                              csr_offsets, csr_indices,
                              neighbors, num_start, total, stream);
    }

    return {reinterpret_cast<std::size_t*>(d_offsets), neighbors,
            static_cast<std::size_t>(num_start + 1), static_cast<std::size_t>(total)};
}

static k_hop_nbrs_result_t path_sort_unique(
    Cache& cache,
    const int32_t* csr_offsets, const int32_t* csr_indices,
    const int32_t* start_vertices, int64_t num_start, int64_t k,
    cudaStream_t stream)
{
    int64_t frontier_size = num_start;
    int64_t* packed_buf = nullptr;
    cudaMalloc(&packed_buf, frontier_size * sizeof(int64_t));
    uint64_t* d_packed = reinterpret_cast<uint64_t*>(packed_buf);
    launch_init_packed(start_vertices, d_packed, frontier_size, stream);

    for (int64_t hop = 0; hop < k; hop++) {
        if (frontier_size == 0) break;

        int64_t* d_degrees = nullptr;
        cudaMalloc(&d_degrees, frontier_size * sizeof(int64_t));
        launch_compute_degrees_packed(d_packed, frontier_size, csr_offsets, d_degrees, stream);

        int64_t* d_write_pos = nullptr;
        cudaMalloc(&d_write_pos, frontier_size * sizeof(int64_t));
        ensure_buf(cache.scan_temp, cache.scan_temp_size, get_scan_temp_size(frontier_size));
        ...
        int64_t* expanded_buf = nullptr;
        cudaMalloc(&expanded_buf, total_expanded * sizeof(int64_t));
        uint64_t* d_expanded = reinterpret_cast<uint64_t*>(expanded_buf);
        launch_expand_packed(d_packed, d_write_pos, csr_offsets, csr_indices,
                            d_expanded, frontier_size, stream);

        cudaFree(d_write_pos);

        int64_t* sorted_buf = nullptr;
        cudaMalloc(&sorted_buf, total_expanded * sizeof(int64_t));
        uint64_t* d_sorted = reinterpret_cast<uint64_t*>(sorted_buf);
        ensure_buf(cache.sort_temp, cache.sort_temp_size, get_sort_temp_size(total_expanded));
        launch_radix_sort(d_expanded, d_sorted, total_expanded,
                         cache.sort_temp, cache.sort_temp_size, stream);

        cudaFree(expanded_buf);

        int64_t* unique_buf = nullptr;
        cudaMalloc(&unique_buf, total_expanded * sizeof(int64_t));
        uint64_t* d_unique = reinterpret_cast<uint64_t*>(unique_buf);
        ensure_buf(cache.unique_temp, cache.unique_temp_size, get_unique_temp_size(total_expanded));
        ...
        cudaMemcpy(&new_frontier_size, cache.d_num_selected, sizeof(int64_t), cudaMemcpyDeviceToHost);

        cudaFree(packed_buf);
        packed_buf = unique_buf;
        d_packed = d_unique;
    ...
    if (frontier_size == 0) {
        cudaFree(packed_buf);
        int64_t* offsets_out = nullptr;
        cudaMalloc(&offsets_out, (num_start + 1) * sizeof(int64_t));
        cudaMemsetAsync(offsets_out, 0, (num_start + 1) * sizeof(int64_t), stream);
        return {reinterpret_cast<std::size_t*>(offsets_out), nullptr,
                static_cast<std::size_t>(num_start + 1), 0};
    }

    int64_t* repacked_buf = nullptr;
    cudaMalloc(&repacked_buf, frontier_size * sizeof(int64_t));
    uint64_t* d_repacked = reinterpret_cast<uint64_t*>(repacked_buf);
    launch_repack(d_packed, d_repacked, frontier_size, stream);

    cudaFree(packed_buf);

    int64_t* sorted_final_buf = nullptr;
    cudaMalloc(&sorted_final_buf, frontier_size * sizeof(int64_t));
    uint64_t* d_sorted_final = reinterpret_cast<uint64_t*>(sorted_final_buf);
    ensure_buf(cache.sort_temp, cache.sort_temp_size, get_sort_temp_size(frontier_size));
    launch_radix_sort(d_repacked, d_sorted_final, frontier_size,
                     cache.sort_temp, cache.sort_temp_size, stream);

    cudaFree(repacked_buf);

    int64_t* offsets_out = nullptr;
    cudaMalloc(&offsets_out, (num_start + 1) * sizeof(int64_t));
    launch_extract_offsets(d_sorted_final, frontier_size, num_start, offsets_out, stream);

    int32_t* neighbors_out = nullptr;
    cudaMalloc(&neighbors_out, frontier_size * sizeof(int32_t));
    launch_extract_neighbors(d_sorted_final, neighbors_out, frontier_size, stream);

    cudaFree(sorted_final_buf);

    return {reinterpret_cast<std::size_t*>(offsets_out), neighbors_out,
            static_cast<std::size_t>(num_start + 1), static_cast<std::size_t>(frontier_size)};
}

static k_hop_nbrs_result_t path_bitmap(
    Cache& cache,
    const int32_t* csr_offsets, const int32_t* csr_indices,
    const int32_t* start_vertices, int64_t num_start, int64_t k,
    ...
    cudaStream_t stream)
{
    ...
    int64_t* frontier_buf = nullptr;
    int64_t frontier_size = 0;

    for (int64_t hop = 1; hop < k; hop++) {
        ...
        cudaMalloc(&frontier_buf, frontier_size * sizeof(int64_t));
        uint64_t* d_frontier = reinterpret_cast<uint64_t*>(frontier_buf);
    ...
    if (frontier_buf) { cudaFree(frontier_buf); frontier_buf = nullptr; }
    ...
    cudaMalloc(&offsets_out, (num_start + 1) * sizeof(int64_t));
    ensure_buf(cache.scan_temp, cache.scan_temp_size, get_scan_temp_size(num_start));
    launch_exclusive_scan(d_counts, offsets_out, num_start,
                         cache.scan_temp, cache.scan_temp_size, stream);

    int64_t last_off, last_cnt;
    cudaMemcpyAsync(&last_off, offsets_out + num_start - 1,
                   sizeof(int64_t), cudaMemcpyDeviceToHost, stream);
    cudaMemcpyAsync(&last_cnt, d_counts + num_start - 1,
                   sizeof(int64_t), cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
    ...
    cudaMemcpyAsync(offsets_out + num_start, &total_neighbors,
                   sizeof(int64_t), cudaMemcpyHostToDevice, stream);

    int32_t* neighbors_out = nullptr;
    if (total_neighbors > 0) {
        cudaMalloc(&neighbors_out, total_neighbors * sizeof(int32_t));
        launch_bitmap_to_neighbors(cur_bitmap, bitmap_words, num_start,
                                  offsets_out, neighbors_out, stream);
    }
    ...
    return {reinterpret_cast<std::size_t*>(offsets_out), neighbors_out,
            static_cast<std::size_t>(num_start + 1), static_cast<std::size_t>(total_neighbors)};
}
...
k_hop_nbrs_result_t k_hop_nbrs_seg(const graph32_t& graph,
    ...
    int64_t num_start = static_cast<int64_t>(num_start_vertices);
    int64_t k_val = static_cast<int64_t>(k);

    cudaStream_t stream = 0;

    if (k_val == 1 && !is_multigraph) {
        return path_simple_k1(cache, csr_offsets, csr_indices,
                             start_vertices, num_start, stream);
    }
    ...
    if (k_val > 1 && bitmap_bytes <= 2LL * 1024 * 1024 * 1024) {
        return path_bitmap(cache, csr_offsets, csr_indices,
                          start_vertices, num_start, k_val, num_vertices, stream);
    }

    return path_sort_unique(cache, csr_offsets, csr_indices,
                           start_vertices, num_start, k_val, stream);
}
