...
__global__ __launch_bounds__(256) void bfs_top_down_warp_kernel(
...
__global__ __launch_bounds__(256) void bfs_bottom_up_kernel(
    const int32_t* __restrict__ offsets,
    const int32_t* __restrict__ indices,
    const uint32_t* __restrict__ edge_mask,
    int32_t* __restrict__ distances,
...
struct Cache : Cacheable {
    ...
    int32_t* d_frontier_queue_0 = nullptr;
    int32_t* d_frontier_queue_1 = nullptr;
    ...
    void ensure_buffers(int32_t num_vertices) {
        ...
        if (queue_elems > max_queue_elems) {
            if (d_frontier_queue_0) cudaFree(d_frontier_queue_0);
            if (d_frontier_queue_1) cudaFree(d_frontier_queue_1);
            cudaMalloc(&d_frontier_queue_0, queue_elems * sizeof(int32_t));
            cudaMalloc(&d_frontier_queue_1, queue_elems * sizeof(int32_t));
...
void bfs_direction_optimizing_mask(const graph32_t& graph,
    ...
    int32_t* d_frontier_queue[2] = {cache.d_frontier_queue_0, cache.d_frontier_queue_1};
    ...
    launch_bfs_set_sources(distances, predecessors, cache.d_visited_bitmap, sources, static_cast<int32_t>(n_sources), compute_predecessors, stream);
    cudaMemcpyAsync(d_frontier_queue[0], sources, n_sources * sizeof(int32_t), cudaMemcpyDeviceToDevice, stream);

    int32_t frontier_size = static_cast<int32_t>(n_sources);
    int32_t depth = 0;
    int cur_q = 0;
    bool topdown = true;
    int64_t total_visited = static_cast<int64_t>(n_sources);
    ...
    while (frontier_size > 0) {
        ...
        if (topdown) {
            launch_bfs_top_down(
                ...
                d_frontier_queue[cur_q], d_frontier_queue[next_q],
            ...
            if (next_size < frontier_size &&
                ...
                topdown = true;
                cudaMemsetAsync(cache.d_counter, 0, sizeof(int32_t), stream);
                launch_bitmap_to_queue(cache.d_next_frontier_bitmap, d_frontier_queue[0], cache.d_counter, bitmap_words, stream);
                cudaStreamSynchronize(stream);
                cur_q = 0;
            }
