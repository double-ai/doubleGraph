...
double compute_mod(const int32_t* off, const int32_t* idx, const double* wt,
    ...
    modularity_kernel<<<gn, 256, 256*sizeof(double)>>>(off, idx, wt, comm, ps, n);
...
struct Cache : Cacheable {
    ...
    int64_t *red_keys;
    double *red_vals;
    int32_t *agg_src, *agg_comm, *new_comm;
    double *agg_delta, *old_sum, *self_loop;
    int32_t *seg_off;
    double *partials;
    void *sort_temp;
    size_t sort_temp_size;
    
    int64_t *ctr_keys1, *ctr_keys2;
    double *ctr_vals1, *ctr_vals2;
    void *ctr_sort_temp;
    size_t ctr_sort_temp_size;
    int64_t *ctr_red_keys;
    double *ctr_red_vals;

    int32_t alloc_n, alloc_e;

    Cache() : keys1(nullptr), keys2(nullptr), vals1(nullptr), vals2(nullptr),
        red_keys(nullptr), red_vals(nullptr),
        agg_src(nullptr), agg_comm(nullptr), new_comm(nullptr),
        agg_delta(nullptr), old_sum(nullptr), self_loop(nullptr),
        seg_off(nullptr), partials(nullptr),
        sort_temp(nullptr), sort_temp_size(0),
        ctr_keys1(nullptr), ctr_keys2(nullptr),
        ctr_vals1(nullptr), ctr_vals2(nullptr),
        ctr_sort_temp(nullptr), ctr_sort_temp_size(0),
        ctr_red_keys(nullptr), ctr_red_vals(nullptr),
        alloc_n(0), alloc_e(0) {}
    ...
    void ensure(int32_t n, int32_t ne) {
        ...
        cudaMalloc(&partials, ((n+255)/256) * 8);
...
ecg_result_double_t ecg(const graph32_t& graph,
    ...
    const double* d_ew = edge_weights;
    ...
    for (std::size_t i = 0; i < ensemble_size; i++) {
        ...
        int dl; double dm;
        louvain_impl(d_off, d_idx, d_ew, nv, ne,
    ...
    double tw = thrust::reduce(thrust::device_pointer_cast(d_ew),
        thrust::device_pointer_cast(d_ew+ne), 0.0);
    thrust::device_vector<double> ovw(nv);
    compute_vw_kernel<<<(nv+255)/256, 256>>>(d_off, d_ew,
