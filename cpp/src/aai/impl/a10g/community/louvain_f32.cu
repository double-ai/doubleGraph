...
struct Cache : Cacheable {
    int32_t* community = nullptr;
    float* kk = nullptr;
    float* sigma_tot = nullptr;
    int32_t* d_changed = nullptr;
    float* d_mod_tmp = nullptr;
    int32_t* edge_src = nullptr;
    int32_t* edge_dst = nullptr;
    float* edge_w = nullptr;

    int64_t community_capacity = 0;
    int64_t kk_capacity = 0;
    int64_t sigma_tot_capacity = 0;
    bool d_changed_allocated = false;
    bool d_mod_tmp_allocated = false;
    int64_t edge_src_capacity = 0;
    int64_t edge_dst_capacity = 0;
    int64_t edge_w_capacity = 0;

    void ensure(int64_t n) {
        if (community_capacity < n) {
            if (community) cudaFree(community);
            cudaMalloc(&community, n * sizeof(int32_t));
            community_capacity = n;
        }
        if (kk_capacity < n) {
            if (kk) cudaFree(kk);
            cudaMalloc(&kk, n * sizeof(float));
            kk_capacity = n;
        }
        if (sigma_tot_capacity < n) {
            if (sigma_tot) cudaFree(sigma_tot);
            cudaMalloc(&sigma_tot, n * sizeof(float));
            sigma_tot_capacity = n;
        }
        if (!d_changed_allocated) {
            cudaMalloc(&d_changed, sizeof(int32_t));
            d_changed_allocated = true;
        }
        if (!d_mod_tmp_allocated) {
            cudaMalloc(&d_mod_tmp, 2 * sizeof(float));
            d_mod_tmp_allocated = true;
        }
    }

    void ensure_edge_bufs(int64_t n) {
        ...
        if (edge_w_capacity < n) {
            if (edge_w) cudaFree(edge_w);
            cudaMalloc(&edge_w, n * sizeof(float));
            edge_w_capacity = n;
        }
    }
    ...
    ~Cache() override {
        ...
        if (d_mod_tmp) cudaFree(d_mod_tmp);
        if (edge_src) cudaFree(edge_src);
        if (edge_dst) cudaFree(edge_dst);
        if (edge_w) cudaFree(edge_w);
    }
...
louvain_result_float_t louvain(const graph32_t& graph,
    ...
    if (num_vertices <= 1 || num_edges == 0) {
        if (num_vertices >= 1) thrust::sequence(thrust::device, clusters, clusters + num_vertices);
    ...
    int32_t* community = cache.community;
    float* kk = cache.kk;
    float* sigma_tot = cache.sigma_tot;
    int32_t* d_changed = cache.d_changed;
    float* d_mod_tmp = cache.d_mod_tmp;
    ...
    thrust::sequence(thrust::device, clusters, clusters + num_vertices);

    int64_t level = 0;
    float prev_modularity = -1e30f;

    
    float avg_degree = (float)num_edges / num_vertices;
    ...
    for (int64_t lvl = 0; lvl < effective_max_level; lvl++) {
        ...
        if (cur_n <= 200) {
            ...
            for (int iter = 0; iter < 100; iter++) {
                ...
                if (cur_avg_deg >= 16) {
                    ...
                    int num_warps = cur_n;
                    int grid = (int)(((int64_t)num_warps * 32 + BLOCK_SIZE - 1) / BLOCK_SIZE);
        ...
        update_mapping_kernel<<<(num_vertices + 255) / 256, 256>>>(clusters, community, num_vertices);
        cudaFree(used_flags); cudaFree(new_ids);

        if (new_n >= cur_n || new_n <= 1) break;

        
        cache.ensure_edge_bufs(cur_edges);
        int32_t* edge_src = cache.edge_src;
        int32_t* edge_dst = cache.edge_dst;
        float* edge_w = cache.edge_w;
    ...
    compute_sigma_tot_kernel<<<(num_vertices + 255) / 256, 256>>>(clusters, orig_k, orig_sigma, num_vertices);

    int32_t max_comm = thrust::reduce(thrust::device_pointer_cast(clusters),
        thrust::device_pointer_cast(clusters + num_vertices), (int32_t)0, thrust::maximum<int32_t>());
    int32_t nc = max_comm + 1;
    float* d_internal; cudaMalloc(&d_internal, nc * sizeof(float));
    cudaMemset(d_internal, 0, nc * sizeof(float));

    const int32_t* cl = clusters;
    ...
    return louvain_result_float_t{(std::size_t)level, Q};
}
