...
struct Cache : Cacheable {
    ...
    double* tw = nullptr;       int64_t tw_cap = 0;
    ...
    void ensure_vw(int64_t n) { if (vw_cap < n) { if (vw) cudaFree(vw); cudaMalloc(&vw, n * sizeof(double)); vw_cap = n; } }
    void ensure_cw(int64_t n) { if (cw_cap < n) { if (cw) cudaFree(cw); cudaMalloc(&cw, n * sizeof(double)); cw_cap = n; } }
    void ensure_flag(int64_t n) { if (flag_cap < n) { if (flag) cudaFree(flag); cudaMalloc(&flag, n * sizeof(int32_t)); flag_cap = n; } }
    void ensure_par(int64_t n) { if (par_cap < n) { if (par) cudaFree(par); cudaMalloc(&par, n * sizeof(int32_t)); par_cap = n; } }
    void ensure_tmp(int64_t n) { if (tmp_cap < n) { if (tmp) cudaFree(tmp); cudaMalloc(&tmp, n * sizeof(int32_t)); tmp_cap = n; } }
    void ensure_tw(int64_t n) { if (tw_cap < n) { if (tw) cudaFree(tw); cudaMalloc(&tw, n * sizeof(double)); tw_cap = n; } }
    void ensure_comm(int64_t n) { if (comm_cap < n) { if (comm) cudaFree(comm); cudaMalloc(&comm, n * sizeof(int32_t)); comm_cap = n; } }
    void ensure_flat(int64_t n) { if (flat_cap < n) { if (flat) cudaFree(flat); cudaMalloc(&flat, n * sizeof(int32_t)); flat_cap = n; } }
    void ensure_ref(int64_t n) { if (ref_cap < n) { if (ref) cudaFree(ref); cudaMalloc(&ref, n * sizeof(int32_t)); ref_cap = n; } }
    void ensure_rw(int64_t n) { if (rw_cap < n) { if (rw) cudaFree(rw); cudaMalloc(&rw, n * sizeof(double)); rw_cap = n; } }
    void ensure_kb(int64_t n) { if (kb_cap < n) { if (kb) cudaFree(kb); cudaMalloc(&kb, n * sizeof(long long)); kb_cap = n; } }
    void ensure_vb(int64_t n) { if (vb_cap < n) { if (vb) cudaFree(vb); cudaMalloc(&vb, n * sizeof(double)); vb_cap = n; } }
