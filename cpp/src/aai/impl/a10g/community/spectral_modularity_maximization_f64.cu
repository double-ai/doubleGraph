...
struct Cache : Cacheable {
    ...
    void ensure_scratch(size_t total) {
        if (scratch_capacity < total) {
            if (scratch) cudaFree(scratch);
            cudaMalloc(&scratch, total * sizeof(double));
...
void spectral_modularity_maximization(const graph32_t& graph,
    ...
    cublasHandle_t cublas_h = cache.cublas_h;
    cusparseHandle_t cusparse_h = cache.cusparse_h;
    cusolverDnHandle_t cusolver_h = cache.cusolver_h;
