...
struct Cache : Cacheable {
    ...
    int32_t* count_buf = nullptr;  

    int64_t orig_vw_capacity = 0;
    int64_t final_comm_capacity = 0;
    int64_t tw_buf_capacity = 0;
    int64_t count_buf_capacity = 0;
    ...
    void ensure_base(int64_t N) {
        if (orig_vw_capacity < N) {
            if (orig_vw) cudaFree(orig_vw);
            cudaMalloc(&orig_vw, N * sizeof(float));
            orig_vw_capacity = N;
        }
        if (final_comm_capacity < N) {
            if (final_comm) cudaFree(final_comm);
            cudaMalloc(&final_comm, N * sizeof(int32_t));
            final_comm_capacity = N;
        }
        if (tw_buf_capacity < 1) {
            if (tw_buf) cudaFree(tw_buf);
            cudaMalloc(&tw_buf, sizeof(float));
            tw_buf_capacity = 1;
        }
        if (count_buf_capacity < 1) {
            if (count_buf) cudaFree(count_buf);
            cudaMalloc(&count_buf, sizeof(int32_t));
            count_buf_capacity = 1;
        }
    }
...
louvain_result_float_t louvain_seg(const graph32_t& graph,
    ...
    cache.ensure_base(orig_N);

    
    launch_compute_vertex_weights(offsets, edge_weights, cache.orig_vw, orig_N);

    launch_reduce_sum(cache.orig_vw, cache.tw_buf, orig_N);
    float total_weight;
    ...
    int* d_count = (int*)cache.count_buf;
    ...
    if (orig_N <= 256) {
        ...
        launch_batch_sequential_louvain(
            offsets, indices, edge_weights, cache.orig_vw,
        ...
        for (int ki = 0; ki < topK; ki++) {
            ...
            const float* cur_vw = cache.orig_vw;
            ...
            for (int level = 0; level < (int)max_level; level++) {
                ...
                float new_Q = launch_compute_modularity(offsets, indices,
                    edge_weights, trial_comm, cache.orig_vw, orig_N, total_weight, resolution);
    ...
    
    {
        ...
        
        {
            ...
            for (int iter = 0; iter < refine_max; iter++) {
                ...
                for (int c = 0; c < 2; c++) {
                    launch_recompute_sigma_tot(d_par, cache.orig_vw, ref_sigma, orig_N);
                    launch_local_move(offsets, indices, edge_weights,
                        d_par, ref_sigma, cache.orig_vw,
        ...
        float par_Q = launch_compute_modularity(offsets, indices,
            edge_weights, d_par, cache.orig_vw, orig_N, total_weight, resolution);
    ...
    float modularity = launch_compute_modularity(
        offsets, indices, edge_weights, d_final, cache.orig_vw,
        orig_N, total_weight, resolution);

    cudaMemcpy(clusters, d_final, orig_N * sizeof(int32_t), cudaMemcpyDeviceToDevice);

    return {(std::size_t)actual_level_count, modularity};
}
