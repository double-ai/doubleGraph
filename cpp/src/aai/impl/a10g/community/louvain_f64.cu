...
struct Cache : Cacheable {
    ...
    int32_t edge_weights_f_cap = 0;

    float* orig_vw = nullptr;
    int32_t orig_vw_cap = 0;

    float* tw_buf = nullptr;  
    bool tw_buf_alloc = false;

    int32_t* final_comm = nullptr;
    int32_t final_comm_cap = 0;

    int* count_buf = nullptr;  
    bool count_buf_alloc = false;

    
    int32_t* all_comm = nullptr;
    int64_t all_comm_cap = 0;

    float* all_sigma = nullptr;
    int64_t all_sigma_cap = 0;

    float* all_mod = nullptr;
    int32_t all_mod_cap = 0;

    int32_t* trial_comm = nullptr;
    int32_t trial_comm_cap = 0;

    int* level_changed = nullptr;
    bool level_changed_alloc = false;

    
    int32_t* par_comm = nullptr;
    int32_t par_comm_cap = 0;

    float* ref_sigma = nullptr;
    int32_t ref_sigma_cap = 0;

    int* ref_changed = nullptr;
    bool ref_changed_alloc = false;

    double* d_accum = nullptr;
    bool d_accum_alloc = false;

    float* mod_sigma = nullptr;
    int32_t mod_sigma_cap = 0;
    ...
    ~Cache() override {
        ...
        if (final_comm) cudaFree(final_comm);
        if (count_buf) cudaFree(count_buf);
        if (all_comm) cudaFree(all_comm);
        if (all_sigma) cudaFree(all_sigma);
        if (all_mod) cudaFree(all_mod);
        if (trial_comm) cudaFree(trial_comm);
        if (level_changed) cudaFree(level_changed);
        if (par_comm) cudaFree(par_comm);
        if (ref_sigma) cudaFree(ref_sigma);
        if (ref_changed) cudaFree(ref_changed);
    ...
    void ensure_edge_weights_f(int32_t n) {
        if (edge_weights_f_cap < n) {
            if (edge_weights_f) cudaFree(edge_weights_f);
            cudaMalloc(&edge_weights_f, (size_t)n * sizeof(float));
            edge_weights_f_cap = n;
        }
    }

    void ensure_orig_vw(int32_t n) {
        if (orig_vw_cap < n) {
            if (orig_vw) cudaFree(orig_vw);
            cudaMalloc(&orig_vw, (size_t)n * sizeof(float));
            orig_vw_cap = n;
        }
    }
    ...
    void ensure_final_comm(int32_t n) {
        if (final_comm_cap < n) {
            if (final_comm) cudaFree(final_comm);
            cudaMalloc(&final_comm, (size_t)n * sizeof(int32_t));
            final_comm_cap = n;
        }
    }
    ...
    void ensure_count_buf() {
        if (!count_buf_alloc) {
            ...
            count_buf_alloc = true;
        }
    }

    void ensure_all_comm(int64_t n) {
        if (all_comm_cap < n) {
            if (all_comm) cudaFree(all_comm);
            cudaMalloc(&all_comm, (size_t)n * sizeof(int32_t));
            all_comm_cap = n;
        }
    }

    void ensure_all_sigma(int64_t n) {
        if (all_sigma_cap < n) {
            if (all_sigma) cudaFree(all_sigma);
            cudaMalloc(&all_sigma, (size_t)n * sizeof(float));
            all_sigma_cap = n;
        }
    }

    void ensure_all_mod(int32_t n) {
        if (all_mod_cap < n) {
            if (all_mod) cudaFree(all_mod);
            cudaMalloc(&all_mod, (size_t)n * sizeof(float));
            all_mod_cap = n;
        }
    }

    void ensure_trial_comm(int32_t n) {
        if (trial_comm_cap < n) {
            if (trial_comm) cudaFree(trial_comm);
            cudaMalloc(&trial_comm, (size_t)n * sizeof(int32_t));
            trial_comm_cap = n;
        }
    }

    void ensure_level_changed() {
        if (!level_changed_alloc) {
            cudaMalloc(&level_changed, sizeof(int));
            level_changed_alloc = true;
        }
    }

    void ensure_par_comm(int32_t n) {
        if (par_comm_cap < n) {
            if (par_comm) cudaFree(par_comm);
            cudaMalloc(&par_comm, (size_t)n * sizeof(int32_t));
            par_comm_cap = n;
        }
    }

    void ensure_ref_sigma(int32_t n) {
        if (ref_sigma_cap < n) {
            if (ref_sigma) cudaFree(ref_sigma);
            cudaMalloc(&ref_sigma, (size_t)n * sizeof(float));
            ref_sigma_cap = n;
        }
    }

    void ensure_ref_changed() {
        if (!ref_changed_alloc) {
            cudaMalloc(&ref_changed, sizeof(int));
            ref_changed_alloc = true;
        }
    }
    ...
    void ensure_mod_sigma(int32_t n) {
        if (mod_sigma_cap < n) {
            if (mod_sigma) cudaFree(mod_sigma);
            cudaMalloc(&mod_sigma, (size_t)n * sizeof(float));
            mod_sigma_cap = n;
        }
    }
};
...
static int do_renumber_communities(int32_t* community, int* used, int* prefix, int N, Cache& cache) {
    int grid = (N + BLOCK_SIZE - 1) / BLOCK_SIZE;
...
static float do_compute_modularity(
    ...
    cache.ensure_mod_sigma(N);
    cudaMemset(cache.mod_sigma, 0, N * sizeof(float));
    recompute_sigma_tot_kernel<<<(N + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(clusters, vw, cache.mod_sigma, N);

    cache.ensure_d_accum();

    cudaMemset(cache.d_accum, 0, sizeof(double));
    int g = (N + BLOCK_SIZE - 1) / BLOCK_SIZE;
    if (g > 1024) g = 1024;
    compute_iw_kernel<<<g, BLOCK_SIZE>>>(offsets, indices, weights, clusters, N, cache.d_accum);
    double iw;
    cudaMemcpy(&iw, cache.d_accum, sizeof(double), cudaMemcpyDeviceToHost);

    cudaMemset(cache.d_accum, 0, sizeof(double));
    compute_ssq_kernel<<<g, BLOCK_SIZE>>>(cache.mod_sigma, N, cache.d_accum);
    double ssq;
    cudaMemcpy(&ssq, cache.d_accum, sizeof(double), cudaMemcpyDeviceToHost);
...
louvain_result_double_t louvain(const graph32_t& graph,
    ...
    const int32_t* g_offsets = graph.offsets;
    const int32_t* g_indices = graph.indices;
    ...
    float* ew_f = cache.edge_weights_f;

    
    cache.ensure_orig_vw(orig_N);
    do_compute_vertex_weights(g_offsets, ew_f, cache.orig_vw, orig_N);
    ...
    cache.ensure_final_comm(orig_N);
    int32_t* d_final = cache.final_comm;
    do_init_communities(d_final, orig_N);

    if (total_weight == 0.0f || orig_N <= 1) {
        cudaMemcpy(clusters, d_final, orig_N * sizeof(int32_t), cudaMemcpyDeviceToDevice);
        return {0, 0.0};
    }
    ...
    if (orig_N <= 256) {
        ...
        cache.ensure_all_comm((int64_t)num_restarts * orig_N);
        cache.ensure_all_sigma((int64_t)num_restarts * orig_N);
        cache.ensure_all_mod(num_restarts);
        do_batch_sequential_louvain(g_offsets, g_indices,
            ew_f, orig_vw, orig_N, total_weight, resolution_f,
            cache.all_comm, cache.all_sigma,
            cache.all_mod, num_restarts);
        cudaDeviceSynchronize();

        std::vector<float> h_mod(num_restarts);
        cudaMemcpy(h_mod.data(), cache.all_mod, num_restarts * sizeof(float), cudaMemcpyDeviceToHost);
        ...
        cache.ensure_trial_comm(orig_N);
        cache.ensure_level_changed();
        int* d_lc = cache.level_changed;

        for (int ki = 0; ki < topK; ki++) {
            int idx = sorted_idx[ki];
            cudaMemcpy(cache.trial_comm,
                       cache.all_comm + (int64_t)idx * orig_N,
                       orig_N * sizeof(int32_t), cudaMemcpyDeviceToDevice);

            
            int32_t* ml_comm = nullptr;
            cudaMalloc(&ml_comm, orig_N * sizeof(int32_t));
            cudaMemcpy(ml_comm, cache.trial_comm,
                       orig_N * sizeof(int32_t), cudaMemcpyDeviceToDevice);

            int cur_N = orig_N, cur_E = orig_E;
            const int32_t* cur_off = g_offsets;
            const int32_t* cur_idx2 = g_indices;
            const float* cur_wt = ew_f;
            ...
            for (int level = 0; level < (int)max_level; level++) {
                int* used = nullptr;
                int* pref = nullptr;
                cudaMalloc(&used, cur_N * sizeof(int));
                cudaMalloc(&pref, cur_N * sizeof(int));
                int new_N = do_renumber_communities(ml_comm, used, pref, cur_N, cache);
                cudaFree(used);
                cudaFree(pref);
                ...
                if (first_level) {
                    cudaMemcpy(cache.trial_comm, ml_comm, orig_N * sizeof(int32_t), cudaMemcpyDeviceToDevice);
                    first_level = false;
                } else {
                    do_compose_communities(cache.trial_comm, ml_comm, orig_N);
                }

                int64_t* ek = nullptr; float* ev = nullptr;
                ...
                int64_t* sk = nullptr; float* sv = nullptr;
                int64_t* uk = nullptr; float* uv = nullptr;
                int* nu = nullptr;
                ...
                int new_E = do_sort_reduce_edges(ek, ev, cur_E, sk, sv, uk, uv, nu, cache);
                cudaFree(ek); cudaFree(ev); cudaFree(sk); cudaFree(sv);
                ...
                c_off = nullptr; c_idx = nullptr; c_wt = nullptr;
                cudaMalloc(&c_off, (size_t)(new_N + 1) * sizeof(int32_t));
                cudaMalloc(&c_idx, (size_t)new_E * sizeof(int32_t));
                cudaMalloc(&c_wt, (size_t)new_E * sizeof(float));
                int* cnt = nullptr;
                cudaMalloc(&cnt, (size_t)new_N * sizeof(int));
                do_build_csr(uk, uv, new_E, new_N, cnt, c_off, c_idx, c_wt, cache);
                cudaFree(uk); cudaFree(uv); cudaFree(nu); cudaFree(cnt);

                c_vw = nullptr;
                cudaMalloc(&c_vw, (size_t)new_N * sizeof(float));
                ...
                do_compose_communities(cache.trial_comm, ml_comm, orig_N);
                float new_Q = do_compute_modularity(g_offsets, g_indices,
                    ew_f, cache.trial_comm, orig_vw, orig_N, total_weight, resolution_f, cache);
                if (new_Q <= level_best) break;
                level_best = new_Q;
            }

            float trial_Q = do_compute_modularity(g_offsets, g_indices,
                ew_f, cache.trial_comm, orig_vw, orig_N, total_weight, resolution_f, cache);
            if (trial_Q > overall_best_Q) {
                overall_best_Q = trial_Q;
                cudaMemcpy(d_final, cache.trial_comm, orig_N * sizeof(int32_t), cudaMemcpyDeviceToDevice);
            }

            
            if (ml_comm) cudaFree(ml_comm);
    ...
    {
        cache.ensure_par_comm(orig_N);
        int32_t* d_par = cache.par_comm;
        ...
        for (int level = 0; level < (int)max_level; level++) {
            ...
            float* sigma = nullptr;
            int32_t* bm_comm = nullptr;
            float* bm_gain = nullptr;
            int32_t* best_saved = nullptr;
            cudaMalloc(&comm, (size_t)cur_N * sizeof(int32_t));
            cudaMalloc(&sigma, (size_t)cur_N * sizeof(float));
            cudaMalloc(&bm_comm, (size_t)cur_N * sizeof(int32_t));
            cudaMalloc(&bm_gain, (size_t)cur_N * sizeof(float));
            cudaMalloc(&best_saved, (size_t)cur_N * sizeof(int32_t));

            do_init_communities(comm, cur_N);
            float new_Q = do_compute_modularity(cur_off, cur_idx2, cur_wt, comm,
                cur_vw, cur_N, total_weight, resolution_f, cache);
            float cur_Q = new_Q - 1;
            ...
            while (new_Q > cur_Q + threshold_f) {
                cur_Q = new_Q;
                do_recompute_sigma_tot(comm, cur_vw, sigma, cur_N);
                do_compute_best_move(cur_off, cur_idx2, cur_wt, comm, sigma,
                    cur_vw, cur_N, total_weight, resolution_f, bm_comm, bm_gain);
                cudaMemset(d_count, 0, sizeof(int));
                do_count_updown(comm, bm_comm, bm_gain, cur_N, up_down, d_count);
                int h_count;
                cudaMemcpy(&h_count, d_count, sizeof(int), cudaMemcpyDeviceToHost);
                bool eff = up_down; if (h_count == 0) eff = !up_down;
                do_apply_updown(comm, bm_comm, bm_gain, cur_N, eff);
                up_down = !up_down;
                new_Q = do_compute_modularity(cur_off, cur_idx2, cur_wt, comm,
                    cur_vw, cur_N, total_weight, resolution_f, cache);
                if (new_Q > cur_Q) cudaMemcpy(best_saved, comm, cur_N * sizeof(int32_t), cudaMemcpyDeviceToDevice);
            }
            cudaMemcpy(comm, best_saved, cur_N * sizeof(int32_t), cudaMemcpyDeviceToDevice);

            bool should_break = (cur_Q <= best_modularity);
            ...
            cudaMalloc(&used, cur_N * sizeof(int));
            cudaMalloc(&pref, cur_N * sizeof(int));
            int new_N = do_renumber_communities(comm, used, pref, cur_N, cache);
            cudaFree(used);
            cudaFree(pref);
            ...
            if (level == 0) cudaMemcpy(d_par, comm, orig_N * sizeof(int32_t), cudaMemcpyDeviceToDevice);
            else do_compose_communities(d_par, comm, orig_N);
            par_levels = level + 1;
            ...
            cudaMalloc(&ek, (size_t)cur_E * sizeof(int64_t));
            cudaMalloc(&ev, (size_t)cur_E * sizeof(float));
            do_map_edges(cur_off, cur_idx2, comm, ek, cur_N);
            cudaMemcpy(ev, cur_wt, cur_E * sizeof(float), cudaMemcpyDeviceToDevice);
            ...
            c_vw = nullptr;
            cudaMalloc(&c_vw, (size_t)new_N * sizeof(float));
            do_compute_new_vw(cur_vw, comm, c_vw, cur_N, new_N);
        ...
        {
            cache.ensure_ref_sigma(orig_N);
            cache.ensure_ref_changed();
            int* d_ref_changed = cache.ref_changed;
            int refine_max = (orig_N > 1000000) ? 10 : 50;
            for (int iter = 0; iter < refine_max; iter++) {
                cudaMemset(d_ref_changed, 0, sizeof(int));
                for (int c = 0; c < 2; c++) {
                    do_recompute_sigma_tot(d_par, orig_vw, cache.ref_sigma, orig_N);
                    do_local_move(g_offsets, g_indices, ew_f,
                        d_par, cache.ref_sigma, orig_vw,
        ...
        float par_Q = do_compute_modularity(g_offsets, g_indices,
            ew_f, d_par, orig_vw, orig_N, total_weight, resolution_f, cache);
        if (par_Q > overall_best_Q) {
            overall_best_Q = par_Q;
            cudaMemcpy(d_final, d_par, orig_N * sizeof(int32_t), cudaMemcpyDeviceToDevice);
    ...
    do_renumber_communities(d_final, ru, rp, orig_N, cache);
    cudaFree(ru); cudaFree(rp);

    float modularity = do_compute_modularity(g_offsets, g_indices,
        ew_f, d_final, orig_vw, orig_N, total_weight, resolution_f, cache);
