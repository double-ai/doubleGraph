...
__global__ void count_indegrees_match_any_k(const int32_t* __restrict__ indices, int32_t ne,
    ...
    for (int i = tid; i < ne; i += stride) {
        ...
        unsigned m = __match_any_sync(am, dst);
        int leader = __ffs(m) - 1;
        if (lane == leader) {
            atomicAdd(&indeg[dst], __popc(m));
        }
    }
}

__global__ void scatter_transpose_warp_k(const int32_t* __restrict__ offsets,
                                        const int32_t* __restrict__ indices,
                                        int32_t nv,
                                        int32_t* __restrict__ rev_off_tmp,
                                        int32_t* __restrict__ rev_indices) {
    int global_tid = blockIdx.x * blockDim.x + threadIdx.x;
    int warp = global_tid >> 5;
    int lane = global_tid & 31;
    int warps_total = (gridDim.x * blockDim.x) >> 5;

    for (int u = warp; u < nv; u += warps_total) {
        int start = offsets[u];
        int end = offsets[u + 1];
...
void launch_build_transpose_atomic(const int32_t* offsets, const int32_t* indices,
    ...
    if (ne > 0) {
        int G = (ne + B - 1) / B;
        if (G > 4096) G = 4096;
        count_indegrees_match_any_k<<<G, B, 0, stream>>>(indices, ne, indeg);
    }
    ...
    if (nv > 0) {
        int warps = nv;
        int G = (warps + (B / 32) - 1) / (B / 32);
        if (G < 1) G = 1;
        if (G > 4096) G = 4096;
        scatter_transpose_warp_k<<<G, B, 0, stream>>>(offsets, indices, nv, rev_off_tmp, rev_indices);
    }
}
...
struct Cache : Cacheable {
    ...
    void gpu_cc(const int32_t* doff, const int32_t* didx, int32_t nv, int32_t ne, int32_t* dc) {
        launch_cc_label_prop(doff, didx, nv, dc, d_flag, h_flag, MAX_CC_ITERS, sc);
    ...
    void gpu_scc(const int32_t* doff, const int32_t* didx, int32_t nv, int32_t ne,
        ...
        const int32_t* rev_off_ptr = d_rev_off;
        const int32_t* rev_idx_ptr = d_rev_idx;

        int32_t pivot = 0;

        launch_bfs_converge_segmented(doff, didx, rev_off_ptr, rev_idx_ptr,
                                       nv, d_fw_dist, d_bw_dist, pivot, MAX_BFS_LEVELS,
                                       d_flag, h_flag, seg_mid, sc);
        launch_mark_scc(nv, d_fw_dist, d_bw_dist, dc, pivot, sc);
        ...
        if (remaining > 0 && remaining < nv * 3 / 4) {
            for (int round = 1; round < MAX_FWBW_ROUNDS && remaining > 0; round++) {
                ...
                int32_t new_pivot = launch_find_pivot(dc, doff, nv, d_flag, h_flag, sc);
                ...
                launch_bfs_converge_segmented_restricted(doff, didx, rev_off_ptr, rev_idx_ptr,
                                                          ...
                                                          d_flag, h_flag, seg_mid, sc);
                launch_mark_scc_restricted(nv, d_fw_dist, d_bw_dist, dc, new_pivot, sc);
        ...
        if (remaining > 0) {
            ...
            cudaMemcpyAsync(dc, h_components, nv * sizeof(int32_t), cudaMemcpyHostToDevice, sc);
            cudaStreamSynchronize(sc);
        } else {
            cudaStreamSynchronize(sc);
        }
...
void strongly_connected_components_seg(const graph32_t& graph,
    ...
    int32_t seg_mid = nv;
    if (graph.segment_offsets.has_value()) {
        seg_mid = graph.segment_offsets.value()[2];
    }
