...
struct Cache : Cacheable {
    ...
    void ensure_scan_temp(size_t needed) {
    ...
    void ensure_unique_temp(size_t needed) {
    ...
    int64_t* __restrict__ degrees)
{
    for (int64_t i = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
        ...
        int32_t v = __ldg(start_vertices + i);
        degrees[i] = (int64_t)(__ldg(csr_offsets + v + 1) - __ldg(csr_offsets + v));
    }
}
...
__global__ __launch_bounds__(EXPAND_BLK, 6)
void expand_balanced_kernel(
    const int32_t* __restrict__ start_vertices,
    ...
    int64_t total_output)
{
    ...
    if (threadIdx.x == 0) {
        ...
        while (lo < hi) {
            ...
            if (__ldg(offsets + mid) <= block_out_start) lo = mid;
            else hi = mid - 1;
        }
        s_seg_start = lo;
        ...
        while (lo < hi) {
            int64_t mid = lo + (hi - lo + 1) / 2;
            if (__ldg(offsets + mid) <= block_out_end - 1) lo = mid;
            else hi = mid - 1;
        }
        s_seg_end = lo + 1;
    }
    ...
    int num_segs = (int)(seg_end - seg_start);
    ...
    for (int i = threadIdx.x; i <= num_segs; i += EXPAND_BLK) {
        int64_t si = seg_start + i;
        s_offsets[i] = __ldg(offsets + si);
        if (i < num_segs) {
            int32_t v = __ldg(start_vertices + si);
            s_csr_start[i] = __ldg(csr_offsets + v);
        }
    }
    __syncthreads();
    ...
    for (int item = 0; item < ITEMS_PER_THR; item++) {
        int64_t out_idx = block_out_start + threadIdx.x + (int64_t)item * EXPAND_BLK;
        if (out_idx >= block_out_end) break;

        
        int lo = prev_seg, hi = num_segs - 1;
        while (lo < hi) {
        ...
        int32_t local = (int32_t)(out_idx - s_offsets[lo]);
        neighbors[out_idx] = __ldg(csr_indices + s_csr_start[lo] + local);
    }
}
    ...
    int32_t bitmap_words)
{
    for (int64_t i = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
        ...
        int32_t start_e = __ldg(csr_offsets + v);
        int32_t end_e = __ldg(csr_offsets + v + 1);
        for (int32_t j = start_e; j < end_e; j++) {
            int32_t nbr = __ldg(csr_indices + j);
    ...
    int32_t bitmap_words)
{
    for (int64_t i = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
        ...
        uint64_t p = __ldg(reinterpret_cast<const uint64_t*>(frontier) + i);
        int32_t v = (int32_t)(p >> 32);
        uint32_t tag = (uint32_t)(p & 0xFFFFFFFFULL);
        int32_t start_e = __ldg(csr_offsets + v);
        int32_t end_e = __ldg(csr_offsets + v + 1);
...
k_hop_nbrs_result_t run_simple_k1(
    Cache& cache,
    const int32_t* csr_offsets, const int32_t* csr_indices,
    ...
    cudaStream_t stream)
{
    ...
    cache.ensure_scan_temp(get_scan_temp_size(num_start));
...
k_hop_nbrs_result_t run_sort_unique(
    Cache& cache,
    const int32_t* csr_offsets, const int32_t* csr_indices,
    ...
    cudaStream_t stream)
{
    ...
    for (int64_t hop = 0; hop < k; hop++) {
        ...
        cache.ensure_scan_temp(get_scan_temp_size(frontier_size));
        ...
        cache.ensure_sort_temp(get_sort_temp_size(total_expanded));
        ...
        cache.ensure_unique_temp(get_unique_temp_size(total_expanded));
    ...
    cache.ensure_sort_temp(get_sort_temp_size(frontier_size));
    ...
    int64_t* d_offsets_out = nullptr;
    cudaMalloc(&d_offsets_out, (num_start + 1) * sizeof(int64_t));
    launch_extract_offsets(d_sorted_final, frontier_size, num_start, d_offsets_out, stream);
...
k_hop_nbrs_result_t run_bitmap(
    Cache& cache,
    const int32_t* csr_offsets, const int32_t* csr_indices,
    const int32_t* start_vertices, int64_t num_start, int64_t k,
    ...
    cudaStream_t stream)
{
    ...
    for (int64_t hop = 1; hop < k; hop++) {
        ...
        cudaMemcpyAsync(&last_cnt, d_counts + num_start - 1,
                         sizeof(int64_t), cudaMemcpyDeviceToHost, stream);
        cudaStreamSynchronize(stream);
        int64_t frontier_size = last_off + last_cnt;
    ...
    if (d_frontier) { cudaFree(d_frontier); d_frontier = nullptr; }
...
k_hop_nbrs_result_t k_hop_nbrs_seg(const graph32_t& graph,
    ...
    if (k_val == 1 && !is_multigraph) {
        ...
        return run_simple_k1(cache, csr_offsets, csr_indices,
    ...
    if (k_val > 1 && bitmap_bytes <= 2LL * 1024 * 1024 * 1024) {
        ...
        return run_bitmap(cache, csr_offsets, csr_indices,
                          start_vertices, num_start, k_val, num_vertices, stream);
    }

    
    return run_sort_unique(cache, csr_offsets, csr_indices,
