...
struct Cache : Cacheable {
    ...
    int32_t valid_capacity = 0;
    int32_t in_next_capacity = 0;
    int32_t rem_a_capacity = 0;
    int32_t rem_b_capacity = 0;
    int32_t front_a_capacity = 0;
    int32_t front_b_capacity = 0;
    bool counters_allocated = false;

    void ensure(int32_t n) {
        if (valid_capacity < n) {
            if (valid) cudaFree(valid);
            cudaMalloc(&valid, (size_t)n * sizeof(int32_t));
            valid_capacity = n;
        }
        if (in_next_capacity < n) {
            if (in_next) cudaFree(in_next);
            cudaMalloc(&in_next, (size_t)n * sizeof(int32_t));
            in_next_capacity = n;
        }
        if (rem_a_capacity < n) {
            if (rem_a) cudaFree(rem_a);
            cudaMalloc(&rem_a, (size_t)n * sizeof(int32_t));
            rem_a_capacity = n;
        }
        if (rem_b_capacity < n) {
            if (rem_b) cudaFree(rem_b);
            cudaMalloc(&rem_b, (size_t)n * sizeof(int32_t));
            rem_b_capacity = n;
        }
        if (front_a_capacity < n) {
            if (front_a) cudaFree(front_a);
            cudaMalloc(&front_a, (size_t)n * sizeof(int32_t));
            front_a_capacity = n;
        }
        if (front_b_capacity < n) {
            if (front_b) cudaFree(front_b);
            cudaMalloc(&front_b, (size_t)n * sizeof(int32_t));
            front_b_capacity = n;
        }
...
void core_number(const graph32_t& graph,
    ...
    cudaMemsetAsync(cache.in_next, 0, (size_t)n * sizeof(int32_t), 0);

    init_kernel<<<(n + 255) / 256, 256, 0, 0>>>(
        offsets, indices, core_numbers, cache.valid,
        degree_mul, k_first_i32, n);

    
    int32_t h_zero = 0;
    cudaMemcpyAsync(cache.counters, &h_zero, sizeof(int32_t), cudaMemcpyHostToDevice, 0);
    build_remaining_kernel<<<(n + 255) / 256, 256, 0, 0>>>(
        cache.valid, cache.rem_a, cache.counters, n);

    int32_t h_rem_count;
    cudaMemcpyAsync(&h_rem_count, cache.counters, sizeof(int32_t), cudaMemcpyDeviceToHost, 0);
    cudaStreamSynchronize(0);

    if (h_rem_count == 0) return;

    int32_t k_start = (k_first >= 2) ? k_first_i32 : 2;
    if (is_inout && (k_start % 2 == 1)) k_start++;
    int32_t k_last_val = (k_last > (size_t)INT32_MAX) ? INT32_MAX : (int32_t)k_last;
    ...
    void* args[] = {
        ...
        (void*)&core_numbers, (void*)&cache.valid, (void*)&cache.in_next,
        (void*)&cache.rem_a, (void*)&cache.rem_b,
        (void*)&cache.front_a, (void*)&cache.front_b,
        (void*)&cache.counters,
        (void*)&h_rem_count,
