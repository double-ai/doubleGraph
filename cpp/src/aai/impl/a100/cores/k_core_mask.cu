...
struct Cache : Cacheable {
    ...
    int64_t removed_mask_capacity = 0;
    int64_t degrees_capacity = 0;
    int64_t frontier_capacity = 0;
    int64_t next_frontier_capacity = 0;
    ...
    void ensure_removed_mask(int64_t n) {
        if (removed_mask_capacity < n) {
            if (d_removed_mask) cudaFree(d_removed_mask);
            cudaMalloc(&d_removed_mask, n * sizeof(uint32_t));
            removed_mask_capacity = n;
        }
    }
    ...
    void ensure_degrees(int64_t n) {
        if (degrees_capacity < n) {
            if (d_degrees) cudaFree(d_degrees);
            cudaMalloc(&d_degrees, n * sizeof(int32_t));
            degrees_capacity = n;
        }
    }

    void ensure_frontier(int64_t n) {
        if (frontier_capacity < n) {
            if (d_frontier) cudaFree(d_frontier);
            cudaMalloc(&d_frontier, n * sizeof(int32_t));
            frontier_capacity = n;
        }
    }

    void ensure_next_frontier(int64_t n) {
        if (next_frontier_capacity < n) {
            if (d_next_frontier) cudaFree(d_next_frontier);
            cudaMalloc(&d_next_frontier, n * sizeof(int32_t));
            next_frontier_capacity = n;
        }
    }
};
...
__device__ __forceinline__ bool is_removed(const uint32_t* removed_mask, int32_t v) {
...
std::size_t k_core_mask(const graph32_t& graph,
    ...
    if (core_numbers != nullptr) {
        ...
        cache.ensure_frontier(num_vertices);
        cache.ensure_next_frontier(num_vertices);
        ...
        if (degree_type == 0) {
            ...
            if (grid > 0)
                build_frontier_bitmask_kernel<<<grid, block>>>(
                    d_degrees, d_removed_mask, d_frontier, cache.d_frontier_size,
        ...
        while (h_frontier_size > 0) {
            ...
            int peel_grid = (h_frontier_size + block - 1) / block;
            if (peel_grid > 0)
                peel_frontier_kernel<<<peel_grid, block>>>(
                    d_offsets, d_indices, d_edge_mask, d_degrees, d_removed_mask,
                    d_frontier, h_frontier_size, d_next_frontier, cache.d_next_frontier_size,
                    threshold, delta);
...

    {
        ...
        if (grid > 0)
            extract_edges_atomic_kernel<<<grid, block>>>(
                d_offsets, d_indices, d_edge_mask, d_removed_mask, cache.d_counter,
                edge_srcs, edge_dsts, num_vertices);
    }
