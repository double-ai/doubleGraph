...
void launch_build_lookup(
    const int32_t* offsets, int32_t* lookup,
    int32_t num_vertices, int32_t num_edges
) {
    int num_chunks = (num_edges + CHUNK_SIZE - 1) / CHUNK_SIZE;
    int block = 256;
    int grid = (num_chunks + block - 1) / block;
    build_lookup_kernel<<<grid, block>>>(offsets, lookup, num_vertices, num_chunks);
}

void launch_init_and_build_worklist(
    const int32_t* offsets, const int32_t* indices,
    const int32_t* lookup,
    int32_t* degree, uint8_t* alive,
    int32_t* worklist, int32_t* wl_size,
    int32_t num_vertices, int32_t num_edges, int32_t multiplier, int32_t k
) {
    int block = 256;
    ...
    init_degrees_from_offsets_kernel<<<grid_v, block>>>(
        offsets, degree, alive, num_vertices, multiplier);
    fix_self_loops_lookup_kernel<<<grid_e, block>>>(
        offsets, indices, lookup, degree, num_vertices, num_edges, multiplier);
    build_worklist_kernel<<<grid_v, block>>>(
        degree, alive, worklist, wl_size, num_vertices, k);
}

void launch_mark_core_numbers(
    const int32_t* core_numbers, uint8_t* alive,
    int32_t num_vertices, int32_t k
) {
    int block = 256;
    int grid = (num_vertices + block - 1) / block;
    mark_core_numbers_kernel<<<grid, block>>>(
        core_numbers, alive, num_vertices, k);
}

void launch_peel(
    const int32_t* offsets, const int32_t* indices,
    int32_t* degree, uint8_t* alive,
    const int32_t* curr_wl, int32_t curr_wl_size,
    int32_t* next_wl, int32_t* next_wl_size,
    int32_t k, int32_t decrement
) {
    if (curr_wl_size == 0) return;
    int block = 256;
    int grid = (curr_wl_size + block - 1) / block;
    if (grid > 4096) grid = 4096;
    peel_kernel<<<grid, block>>>(
        offsets, indices, degree, alive,
        curr_wl, curr_wl_size, next_wl, next_wl_size,
        k, decrement);
}
...
void launch_extract_edges(
    ...
    extract_edges_lookup_kernel<<<grid, block>>>(
        offsets, indices, lookup, alive,
        edge_srcs, edge_dsts, num_output_edges,
...
std::size_t k_core(const graph32_t& graph,
    ...
    size_t need_lookup = (static_cast<size_t>(num_edges) + 31) / 32;
    cache.ensure_capacity(need_vertices, need_lookup);

    
    launch_build_lookup(d_offsets, cache.d_lookup, num_vertices, num_edges);

    if (core_numbers != nullptr) {
        launch_mark_core_numbers(core_numbers, cache.d_alive, num_vertices, k32);
    } else {
        int32_t multiplier = (degree_type == 2) ? 2 : 1;
        int32_t decrement = (degree_type == 2) ? 2 : 1;

        cudaMemset(cache.d_counter, 0, sizeof(int32_t));
        launch_init_and_build_worklist(
            d_offsets, d_indices, cache.d_lookup,
            cache.d_degree, cache.d_alive,
            cache.d_worklist1, cache.d_counter,
            num_vertices, num_edges, multiplier, k32);
    ...
    cudaMemset(cache.d_counter, 0, sizeof(int32_t));
    launch_extract_edges(
        d_offsets, d_indices, cache.d_lookup, cache.d_alive,
        edge_srcs, edge_dsts, cache.d_counter,
        num_vertices, num_edges);
