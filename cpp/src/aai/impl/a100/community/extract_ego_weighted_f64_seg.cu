...
#define BFS_BLOCK 256
#define WORK_BLOCK 512
...
__global__ void __launch_bounds__(512, 2) count_ego_edges_kernel(
    const int32_t* __restrict__ csr_offsets,
    const int32_t* __restrict__ csr_indices,
    const uint32_t* __restrict__ bitmaps,
...
__global__ void __launch_bounds__(512, 2) extract_ego_edges_kernel(
    const int32_t* __restrict__ csr_offsets,
    const int32_t* __restrict__ csr_indices,
...
extract_ego_weighted_result_double_t extract_ego_weighted_f64_seg(
    ...
    int32_t *fa, *fb, *d_sa, *d_sb;
    cudaMalloc(&fa, (int64_t)ns * max_frontier * sizeof(int32_t));
    cudaMalloc(&fb, (int64_t)ns * max_frontier * sizeof(int32_t));
    cudaMalloc(&d_sa, (int64_t)ns * sizeof(int32_t));
    cudaMalloc(&d_sb, (int64_t)ns * sizeof(int32_t));

    bfs_init_kernel<<<ns, BFS_BLOCK>>>(d_sources, d_bm, fa, d_sa, ns, bw, max_frontier);

    int32_t *cf = fa, *cs = d_sa;
    int32_t *nf = fb, *ns_buf = d_sb;

    for (int h = 0; h < radius; h++) {
        bfs_expand_kernel<<<ns, BFS_BLOCK>>>(d_csr_off, d_csr_idx, d_bm, cf, cs, nf, ns_buf,
                        ns, bw, max_frontier);
        std::swap(cf, nf);
        std::swap(cs, ns_buf);
    }
    ...
    popcount_bitmap_kernel<<<(int)((total_words + WORK_BLOCK - 1) / WORK_BLOCK), WORK_BLOCK>>>(
        d_bm, pc, bw, ns, num_vertices);

    int32_t* pc_scan;
    cudaMalloc(&pc_scan, total_words * sizeof(int32_t));
    {
        size_t scan_temp = 0;
        cub::DeviceScan::ExclusiveSum(nullptr, scan_temp, pc, pc_scan, total_words);
        cache.ensure_temp(scan_temp);
        cub::DeviceScan::ExclusiveSum(cache.d_temp, cache.temp_size, pc, pc_scan, total_words);
    }

    
    int64_t* ego_vo;
    cudaMalloc(&ego_vo, (ns + 1) * sizeof(int64_t));
    {
        int threads = WORK_BLOCK;
    ...
    {
        int total = ns * bw;
        extract_ego_vertices_kernel<<<(total + WORK_BLOCK - 1) / WORK_BLOCK, WORK_BLOCK>>>(
            d_bm, pc_scan, ego_v, bw, ns, num_vertices);
    }
    ...
    if (total_ego_verts > 0) {
        count_ego_edges_kernel<<<(int)((total_ego_verts + WORK_BLOCK - 1) / WORK_BLOCK), WORK_BLOCK>>>(
    ...
    if (total_ego_verts > 0) {
        int32_to_int64_kernel<<<(int)((total_ego_verts + WORK_BLOCK - 1) / WORK_BLOCK), WORK_BLOCK>>>(
            ec, ec64, total_ego_verts);
    }
    ...
    if (total_ego_verts > 0) {
        size_t scan_temp = 0;
        cub::DeviceScan::ExclusiveSum(nullptr, scan_temp, ec64, escan, total_ego_verts);
        cache.ensure_temp(scan_temp);
        cub::DeviceScan::ExclusiveSum(cache.d_temp, cache.temp_size, ec64, escan, total_ego_verts);
    }

    
    int64_t* eeo;
    cudaMalloc(&eeo, (ns + 1) * sizeof(int64_t));
    {
        int threads = WORK_BLOCK;
        int blocks = (ns + 1 + threads - 1) / threads;
    ...
    if (total_edges > 0) {
        ...
        if (total_ego_verts > 0) {
            extract_ego_edges_kernel<<<(int)((total_ego_verts + WORK_BLOCK - 1) / WORK_BLOCK), WORK_BLOCK>>>(
                d_csr_off, d_csr_idx, d_csr_wt, d_bm, bw, ego_v, ego_vo,
                escan, eeo, ns, out_s, out_d, out_w, total_ego_verts);

            sort_weight_ties_kernel<<<(int)((total_edges + WORK_BLOCK - 1) / WORK_BLOCK), WORK_BLOCK>>>(
