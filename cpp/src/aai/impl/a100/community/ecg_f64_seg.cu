...
struct Cache : Cacheable {
    ...
    void ensure(int32_t nv, int32_t ne) {
        if (!fixed_allocated) {
            ...
            cudaMalloc(&d_moved, sizeof(int32_t));
        ...
        if (nv_cap < nv) {
            ...
            cudaMalloc(&d_vw, nv * sizeof(double));
            cudaMalloc(&d_cw, nv * sizeof(double));
            cudaMalloc(&d_cl, nv * sizeof(int32_t));
            cudaMalloc(&d_best, nv * sizeof(int32_t));
            cudaMalloc(&d_cp, nv * sizeof(int32_t));
            cudaMalloc(&d_tmp, nv * sizeof(int32_t));
            cudaMalloc(&d_rk, nv * sizeof(uint32_t));
            nv_cap = nv;
        }
        if (ne_cap < ne) {
            ...
            cudaMalloc(&d_ecg, ne * sizeof(double));
            cudaMalloc(&d_mw, ne * sizeof(double));
            cudaMalloc(&d_ek, ne * sizeof(uint64_t));
            cudaMalloc(&d_ew2, ne * sizeof(double));
            cudaMalloc(&d_rkeys, ne * sizeof(uint64_t));
            cudaMalloc(&d_rw, ne * sizeof(double));
            cudaMalloc(&d_ns, ne * sizeof(int32_t));
            cudaMalloc(&d_nd, ne * sizeof(int32_t));
            ne_cap = ne;
        }
    }
...
ecg_result_double_t ecg_seg(const graph32_t& graph,
    ...
    int64_t ens = (int64_t)ensemble_size;
    int64_t ml = (int64_t)max_level;
    double thr = threshold;
    double res = resolution;
    double min_w = min_weight;

    cudaMemset(cache.d_ecg, 0, ne * sizeof(double));
    double tw = get_total_weight(edge_weights, ne);
    k_vertex_weights<<<BLK(nv), BS>>>(off, edge_weights, cache.d_vw, nv);

    
    for (int64_t i = 0; i < ens; i++) {
        gen_perm(cache.d_cl, cache.d_rk, nv, (uint32_t)(i * 12347 + 42));
        run_local_move(off, idx, edge_weights, cache.d_cl, cache.d_vw, cache.d_cw, tw, res, thr, nv,
                       cache.d_best, cache.d_buf, cache.d_moved, 100, true, 1, cache.d_tmp);
        k_ecg_counts<<<BLK(nv), BS>>>(off, idx, cache.d_cl, cache.d_ecg, nv);
    }

    
    k_ecg_weights<<<BLK(ne), BS>>>(edge_weights, cache.d_ecg, cache.d_mw, min_w, 1.0/(double)ens, ne);

    
    int64_t h_lv; double h_mod;
    run_louvain(off, idx, cache.d_mw, nv, ne, ml, thr, res,
                true, (uint32_t)(ens * 54321 + 99), clusters, &h_lv, &h_mod,
    ...
    double fm = compute_mod(off, idx, edge_weights, clusters, cache.d_cw, tw, res, nv, cache.d_buf);
