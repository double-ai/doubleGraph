...
__global__ __launch_bounds__(256, 2) void bfs_expand_kernel(
...
__global__ __launch_bounds__(256, 2) void count_ego_edges_compact_kernel(
    const int32_t* __restrict__ csr_offsets,
    const int32_t* __restrict__ csr_indices,
    const uint32_t* __restrict__ visited,
...
__global__ __launch_bounds__(256, 2) void extract_ego_edges_compact_kernel(
    const int32_t* __restrict__ csr_offsets,
    const int32_t* __restrict__ csr_indices,
...
void launch_bfs_init(const int32_t* s, uint32_t* v, uint32_t* f, int32_t ns, int32_t bw, cudaStream_t st) {
    if (ns == 0) return;
    bfs_init_kernel<<<(ns+255)/256, 256, 0, st>>>(s, v, f, ns, bw);
}

void launch_bfs_expand(const int32_t* co, const int32_t* ci, uint32_t* v, const uint32_t* f, uint32_t* nf,
                       int32_t nv, int32_t ns, int32_t bw, cudaStream_t st) {
    if (ns == 0 || bw == 0) return;
    dim3 grid((bw + 7) / 8, ns);
    bfs_expand_kernel<<<grid, 256, 0, st>>>(co, ci, v, f, nf, nv, ns, bw);
}

void launch_find_nonzero(const uint32_t* visited, int32_t ns, int32_t bw,
                         int32_t* nz_sids, int32_t* nz_wids, int32_t* nz_count, cudaStream_t st) {
    if (ns == 0 || bw == 0) return;
    dim3 grid((bw + 255) / 256, ns);
    find_nonzero_words_kernel<<<grid, 256, 0, st>>>(visited, ns, bw, nz_sids, nz_wids, nz_count);
}

void launch_count_ego_compact(const int32_t* co, const int32_t* ci, const uint32_t* visited,
                               int32_t nv, int32_t bw, const int32_t* nz_sids, const int32_t* nz_wids,
                               int32_t total_nz, int64_t* ec, cudaStream_t st) {
    if (total_nz == 0) return;
    int threads = total_nz * 32;
    int block = 256;
    int grid = (threads + block - 1) / block;
    count_ego_edges_compact_kernel<<<grid, block, 0, st>>>(
        co, ci, visited, nv, bw, nz_sids, nz_wids, total_nz, ec);
}

void launch_extract_ego_compact(const int32_t* co, const int32_t* ci, const float* ew,
                                 const uint32_t* visited, int32_t nv, int32_t bw,
                                 const int32_t* nz_sids, const int32_t* nz_wids, int32_t total_nz,
                                 uint64_t* ok, float* ow, const int64_t* eo,
                                 unsigned long long* wp, int32_t bpv, cudaStream_t st) {
    if (total_nz == 0) return;
    int threads = total_nz * 32;
    int block = 256;
    int grid = (threads + block - 1) / block;
    extract_ego_edges_compact_kernel<<<grid, block, 0, st>>>(
        co, ci, ew, visited, nv, bw, nz_sids, nz_wids, total_nz,
        ok, ow, eo, wp, bpv);
}

void launch_unpack_keys(const uint64_t* pk, int32_t* s, int32_t* d, int64_t n, int32_t bpv, cudaStream_t st) {
    if (n == 0) return;
    unpack_sort_keys_kernel<<<(int)((n+255)/256), 256, 0, st>>>(pk, s, d, n, bpv);
}
...
extract_ego_weighted_result_float_t extract_ego_weighted_f32_seg(
    ...
    launch_bfs_init(d_sources, d_visited, d_frontier, ns, bitmap_words, stream);

    for (int hop = 0; hop < radius; hop++) {
        if (bitmap_total > 0)
            cudaMemsetAsync(d_next_frontier, 0, bitmap_total * sizeof(uint32_t), stream);
        launch_bfs_expand(d_offsets, d_indices, d_visited, d_frontier, d_next_frontier,
                         num_vertices, ns, bitmap_words, stream);
    ...
    launch_find_nonzero(d_visited, ns, bitmap_words, d_nz_sids, d_nz_wids, d_nz_count, stream);
    ...
    launch_count_ego_compact(d_offsets, d_indices, d_visited,
                             num_vertices, bitmap_words,
                             d_nz_sids, d_nz_wids, h_total_nz, d_edge_counts, stream);
    ...
    if (total_edges > 0) {
        ...
        cudaMalloc(&out_wt, total_edges * sizeof(float));
    }
    cudaMalloc(&d_out_offsets, (ns + 1) * sizeof(int64_t));
    cudaMemcpyAsync(d_out_offsets, h_offsets.data(),
    ...
    if (total_edges > 0) {
        ...
        launch_extract_ego_compact(d_offsets, d_indices, d_weights, d_visited,
            num_vertices, bitmap_words,
            d_nz_sids, d_nz_wids, h_total_nz,
            d_keys1, out_wt,
            reinterpret_cast<const int64_t*>(d_out_offsets),
            d_write_pos, bpv, stream);
        ...
        if (is_multigraph) {
            ...
            size_t t1 = 0, t2 = 0;
            cub::DeviceRadixSort::SortPairs(nullptr, t1, out_wt, d_fbuf,
                d_keys1, d_keys2, num_items, 0, 32, stream);
            cub::DeviceRadixSort::SortPairs(nullptr, t2, d_keys2, d_keys1, d_fbuf,
                out_wt, num_items, 0, end_bit, stream);

            size_t needed = (t1 > t2) ? t1 : t2;
            cache.ensure_temp(needed);

            cub::DeviceRadixSort::SortPairs(cache.d_temp, t1, out_wt, d_fbuf,
                d_keys1, d_keys2, num_items, 0, 32, stream);
            cub::DeviceRadixSort::SortPairs(cache.d_temp, t2, d_keys2, d_keys1, d_fbuf,
                out_wt, num_items, 0, end_bit, stream);

            launch_unpack_keys(d_keys1, out_srcs, out_dsts, total_edges, bpv, stream);

            cudaFree(d_fbuf);
        } else {
            size_t t1 = 0;
            cub::DeviceRadixSort::SortPairs(nullptr, t1, d_keys1, d_keys2,
                out_wt, d_wbuf, num_items, 0, end_bit, stream);

            cache.ensure_temp(t1);

            cub::DeviceRadixSort::SortPairs(cache.d_temp, t1, d_keys1, d_keys2,
                out_wt, d_wbuf, num_items, 0, end_bit, stream);

            cudaMemcpyAsync(out_wt, d_wbuf,
                           total_edges * sizeof(float), cudaMemcpyDeviceToDevice, stream);
            launch_unpack_keys(d_keys2, out_srcs, out_dsts, total_edges, bpv, stream);
        }
