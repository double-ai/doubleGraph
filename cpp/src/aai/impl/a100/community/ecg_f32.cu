...
static inline int gSz(int n,int bs=256){int g=(n+bs-1)/bs;return g<1?1:(g>1024?1024:g);}
...
static float comp_mod_h(const int*off,const int*idx,const float*wt,
    ...
    int g=gSz(nv);if(g<64)g=64;if(g>256)g=256;
...
static ecg_result_float_t ecg_run_impl(
    const int*d_off,const int*d_idx,const float*d_ew,
    int nv,int ne,float minw,int ens_sz,int max_lv,float thr,float res,
    int*d_out_cl,
    void*scratch,size_t scratch_size)
{
    cudaStream_t s=0;BumpAlloc pool;pool.init(scratch,scratch_size);
    ...
    cudaStreamSynchronize(s);pool.reset();
...
ecg_result_float_t ecg(const graph32_t& graph,
    ...
    return ecg_run_impl(
        ...
        nv, ne, min_weight, (int)ensemble_size, (int)max_level, threshold, resolution,
        clusters,
