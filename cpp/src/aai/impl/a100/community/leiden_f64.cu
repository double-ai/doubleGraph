...
__global__ __launch_bounds__(128, 8) void local_moving_kernel(
...
__global__ __launch_bounds__(128, 8) void leiden_refine_kernel(
    const int* __restrict__ offsets, const int* __restrict__ indices,
    const double* __restrict__ edge_weights,
...
__global__ __launch_bounds__(128, 8) void cc_hook_kernel(
    const int* __restrict__ offsets, const int* __restrict__ indices,
...
static void launch_iota(int* a, int n) { iota_kernel<<<(n+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(a, n); }
static void launch_zero_double(double* a, int n) { zero_double_kernel<<<(n+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(a, n); }
static void launch_reduce_sum(const double* in, double* out, int n) { if(n<=0)return; int b = (n+BLOCK_SIZE-1)/BLOCK_SIZE; if(b>256)b=256; reduce_sum_kernel<<<b,BLOCK_SIZE>>>(in,out,n); }
static void launch_compute_vw(const int* off, const double* w, double* vw, int n) { compute_vertex_weights_kernel<<<(n+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(off, w, vw, n); }

static void launch_local_moving(const int* off, const int* idx, const double* w,
...
static void launch_leiden_refine(const int* off, const int* idx, const double* w,
...
static void launch_cc_compress(int* par, int n) { cc_compress_kernel<<<(n+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(par, n); }
static void launch_apply_cc(int* dst, const int* par, int n) { apply_cc_labels_kernel<<<(n+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(dst, par, n); }
static void launch_recompute_cw(const int* comm, const double* vw, double* cw, int n) {
    recompute_cw_kernel<<<(n+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(comm, vw, cw, n);
}

static void launch_compute_internal_weight(const int* off, const int* idx, const double* w,
    const int* part, double* out, int n) {
    compute_internal_weight_kernel<<<(n+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(off, idx, w, part, out, n);
}

static void launch_square_array(const double* in, double* out, int n) {
...
static int launch_coarsen_graph(const int* offsets, const int* indices, const double* weights,
...
static void launch_compose(int* p, const int* cp, int n) { compose_partitions_kernel<<<(n+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(p, cp, n); }
...
struct Cache : Cacheable {
    ...
    double* tw_buf = nullptr;   bool tw_alloc = false;
    ...
    int32_t* flag = nullptr;    bool flag_alloc = false;
    ...
    void ensure_tw() {
        if (!tw_alloc) { cudaMalloc(&tw_buf, sizeof(double)); tw_alloc = true; }
    }
    ...
    void ensure_flag() {
        if (!flag_alloc) { cudaMalloc(&flag, sizeof(int32_t)); flag_alloc = true; }
    }
    ...
    void ensure_cnt(int64_t n) {
        if (cnt_cap < n) { if (cnt) cudaFree(cnt); cudaMalloc(&cnt, n * sizeof(int32_t)); cnt_cap = n; }
    }
    void ensure_cinit(int64_t n) {
        if (cinit_cap < n) { if (cinit) cudaFree(cinit); cudaMalloc(&cinit, n * sizeof(int32_t)); cinit_cap = n; }
    }
...
static void do_local_moving(const int* off, const int* idx, const double* w,
...
static void do_leiden_refine(const int* off, const int* idx, const double* w,
    const int* flat, int* refined, double* rw, const double* vw,
...
leiden_result_double_t leiden(const graph32_t& graph,
    ...
    for (size_t level = 0; level < max_level; level++) {
        ...
        int32_t* new_c_off = nullptr;
        int32_t* new_c_idx = nullptr;
        double* new_c_w = nullptr;
        cudaMalloc(&new_c_off, (K_ref + 1) * sizeof(int32_t));
        cudaMalloc(&new_c_idx, cur_E * sizeof(int32_t));
        cudaMalloc(&new_c_w, cur_E * sizeof(double));

        cache.ensure_cnt(K_ref + 1);
    ...
    
    {
        ...
        return {levels_completed, modularity};
    }
}
