...
struct Cache : Cacheable {
    ...
    int64_t rev_capacity = 0;
    int64_t is_fwd_capacity = 0;
    int64_t prefix_capacity = 0;
    int64_t alive_capacity = 0;
    int64_t fwd_pos_a_capacity = 0;
    int64_t fwd_pos_b_capacity = 0;
    bool removed_allocated = false;
    size_t cub_capacity = 0;

    void ensure(int64_t num_edges, size_t cub_bytes) {
        if (edge_src_capacity < num_edges) {
            if (edge_src) cudaFree(edge_src);
            cudaMalloc(&edge_src, num_edges * sizeof(int32_t));
            edge_src_capacity = num_edges;
        }
        if (rev_capacity < num_edges) {
            if (rev) cudaFree(rev);
            cudaMalloc(&rev, num_edges * sizeof(int32_t));
            rev_capacity = num_edges;
        }
        if (is_fwd_capacity < num_edges) {
            if (is_fwd) cudaFree(is_fwd);
            cudaMalloc(&is_fwd, num_edges * sizeof(int32_t));
            is_fwd_capacity = num_edges;
        }
        if (prefix_capacity < num_edges) {
            if (prefix) cudaFree(prefix);
            cudaMalloc(&prefix, num_edges * sizeof(int32_t));
            prefix_capacity = num_edges;
        }
        if (alive_capacity < num_edges) {
            if (alive) cudaFree(alive);
            cudaMalloc(&alive, num_edges * sizeof(uint8_t));
            alive_capacity = num_edges;
        }
        if (fwd_pos_a_capacity < num_edges) {
            if (fwd_pos_a) cudaFree(fwd_pos_a);
            cudaMalloc(&fwd_pos_a, num_edges * sizeof(int32_t));
            fwd_pos_a_capacity = num_edges;
        }
        if (fwd_pos_b_capacity < num_edges) {
            if (fwd_pos_b) cudaFree(fwd_pos_b);
            cudaMalloc(&fwd_pos_b, num_edges * sizeof(int32_t));
            fwd_pos_b_capacity = num_edges;
        }
        if (!removed_allocated) {
            cudaMalloc(&removed, sizeof(int32_t));
            removed_allocated = true;
        }
...
__global__ __launch_bounds__(128) void count_and_mark(
    const int* __restrict__ offsets,
    const int* __restrict__ indices,
    const int* __restrict__ edge_src,
...
k_truss_result_t k_truss_seg(const graph32_t& graph, int32_t k) {
    static int tag;
    ...
    size_t cub_bytes = 0;
    cub::DeviceScan::ExclusiveSum(nullptr, cub_bytes, (int*)nullptr, (int*)nullptr, num_edges);
    cub_bytes = std::max(cub_bytes, (size_t)256);

    
    cache.ensure(num_edges, cub_bytes);

    int* d_edge_src = cache.edge_src;
    int* d_rev = cache.rev;
    int* d_is_fwd = cache.is_fwd;
    int* d_prefix = cache.prefix;
    uint8_t* d_alive = cache.alive;
    int* d_fwd_pos_a = cache.fwd_pos_a;
    int* d_fwd_pos_b = cache.fwd_pos_b;
    int* d_removed = cache.removed;
    ...
    int* d_fwd_cur = d_fwd_pos_a;
    int* d_fwd_next = d_fwd_pos_b;

    
    int* d_flag = d_is_fwd;
    int* d_fwd_prefix = d_prefix;
    ...
    for (;;) {
        ...
        size_t cb = 0;
        cub::DeviceScan::ExclusiveSum(nullptr, cb, (int*)nullptr, (int*)nullptr, cur_fwd);
        if (cb > cache.cub_capacity) {
            cache.ensure(num_edges, cb);
            d_cub_temp = cache.cub_temp;
        }
        cub_bytes = cache.cub_capacity;
        cub::DeviceScan::ExclusiveSum(d_cub_temp, cub_bytes, d_flag, d_fwd_prefix, cur_fwd, stream);
    ...
    if (cur_fwd > 0) {
        ...
        int b = 256, g = (cur_fwd + b - 1) / b;
        if (g > 0) extract_edges<<<g, b, 0, stream>>>(d_fwd_cur, d_edge_src, d_indices,
            out_src, out_dst, cur_fwd);
    }

    cudaStreamSynchronize(stream);

    return {out_src, out_dst, (std::size_t)total_output};
}
