...
__global__ void __launch_bounds__(256, 4) bfs_expand_kernel(
...
__global__ void __launch_bounds__(256, 4) count_edges_kernel(
    const int32_t* __restrict__ offsets,
    const int32_t* __restrict__ indices,
...
__global__ void __launch_bounds__(256, 4) extract_edges_kernel(
    const int32_t* __restrict__ offsets,
    const int32_t* __restrict__ indices,
...
extract_ego_weighted_result_float_t extract_ego_weighted_f32(
    ...
    cudaMemcpy(result.offsets, h_offsets.data(),
    ...
    for (size_t s = 0; s < n_sources; s++) {
        ...
        if (edge_counts[s] > 1) {
            ...
            thrust::sort(thrust::cuda::par.on(0), z, z + edge_counts[s], EdgeCmp());
        }
