...
struct Cache : Cacheable {
    ...
    int* d_mst_count = nullptr;
    int alloc_verts = 0;

    void ensure_scratch(int nv) {
        if (nv > alloc_verts) {
            auto freeIfSet = [](auto*& p) { if (p) { cudaFree(p); p = nullptr; } };
            freeIfSet(d_parent); freeIfSet(d_comp_min_enc);
            freeIfSet(d_vert_min_enc); freeIfSet(d_vert_min_dst);
            freeIfSet(d_comp_min_src); freeIfSet(d_active_list);
        ...
        if (!d_mst_count) cudaMalloc(&d_mst_count, sizeof(int));
    }

    ~Cache() override {
        auto f = [](auto*& p) { if (p) { cudaFree(p); p = nullptr; } };
        f(d_parent); f(d_comp_min_enc); f(d_vert_min_enc);
        f(d_vert_min_dst); f(d_comp_min_src); f(d_active_list);
        f(d_num_active); f(d_mst_count);
        alloc_verts = 0;
    }
};
...
std::size_t minimum_spanning_tree(const graph32_t& graph,
    ...
    match_merge_output_kernel<<<(nv + 255) / 256, 256>>>(
        ...
        cache.d_comp_min_src, mst_srcs, mst_dsts, mst_weights, cache.d_mst_count, nv, me);
    ...
    int prev_count;
    cudaMemcpy(&prev_count, cache.d_mst_count, sizeof(int), cudaMemcpyDeviceToHost);

    for (int round = 1; round < 40 && prev_count > 0; round++) {
        {
            int64_t total = (int64_t)nv * 32;
            find_min_edge_kernel<<<(int)((total + 255) / 256), 256>>>(
                d_offsets, d_indices, edge_weights, cache.d_parent,
        ...
        if (round & 1) {
            int current_count;
            cudaMemcpy(&current_count, cache.d_mst_count, sizeof(int), cudaMemcpyDeviceToHost);
    ...
    int h_mst_count;
    cudaMemcpy(&h_mst_count, cache.d_mst_count, sizeof(int), cudaMemcpyDeviceToHost);
    return static_cast<std::size_t>(h_mst_count);
}
