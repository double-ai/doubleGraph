...
std::size_t minimum_spanning_tree(const graph32_t& graph,
...

    {
        ...
        init_parent_kernel<<<g, b, 0, stream>>>(cache.d_parent, num_vertices);
    }
    ...
    for (int iter = 0; iter < 40; iter++) {
...

        {
            ...
            find_min_edge_warp_kernel<<<num_blocks, threads_per_block, 0, stream>>>(
                d_offsets, d_indices, d_weights, cache.d_parent, cache.d_min_edge, num_vertices);
        }
...

        {
            ...
            collect_mst_kernel<<<g, b, 0, stream>>>(
                d_offsets, d_indices, d_weights, cache.d_parent, cache.d_min_edge, cache.d_was_root,
