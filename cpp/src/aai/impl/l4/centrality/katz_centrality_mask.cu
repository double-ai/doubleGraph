...
struct Cache : Cacheable {
    ...
    void ensure(int32_t num_vertices) {
        if (buf_capacity < num_vertices) {
            if (buf) cudaFree(buf);
            cudaMalloc(&buf, (size_t)num_vertices * sizeof(float));
...
katz_centrality_result_t katz_centrality_mask(const graph32_t& graph,
    ...
    if (!has_initial_guess) {
        if (max_iterations >= 1) {
            ...
            if (!betas && iter < max_iterations) {
                analytical_iter2<<<(num_vertices + BLOCK_V - 1) / BLOCK_V, BLOCK_V>>>(
                    d_offsets, d_edge_mask,
                    bufs[1 - cur], alpha * beta, beta, num_vertices);
                cur = 1 - cur;
                iter = 2;
