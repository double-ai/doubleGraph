...
struct Cache : Cacheable {
    ...
    void ensure(int num_vertices, int num_edges) {
        if (num_vertices <= vert_cap && num_edges <= edge_cap) return;
        free_all();
        if (num_vertices > vert_cap) vert_cap = num_vertices;
        if (num_edges > edge_cap) edge_cap = num_edges;

        cudaMalloc(&d_parent, vert_cap * sizeof(int));
        cudaMalloc(&d_min_edge, vert_cap * sizeof(unsigned long long));
        cudaMalloc(&d_src_array, edge_cap * sizeof(int));
        cudaMalloc(&d_mst_srcs_tmp, vert_cap * sizeof(int));
        cudaMalloc(&d_mst_dsts_tmp, vert_cap * sizeof(int));
        cudaMalloc(&d_mst_weights_tmp, vert_cap * sizeof(float));
    ...
    void free_all() {
        auto free_dev = [](auto*& p) { if (p) { cudaFree(p); p = nullptr; } };
        free_dev(d_parent);
        free_dev(d_min_edge);
        free_dev(d_src_array);
        free_dev(d_mst_srcs_tmp);
        free_dev(d_mst_dsts_tmp);
        free_dev(d_mst_weights_tmp);
        free_dev(d_mst_count);
        free_dev(d_changed_flag);
...
std::size_t minimum_spanning_tree(const graph32_t& graph,
    ...
    for (int iter = 0; iter < 40; iter++) {
...

        {
            ...
            find_min_edge_kernel<<<grid, block>>>(d_offsets, d_indices, d_weights, cache.d_parent, cache.d_min_edge, num_vertices);
        }
        ...
        cudaMemsetAsync(cache.d_changed_flag, 0, sizeof(int), stream);
        {
            ...
            merge_and_record_kernel<<<grid, block>>>(
                ...
                d_indices, cache.d_src_array, d_weights,
        ...
        cudaMemcpyAsync(cache.h_changed_pinned, cache.d_changed_flag, sizeof(int), cudaMemcpyDeviceToHost, stream);
        cudaStreamSynchronize(stream);
        if (!*cache.h_changed_pinned) break;
    }

    
    cudaMemcpyAsync(cache.h_mst_count_pinned, cache.d_mst_count, sizeof(long long), cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
    ...
    return 2 * static_cast<std::size_t>(h_mst_count);
}
