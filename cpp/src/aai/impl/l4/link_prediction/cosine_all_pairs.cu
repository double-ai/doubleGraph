...
struct Cache : Cacheable {
    ...
    void ensure_offsets(int64_t n) {
        if (offsets_capacity < n) {
            if (offsets_buf) cudaFree(offsets_buf);
            cudaMalloc(&offsets_buf, n * sizeof(int64_t));
            offsets_capacity = n;
        }
    }
...
similarity_result_float_t cosine_all_pairs_similarity(const graph32_t& graph,
    ...
    if (topk.has_value()) {
        ...
        if (use_gmem) {
            launch_compute_topk_gmem(
                d_offsets, d_indices, num_v, d_seeds, num_seeds,
                out_first, out_second, out_scores,
                (unsigned long long*)cache.counter, topk_val,
        ...
        cudaMemcpyAsync(f, out_first, result_count * sizeof(int32_t), cudaMemcpyDeviceToDevice, stream);
        cudaMemcpyAsync(s, out_second, result_count * sizeof(int32_t), cudaMemcpyDeviceToDevice, stream);
        cudaMemcpyAsync(sc, out_scores, result_count * sizeof(float), cudaMemcpyDeviceToDevice, stream);
    ...
    cache.ensure_offsets(num_seeds);
    cudaMemsetAsync(cache.counter, 0, sizeof(int64_t), stream);

    
    if (use_gmem) {
        launch_count_gmem(d_offsets, d_indices, num_v, d_seeds, num_seeds,
                         cache.offsets_buf,
                         (unsigned long long*)cache.counter,
                         cache.bitmaps, bitmap_words,
                         block_size, stream);
    } else {
        launch_count(d_offsets, d_indices, num_v, d_seeds, num_seeds,
                     cache.offsets_buf,
                     (unsigned long long*)cache.counter,
                     block_size, stream);
    }
    ...
    if (use_gmem) {
        launch_compute_gmem(d_offsets, d_indices, num_v, d_seeds, num_seeds,
                           cache.offsets_buf,
                           out_first, out_second,
                           cache.bitmaps, bitmap_words,
                           block_size, stream);
    } else {
        launch_compute(d_offsets, d_indices, num_v, d_seeds, num_seeds,
                       cache.offsets_buf,
                       out_first, out_second,
                       block_size, stream);
    }
