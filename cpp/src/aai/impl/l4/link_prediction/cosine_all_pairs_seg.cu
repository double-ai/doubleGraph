...
struct Cache : Cacheable {
    ...
    void ensure_temp_storage(size_t needed) {
        if (temp_storage_bytes < needed) {
            if (d_temp_storage) cudaFree(d_temp_storage);
...
void launch_count_pairs(const int32_t* off, const int32_t* idx,
    ...
    count_kernel<512><<<n, 512, sm, s>>>(off, idx, seeds, n, bw, counts);
}
...
void launch_output_pairs(const int32_t* off, const int32_t* idx,
    ...
    output_kernel<512><<<n, 512, sm, st>>>(off, idx, seeds, n, bw, offsets, f, s, sc, max_out);
}
...
void launch_single_pass_topk(const int32_t* off, const int32_t* idx,
    ...
    if (n <= 0) return;
    int sm = get_smem(bw);
    cudaFuncSetAttribute(topk_kernel<512>, cudaFuncAttributeMaxDynamicSharedMemorySize, sm);
    topk_kernel<512><<<n, 512, sm, st>>>(off, idx, seeds, n, bw, topk, ctr, f, s, sc);
}
...
void launch_count_pairs_gmem(const int32_t* off, const int32_t* idx,
    ...
    if (n <= 0) return;
    count_kernel_gmem<512><<<n, 512, 0, s>>>(off, idx, seeds, n, bw, counts, global_bitmaps);
}
...
void launch_output_pairs_gmem(const int32_t* off, const int32_t* idx,
    ...
    if (n <= 0) return;
    output_kernel_gmem<512><<<n, 512, 0, st>>>(off, idx, seeds, n, bw, offsets, f, s, sc, max_out, global_bitmaps);
}
...
void launch_single_pass_topk_gmem(const int32_t* off, const int32_t* idx,
    ...
    if (n <= 0) return;
    topk_kernel_gmem<512><<<n, 512, 0, st>>>(off, idx, seeds, n, bw, topk, ctr, f, s, sc, global_bitmaps);
}

}  

similarity_result_float_t cosine_all_pairs_similarity_seg(
    ...
    if (topk.has_value()) {
        ...
        cudaMemsetAsync(cache.d_counter, 0, sizeof(int64_t), stream);

        if (use_gmem) {
            launch_single_pass_topk_gmem(d_offsets, d_indices, d_seeds, num_seeds,
                bitmap_words, topk_val, (unsigned long long*)cache.d_counter,
