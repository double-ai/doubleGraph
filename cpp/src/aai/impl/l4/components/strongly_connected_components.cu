...
void tarjan_full(const int32_t* off, const int32_t* idx, int32_t n, int32_t* comp,
                 int32_t* disc, int32_t* low, int32_t* stk, int8_t* on_stack, Frame* call_stack) {
    ...
    for (int32_t s = 0; s < n; s++) {
        ...
        while (cs > 0) {
            Frame& f = call_stack[cs - 1]; int32_t v = f.v, end = off[v + 1]; bool pushed = false;
            while (f.ei < end) { int32_t w = idx[f.ei++];
                if (disc[w] < 0) { disc[w] = low[w] = timer++; stk[st++] = w; on_stack[w] = 1; call_stack[cs++] = {w, off[w]}; pushed = true; break; }
                else if (on_stack[w]) { if (disc[w] < low[v]) low[v] = disc[w]; }
            }
            if (!pushed) { if (low[v] == disc[v]) { int32_t w; do { w = stk[--st]; on_stack[w] = 0; comp[w] = v; } while (w != v); }
                cs--; if (cs > 0) { int32_t pa = call_stack[cs - 1].v; if (low[v] < low[pa]) low[pa] = low[v]; } }
        }
    }
}
...
void gpu_scc(Cache& c, const int32_t* doff, const int32_t* didx, int32_t nv, int32_t ne, int32_t* dc) {
    ...
    if (remaining > 0) {
        ...
        tarjan_partial(c.h_offsets, c.h_indices, nv, c.h_components,
                       c.disc, c.low, c.stk, c.on_stack, c.call_stack);
...
void cpu_scc(Cache& c, const int32_t* doff, const int32_t* didx, int32_t nv, int32_t ne, int32_t* dc) {
    ...
    tarjan_full(c.h_offsets, c.h_indices, nv, c.h_components,
                c.disc, c.low, c.stk, c.on_stack, c.call_stack);
    cudaMemcpy(dc, c.h_components, nv * sizeof(int32_t), cudaMemcpyHostToDevice);
}
