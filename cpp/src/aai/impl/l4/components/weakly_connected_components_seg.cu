...
void weakly_connected_components_seg(const graph32_t& graph,
    ...
    init_kernel<<<grid, BLOCK_SIZE>>>(d_parent, n);
    fused_sample_hook_kernel<<<grid, BLOCK_SIZE>>>(d_offsets, d_indices, d_parent, n);
    compress_kernel<<<grid, BLOCK_SIZE>>>(d_parent, n);
    full_hook_kernel<<<grid, BLOCK_SIZE>>>(d_offsets, d_indices, d_parent, n);
    compress_kernel<<<grid, BLOCK_SIZE>>>(d_parent, n);
}
