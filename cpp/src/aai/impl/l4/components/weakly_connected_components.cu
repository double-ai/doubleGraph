...
void weakly_connected_components(const graph32_t& graph,
    ...
    if (grid > 58 * 8) grid = 58 * 8;

    cudaStream_t stream = 0;

    
    wcc_init<<<grid, BLOCK, 0, stream>>>(components, num_vertices);

    
    wcc_hook_sample<<<grid, BLOCK, 0, stream>>>(offsets, indices, components, num_vertices, 2);

    
    wcc_compress<<<grid, BLOCK, 0, stream>>>(components, num_vertices);

    
    wcc_hook_full<<<grid, BLOCK, 0, stream>>>(offsets, indices, components, num_vertices, 0);
    ...
    *h_flag = 0;
    __sync_synchronize();
    wcc_shortcut_check<<<grid, BLOCK, 0, stream>>>(components, num_vertices, d_flag);
    ...
    for (int iter = 0; iter < 100; iter++) {
        for (int j = 0; j < 5; j++) {
            wcc_shortcut<<<grid, BLOCK, 0, stream>>>(components, num_vertices);
        }
        *h_flag = 0;
        __sync_synchronize();
        wcc_shortcut_check<<<grid, BLOCK, 0, stream>>>(components, num_vertices, d_flag);
