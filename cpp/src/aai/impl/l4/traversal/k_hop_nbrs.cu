...
static size_t get_inclusive_sum_temp_bytes(int64_t n) {
    size_t tb = 0;
    cub::DeviceScan::InclusiveSum(nullptr, tb, (const int64_t*)nullptr, (int64_t*)nullptr, n);
    return tb;
}

static size_t get_seg_sort_temp_bytes(int64_t ni, int64_t ns) {
    size_t tb = 0;
    cub::DeviceSegmentedSort::SortKeys(nullptr, tb,
        (const int32_t*)nullptr, (int32_t*)nullptr, ni, (int)ns,
...
static void do_expand(const int32_t* d_go, const int32_t* d_gi,
                      const int32_t* frontier, int64_t frontier_size,
                      const int64_t* seg_offsets, int64_t num_sv,
                      int32_t** out_frontier, int64_t* out_frontier_size,
                      int64_t** out_seg_offsets) {
    ...
    {
        size_t tb = get_inclusive_sum_temp_bytes(frontier_size);
        void* tmp;
    ...
    int32_t* nf = nullptr;
    if (total > 0) {
        cudaMalloc(&nf, total * sizeof(int32_t));
        expand_frontier_kernel<<<(int)((total + 255) / 256), 256>>>(
            d_go, d_gi, frontier, eo, nf, frontier_size, total);
    }

    cudaFree(eo);

    *out_frontier = nf;
    *out_frontier_size = total;
    *out_seg_offsets = new_seg;
}



static bool do_sort_dedup(const int32_t* frontier, int64_t frontier_size,
                          const int64_t* seg_offsets, int64_t num_sv,
                          int32_t** out_frontier, int64_t* out_frontier_size,
                          int64_t** out_seg_offsets) {
    ...
    {
        size_t tb = get_seg_sort_temp_bytes(frontier_size, num_sv);
        void* tmp;
        cudaMalloc(&tmp, std::max(tb, (size_t)1));
        cub::DeviceSegmentedSort::SortKeys(tmp, tb,
            frontier, sorted, frontier_size, (int)num_sv,
    ...
    *out_frontier = compacted;
    *out_frontier_size = total_unique;
    *out_seg_offsets = new_seg;
    return true;
}


static void do_bitmap_hop(const int32_t* d_go, const int32_t* d_gi,
                          const int32_t* frontier, int64_t frontier_size,
                          const int64_t* seg_offsets, int64_t num_sv,
    ...
    {
        size_t tb = get_inclusive_sum_temp_bytes(num_sv);
        void* tmp;
        cudaMalloc(&tmp, std::max(tb, (size_t)1));
...
k_hop_nbrs_result_t k_hop_nbrs(const graph32_t& graph,
    ...
    if (k_val == 1) {
        ...
        do_expand(d_go, d_gi, frontier, frontier_size, seg_offsets, num_sv,
                  &new_f, &new_fs, &new_so);
        if (frontier_owned) cudaFree(frontier);
        cudaFree(seg_offsets);
        frontier = new_f;
        frontier_size = new_fs;
        seg_offsets = new_so;
        frontier_owned = true;

        
        if (is_multigraph && frontier_size > 0) {
            if (do_sort_dedup(frontier, frontier_size, seg_offsets, num_sv,
                              &new_f, &new_fs, &new_so)) {
                cudaFree(frontier);
                cudaFree(seg_offsets);
                frontier = new_f;
