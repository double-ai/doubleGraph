...
static void launch_init_sssp(void* dist_pred, int32_t num_vertices, int32_t source) {
    init_sssp_kernel<<<(num_vertices + 255) / 256, 256>>>(
        static_cast<DistPred*>(dist_pred), num_vertices, source);
}

static void launch_relax(
    const int32_t* offsets, const int32_t* indices, const float* weights,
    const uint32_t* edge_mask, void* dist_pred,
    const int32_t* frontier, int32_t frontier_size,
    int32_t* next_frontier, int32_t* next_frontier_count,
    uint32_t* next_bitmap, float cutoff
) {
    ...
    if (frontier_size < 32) {
        ...
        relax_thread_kernel<<<(frontier_size + 255) / 256, 256>>>(
            ...
            static_cast<DistPred*>(dist_pred),
            frontier, frontier_size,
            next_frontier, next_frontier_count, next_bitmap, cutoff);
...
static void launch_extract_results(
    const void* dist_pred, float* distances, int32_t* predecessors, int32_t num_vertices
) {
    extract_results_kernel<<<(num_vertices + 255) / 256, 256>>>(
        static_cast<const DistPred*>(dist_pred), distances, predecessors, num_vertices);
}

static void launch_clear_frontier_bitmap(
    uint32_t* bitmap, const int32_t* frontier, int32_t frontier_size
) {
    if (frontier_size <= 0) return;
    clear_frontier_bitmap_kernel<<<(frontier_size + 255) / 256, 256>>>(
        bitmap, frontier, frontier_size);
}
...
struct Cache : Cacheable {
    ...
    int64_t dist_pred_cap = 0;
    int64_t frontier1_cap = 0;
    int64_t frontier2_cap = 0;
...
void sssp_mask(const graph32_t& graph,
    ...
    void* d_dist_pred = cache.dist_pred;
    ...
    launch_init_sssp(d_dist_pred, num_vertices, source);
    ...
    while (frontier_size > 0) {
        ...
        launch_relax(d_offsets, d_indices, d_weights, d_edge_mask, d_dist_pred,
        ...
        if (frontier_size > 0) {
            if (frontier_size < bitmap_words) {
                launch_clear_frontier_bitmap(d_bitmap, next_frontier, frontier_size);
            } else {
                cudaMemset(d_bitmap, 0, bitmap_words * sizeof(uint32_t));
            }
        }
    ...
    launch_extract_results(d_dist_pred, distances, predecessors, num_vertices);
}
