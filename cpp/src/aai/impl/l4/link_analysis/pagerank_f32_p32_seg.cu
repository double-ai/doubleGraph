...
PageRankResult personalized_pagerank_seg(const graph32_t& graph,
    ...
    for (size_t iter = 0; iter < max_iterations; iter++) {
        ...
        if (use_split) {
            ...
            if (pers_size > 0) {
                int g = ((int)pers_size + BLOCK - 1) / BLOCK;
                if (g > 32) g = 32;
                pers_scatter_adjust_kernel<<<g, BLOCK, 0, stream>>>(nxt, cur, d_pv, d_pn, (int32_t)pers_size,
    ...
    if (cur_idx != 0) {
        cudaMemcpy(pageranks, cur, (size_t)V * sizeof(float), cudaMemcpyDeviceToDevice);
    }
