...
struct Cache : Cacheable {
    ...
    int32_t* dangling_indices_buf = nullptr;
    ...
    void ensure_n(int64_t n) {
        if (n_capacity < n) {
            ...
            cudaMalloc(&out_weights_buf, n * sizeof(double));
            cudaMalloc(&dangling_indices_buf, n * sizeof(int32_t));
            n_capacity = n;
        }
    }

    void ensure_m(int64_t m) {
        if (m_capacity < m) {
            if (scaled_w_f32) cudaFree(scaled_w_f32);
            cudaMalloc(&scaled_w_f32, m * sizeof(float));
            m_capacity = m;
        }
    }

    void ensure_pers(int64_t ps) {
        if (pers_capacity < ps) {
            if (pers_norm_sparse) cudaFree(pers_norm_sparse);
            cudaMalloc(&pers_norm_sparse, ps * sizeof(double));
            pers_capacity = ps;
        }
    }
...
__global__ __launch_bounds__(256, 2) void gather_dangling_sum_kernel(
...
__global__ __launch_bounds__(256, 2) void spmv_f32val_kernel(
...
__global__ __launch_bounds__(256, 2) void scatter_pers_kernel(
    double* __restrict__ y,
...
PageRankResult personalized_pagerank(const graph32_t& graph,
    ...
    cache.ensure_n(n);
    cache.ensure_pers(pers_size);

    double* pr_cur = cache.pr_buf0;
    double* pr_new = cache.pr_buf1;
    double* out_weights = cache.out_weights_buf;
    double* pers_norm_sparse = cache.pers_norm_sparse;
    int32_t* dangling_indices = cache.dangling_indices_buf;
    int32_t* d_num_dangling = cache.d_num_dangling;
    double* d_diff = cache.d_diff;
    double* d_dang = cache.d_dang;
    double* d_pers_sum = cache.d_pers_sum;

    float* scaled_w_f32 = nullptr;
    if (m > 0) {
        cache.ensure_m(m);
        scaled_w_f32 = cache.scaled_w_f32;
    }
    ...
    if (has_precomputed) {
        ...
        cudaMemsetAsync(out_weights, 0, n * sizeof(double), stream);
        launch_compute_out_weights(d_indices, edge_weights, out_weights, m, stream);
    }

    
    if (m > 0) {
        launch_prescale_and_cast(edge_weights, d_indices, out_weights, scaled_w_f32, alpha, m, stream);
    }

    
    cudaMemsetAsync(d_num_dangling, 0, sizeof(int32_t), stream);
    launch_build_dangling_indices(out_weights, dangling_indices, d_num_dangling, n, stream);

    
    cudaMemsetAsync(d_pers_sum, 0, sizeof(double), stream);
    launch_pers_sum(personalization_values, d_pers_sum, pers_size, stream);

    cudaMemcpyAsync(&cache.h_scalars_pinned[0], d_pers_sum, sizeof(double), cudaMemcpyDeviceToHost, stream);
    cudaMemcpyAsync(&cache.h_scalars_pinned[1], d_num_dangling, sizeof(int32_t), cudaMemcpyDeviceToHost, stream);
    ...
    launch_build_pers_norm_sparse(personalization_values, pers_norm_sparse, inv_pers_sum, pers_size, stream);
    ...
    cudaMemsetAsync(d_dang, 0, sizeof(double), stream);
    if (h_num_dangling > 0) {
        launch_gather_dangling_sum(pr_cur, dangling_indices, d_dang, h_num_dangling, stream);
    }
    ...
    if (use_cusparse) {
        ...
        cusparseCreateCsr(&matA, n, n, m,
            (void*)d_offsets, (void*)d_indices, (void*)scaled_w_f32,
    ...
    for (size_t iter = 0; iter < max_iterations; ++iter) {
        ...
        if (use_cusparse) {
            ...
            launch_spmv_f32val(d_offsets, d_indices, scaled_w_f32, pr_cur, pr_new, n, stream);
        }

        
        launch_scatter_pers(pr_new, personalization_vertices, pers_norm_sparse, d_dang,
                          alpha, one_minus_alpha, pers_size, stream);

        
        cudaMemsetAsync(d_diff, 0, sizeof(double), stream);
        launch_diff(pr_new, pr_cur, d_diff, n, stream);

        
        cudaMemsetAsync(d_dang, 0, sizeof(double), stream);
        if (h_num_dangling > 0) {
            launch_gather_dangling_sum(pr_new, dangling_indices, d_dang, h_num_dangling, stream);
        }

        iterations = iter + 1;

        
        cudaMemcpyAsync(cache.h_scalars_pinned, d_diff, sizeof(double), cudaMemcpyDeviceToHost, stream);
        cudaStreamSynchronize(stream);
