...
ecg_result_float_t ecg(const graph32_t& graph,
    ...
    cudaStream_t s = 0;
    BumpAlloc pool;
    pool.init(cache.scratch, cache.scratch_size);
    ...
    thrust::sequence(thrust::cuda::par.on(s),clusters,clusters+nv);
    for(int l=0;l<nlev;l++)flatten_d<<<gSz(nv),256,0,s>>>(clusters,d_den[l],nv);

    
    compute_vw<<<gSz(nv),256,0,s>>>(d_off,d_ew,d_vw,nv);
    recomp_cw(clusters,d_vw,d_cw,nv,s);
    float fmod=comp_mod_h(d_off,d_idx,d_ew,clusters,d_cw,tew_orig,res,nv,d_pi,d_pd,d_out,d_ret,s);

    cudaStreamSynchronize(s);
    pool.reset();

    ecg_result_float_t result;
    result.level_count = static_cast<std::size_t>(nlev);
    result.modularity = fmod;
    return result;
}
