...
void spectral_modularity_maximization(const graph32_t& graph,
    ...
    compute_degrees_kernel<<<grid, blk>>>(d_offsets, edge_weights, d_deg, n);
    double two_m;
    cublasSetPointerMode(cache.cublas_h, CUBLAS_POINTER_MODE_HOST);
    cublasDasum(cache.cublas_h, n, d_deg, 1, &two_m);
    ...
    double nrm;
    cublasDnrm2(cache.cublas_h, n, d_Q, 1, &nrm);
    double inv_nrm = 1.0/nrm;
    cublasDscal(cache.cublas_h, n, &inv_nrm, d_Q, 1);
    ...
    for (int j = 0; j < max_lanczos; j++) {
        ...
        cublasDdot(cache.cublas_h, n, d_deg, 1, qj, 1, d_dot_kq);
        axpy_device_scalar_kernel<<<grid, blk>>>(d_w, d_deg, d_dot_kq, two_m, n);

        cublasDdot(cache.cublas_h, n, qj, 1, d_w, 1, d_alpha_arr + j);
        lanczos_update_kernel<<<grid, blk>>>(d_w, qj,
            j>0 ? d_Q+(size_t)(j-1)*n : nullptr,
            d_alpha_arr+j, j>0 ? d_beta_arr+(j-1) : nullptr, n, j>0);

        int cols = j+1;
        cublasDgemv(cache.cublas_h, CUBLAS_OP_T, n, cols, d_one, d_Q, n, d_w, 1, d_zero, d_h, 1);
        cublasDgemv(cache.cublas_h, CUBLAS_OP_N, n, cols, d_neg_one, d_Q, n, d_h, 1, d_one, d_w, 1);

        cublasDnrm2(cache.cublas_h, n, d_w, 1, d_beta_arr + j);
        m = j+1;
    ...
    int lwork;
    cusolverDnDsyevd_bufferSize(cache.cusolver_h, CUSOLVER_EIG_MODE_VECTOR, CUBLAS_FILL_MODE_LOWER,
        m, d_T, m, d_eigenvalues, &lwork);
    double* d_work = ba.alloc_doubles(lwork);
    int* d_info = ba.alloc_int(1);
    cusolverDnDsyevd(cache.cusolver_h, CUSOLVER_EIG_MODE_VECTOR, CUBLAS_FILL_MODE_LOWER,
    ...
    cublasDgemm(cache.cublas_h, CUBLAS_OP_N, CUBLAS_OP_N,
    ...
    for (int restart = 0; restart < 2; restart++) {
        ...
        double cost;
        cublasDasum(cache.cublas_h, n, d_point_costs, 1, &cost);
        if (cost < best_cost) {
            best_cost = cost;
