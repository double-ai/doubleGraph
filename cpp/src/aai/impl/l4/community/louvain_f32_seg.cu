...
static int launch_renumber_communities(int32_t* community, int* used, int* prefix, int N, Cache& cache) {
    int grid = (N + BLOCK_SIZE - 1) / BLOCK_SIZE;
...
louvain_result_float_t louvain_seg(const graph32_t& graph,
    ...
    launch_compute_vertex_weights(offsets, edge_weights, orig_vw, orig_N);
    ...
    int32_t* d_final = clusters;
    launch_init_communities(d_final, orig_N);
    ...
    if (orig_N <= 256) {
        ...
        float* all_sigma;
        float* all_mod;
        cudaMalloc(&all_comm, (int64_t)num_restarts * orig_N * sizeof(int32_t));
        cudaMalloc(&all_sigma, (int64_t)num_restarts * orig_N * sizeof(float));
        cudaMalloc(&all_mod, num_restarts * sizeof(float));

        launch_batch_sequential_louvain(
        ...
        int32_t* trial_comm;
        cudaMalloc(&trial_comm, orig_N * sizeof(int32_t));
        int* d_lc;
        ...
        for (int ki = 0; ki < topK; ki++) {
            ...
            float* cur_vw = nullptr;
            cudaMalloc(&cur_vw, orig_N * sizeof(float));
            cudaMemcpy(cur_vw, orig_vw, orig_N * sizeof(float), cudaMemcpyDeviceToDevice);
            ...
            for (int level = 0; level < (int)max_level; level++) {
                ...
                int new_N = launch_renumber_communities(ml_comm, used, pref, cur_N, cache);
                cudaFree(used);
                cudaFree(pref);
                ...
                int64_t* ek;
                float* ev;
                cudaMalloc(&ek, cur_E * sizeof(int64_t));
                cudaMalloc(&ev, cur_E * sizeof(float));
                launch_map_edges(cur_off, cur_idx2, ml_comm, ek, cur_N);
                ...
                float* uv;
                int* nu;
                cudaMalloc(&sk, cur_E * sizeof(int64_t));
                cudaMalloc(&sv, cur_E * sizeof(float));
                cudaMalloc(&uk, cur_E * sizeof(int64_t));
                cudaMalloc(&uv, cur_E * sizeof(float));
                ...
                int* cnt;
                cudaMalloc(&cnt, new_N * sizeof(int));
                launch_build_csr(uk, uv, new_E, new_N, cnt, c_off, c_idx, c_wt, cache);
                cudaFree(uk); cudaFree(uv); cudaFree(nu); cudaFree(cnt);

                cudaMalloc(&c_vw, new_N * sizeof(float));
                launch_compute_new_vw(cur_vw, ml_comm, c_vw, cur_N, new_N);
                ...
                float* ls;
                cudaMalloc(&ls, cur_N * sizeof(float));
                ...
                for (int iter = 0; iter < 200; iter++) {
                    ...
                    launch_sequential_local_move(cur_off, cur_idx2, cur_wt,
                        ml_comm, ls, cur_vw, cur_N, total_weight, resolution, d_lc, 0);
                    int h_changed = 0;
                ...
                float new_Q = launch_compute_modularity(offsets, indices,
                    edge_weights, trial_comm, orig_vw, orig_N, total_weight, resolution);
            ...
            if (trial_Q > overall_best_Q) {
                ...
                cudaMemcpy(d_final, trial_comm, orig_N * sizeof(int32_t), cudaMemcpyDeviceToDevice);
            }

            cudaFree(ml_comm);
            if (c_off) { cudaFree(c_off); cudaFree(c_idx); cudaFree(c_wt); }
            if (c_vw) cudaFree(c_vw);
            
            if (cur_vw != orig_vw && cur_vw != c_vw) cudaFree(cur_vw);
        }
    ...
    {
        int32_t* d_par;
        cudaMalloc(&d_par, orig_N * sizeof(int32_t));
        launch_init_communities(d_par, orig_N);
        ...
        for (int level = 0; level < (int)max_level; level++) {
            ...
            float* bm_gain;
            int32_t* best_saved;
            cudaMalloc(&d_comm, cur_N * sizeof(int32_t));
            cudaMalloc(&d_sigma, cur_N * sizeof(float));
            cudaMalloc(&bm_comm, cur_N * sizeof(int32_t));
            cudaMalloc(&bm_gain, cur_N * sizeof(float));
            cudaMalloc(&best_saved, cur_N * sizeof(int32_t));

            launch_init_communities(d_comm, cur_N);
            float new_Q = launch_compute_modularity(cur_off, cur_idx2, cur_wt, d_comm,
            ...
            while (new_Q > cur_Q + threshold) {
                cur_Q = new_Q;
                launch_recompute_sigma_tot(d_comm, cur_vw, d_sigma, cur_N);
                launch_compute_best_move(cur_off, cur_idx2, cur_wt, d_comm, d_sigma,
                    cur_vw, cur_N, total_weight, resolution, bm_comm, bm_gain);
                cudaMemset(d_count, 0, sizeof(int));
                launch_count_updown(d_comm, bm_comm, bm_gain, cur_N, up_down, d_count);
                int h_count;
                cudaMemcpy(&h_count, d_count, sizeof(int), cudaMemcpyDeviceToHost);
                bool eff = up_down; if (h_count == 0) eff = !up_down;
                launch_apply_updown(d_comm, bm_comm, bm_gain, cur_N, eff);
                up_down = !up_down;
                new_Q = launch_compute_modularity(cur_off, cur_idx2, cur_wt, d_comm,
                    cur_vw, cur_N, total_weight, resolution);
            ...
            if (cur_Q <= best_modularity) {
                cudaFree(d_comm); cudaFree(d_sigma); cudaFree(bm_comm); cudaFree(bm_gain); cudaFree(best_saved);
                break;
            }
            ...
            cudaMalloc(&used, cur_N * sizeof(int));
            cudaMalloc(&pref, cur_N * sizeof(int));
            int new_N = launch_renumber_communities(d_comm, used, pref, cur_N, cache);
            ...
            else launch_compose_communities(d_par, d_comm, orig_N);
            ...
            cudaMalloc(&ek, cur_E * sizeof(int64_t));
            cudaMalloc(&ev, cur_E * sizeof(float));
            launch_map_edges(cur_off, cur_idx2, d_comm, ek, cur_N);
            cudaMemcpy(ev, cur_wt, cur_E * sizeof(float), cudaMemcpyDeviceToDevice);
            ...
            cudaMalloc(&c_vw, new_N * sizeof(float));
            launch_compute_new_vw(cur_vw, d_comm, c_vw, cur_N, new_N);
        ...
        {
            float* ref_sigma;
            cudaMalloc(&ref_sigma, orig_N * sizeof(float));
            int* d_ref_changed;
            ...
            for (int iter = 0; iter < refine_max; iter++) {
                ...
                for (int c = 0; c < 2; c++) {
                    launch_recompute_sigma_tot(d_par, orig_vw, ref_sigma, orig_N);
                    launch_local_move(offsets, indices, edge_weights,
        ...
        float par_Q = launch_compute_modularity(offsets, indices,
            edge_weights, d_par, orig_vw, orig_N, total_weight, resolution);
    ...
    launch_renumber_communities(d_final, ru, rp, orig_N, cache);
    cudaFree(ru); cudaFree(rp);

    float modularity = launch_compute_modularity(
        offsets, indices, edge_weights, d_final, orig_vw,
        orig_N, total_weight, resolution);

    cudaFree(orig_vw);
    cudaFree(d_count);

    return louvain_result_float_t{1, modularity};
}
