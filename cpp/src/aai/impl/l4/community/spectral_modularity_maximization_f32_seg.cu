...
#define LAUNCH(kernel, n, stream, ...) do { \
    int blk_ = 128, grd_ = ((n) + blk_ - 1) / blk_; \
    kernel<<<grd_, blk_, 0, stream>>>(__VA_ARGS__); \
} while(0)
...
void spectral_modularity_maximization_seg(const graph32_t& graph,
    ...
    int32_t* d_clustering = clustering;

    cudaStream_t stream = cache.stream;
    cublasHandle_t cublas = cache.cublas;
    cusparseHandle_t cusparse = cache.cusparse;
    cusolverDnHandle_t cusolver = cache.cusolver;

    
    float* d_degrees;
    cudaMalloc(&d_degrees, n * sizeof(float));
    LAUNCH(compute_degrees_kernel, n, stream, d_offsets, d_weights, d_degrees, n);

    float* d_inv_2m;
    cudaMalloc(&d_inv_2m, sizeof(float));
    compute_inv_asum_kernel<<<1, 256, 0, stream>>>(d_degrees, d_inv_2m, n);
    ...
    cudaMemsetAsync(d_beta, 0, (lanczos_dim + 1) * sizeof(float), stream);
    ...
    if (needed > cache.spmv_buffer_size) {
        cudaFree(cache.spmv_buffer);
        cache.spmv_buffer_size = needed;
        cudaMalloc(&cache.spmv_buffer, cache.spmv_buffer_size);
    }

    
    LAUNCH(fill_random_kernel, n, stream, d_Q, n, 42ULL);
    cublasSetPointerMode(cublas, CUBLAS_POINTER_MODE_DEVICE);
    cublasSnrm2(cublas, n, d_Q, 1, d_dot);
    LAUNCH(scale_by_inv_device_kernel, n, stream, d_Q, d_dot, n);
    ...
    for (int j = 0; j < lanczos_dim; j++) {
        ...
        cublasSdot(cublas, n, d_degrees, 1, d_qj, 1, d_dot);
        LAUNCH(apply_degree_correction_dev2_kernel, n, stream, d_qj1, d_degrees, d_dot, d_inv_2m, n);

        
        int nq = j + 1;
        float* d_dots_j = d_dots_buf + (int64_t)j * lanczos_dim;
        cublasSgemv(cublas, CUBLAS_OP_T, n, nq, cache.d_one, d_Q, n, d_qj1, 1, cache.d_zero, d_dots_j, 1);
        cublasSgemv(cublas, CUBLAS_OP_N, n, nq, cache.d_neg_one, d_Q, n, d_dots_j, 1, cache.d_one, d_qj1, 1);

        
        cublasSnrm2(cublas, n, d_qj1, 1, &d_beta[j + 1]);

        
        LAUNCH(scale_by_inv_device_kernel, n, stream, d_qj1, &d_beta[j + 1], n);

        
        if (j + 1 >= min_steps && (j + 1) % 5 == 0) {
            float h_beta_j1;
            cudaMemcpyAsync(&h_beta_j1, &d_beta[j + 1], sizeof(float), cudaMemcpyDeviceToHost, stream);
    ...
    LAUNCH(extract_diagonal_kernel, m, stream, d_alpha, d_dots_buf, m, lanczos_dim);

    
    std::vector<float> h_alpha(m), h_beta(m + 1);
    cudaMemcpyAsync(h_alpha.data(), d_alpha, m * sizeof(float), cudaMemcpyDeviceToHost, stream);
    cudaMemcpyAsync(h_beta.data(), d_beta, (m + 1) * sizeof(float), cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
    ...
    cudaMemcpyAsync(d_T, T.data(), m * m * sizeof(float), cudaMemcpyHostToDevice, stream);

    int work_size = 0;
    cusolverDnSsyevd_bufferSize(cusolver, CUSOLVER_EIG_MODE_VECTOR,
                                CUBLAS_FILL_MODE_LOWER, m, d_T, m, d_eigenvalues, &work_size);
    float* d_work;
    cudaMalloc(&d_work, (size_t)work_size * sizeof(float));
    cusolverDnSsyevd(cusolver, CUSOLVER_EIG_MODE_VECTOR,
    ...
    cublasSgemm(cublas, CUBLAS_OP_N, CUBLAS_OP_N, n, num_eigenvectors, m,
    ...
    cublasSgeam(cublas, CUBLAS_OP_T, CUBLAS_OP_N, k_ev, n, &h_one, d_V, n, &h_zero, d_V, k_ev, d_emb, k_ev);
    LAUNCH(row_normalize_kernel, n, stream, d_emb, n, k_ev);
    ...
    cudaMemcpyAsync(d_centroids, d_emb, k_ev * sizeof(float), cudaMemcpyDeviceToDevice, stream);
    
    LAUNCH(fill_constant_kernel, n, stream, d_min_dists, 1e30f, n);
    LAUNCH(farthest_first_update_kernel, n, stream, d_emb, d_centroids, d_min_dists, n, k_ev, 0);

    for (int c = 1; c < num_clusters; c++) {
        farthest_first_select_kernel<<<1, 256, 0, stream>>>(d_emb, d_min_dists, d_centroids, n, k_ev, c);
        LAUNCH(farthest_first_update_kernel, n, stream, d_emb, d_centroids, d_min_dists, n, k_ev, c);
    }

    cudaMemsetAsync(d_clustering, 0xFF, n * sizeof(int32_t), stream);
    ...
    
    {
        ...
        void* args[] = {
            (void*)&d_emb, (void*)&d_centroids, (void*)&d_clustering,
            (void*)&d_new_centroids, (void*)&d_counts, (void*)&d_converged,
