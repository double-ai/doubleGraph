...
struct Cache : Cacheable {
    ...
    void ensure_base(int64_t nv, int64_t ne) {
        ...
        if (vb_cap < ne * 2) {
            if (vb) cudaFree(vb);
            cudaMalloc(&vb, ne * 2 * sizeof(double));
            vb_cap = ne * 2;
        }
    }
...
static void launch_zero_double(double* a, int n) {
    zero_double_kernel<<<(n+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(a, n);
}

static void launch_compare(const int* a, const int* b, int n, int* d) {
    compare_arrays_kernel<<<(n+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(a, b, n, d);
}

static void launch_reduce_sum(const double* in, double* out, int n) {
    if (n <= 0) return;
...
static void launch_compute_vw(const int* off, const double* w, double* vw, int n) {
...
leiden_result_double_t leiden(const graph32_t& graph,
    ...
    for (size_t level = 0; level < max_level; level++) {
        ...
        if (cur_V * 2 > cache.tmp_cap) {
            cudaFree(cache.tmp);
            cudaMalloc(&cache.tmp, (int64_t)cur_V * 2 * sizeof(int));
        ...
        int new_E = launch_coarsen_graph(cur_off, cur_idx, cur_w, d_ref,
            cur_V, cur_E, K_ref,
            c_off, c_idx, c_w, d_kb, d_vb, c_cnt);
        ...
        if (cur_V > cache.comm_cap) {
            cudaFree(cache.comm);
            cudaMalloc(&cache.comm, cur_V * sizeof(int));
    ...
    if (c_off) { cudaFree(c_off); c_off = nullptr; }
    if (c_idx) { cudaFree(c_idx); c_idx = nullptr; }
    if (c_w) { cudaFree(c_w); c_w = nullptr; }
    if (c_vw) { cudaFree(c_vw); c_vw = nullptr; }
    if (c_cw) { cudaFree(c_cw); c_cw = nullptr; }
    if (c_flat) { cudaFree(c_flat); c_flat = nullptr; }
    if (c_par) { cudaFree(c_par); c_par = nullptr; }
    if (c_ref) { cudaFree(c_ref); c_ref = nullptr; }
    if (c_rw) { cudaFree(c_rw); c_rw = nullptr; }
    if (c_init) { cudaFree(c_init); c_init = nullptr; }
    if (c_cnt) { cudaFree(c_cnt); c_cnt = nullptr; }
    ...
    if (nv > cache.par_cap) {
        cudaFree(cache.par);
        cudaMalloc(&cache.par, nv * sizeof(int));
        cache.par_cap = nv;
        d_par = cache.par;
    }
