...
struct Cache : Cacheable {
    ...
    size_t cluster_degrees_capacity = 0;
    ...
    void ensure(int32_t num_clusters, int32_t num_vertices) {
        if ((size_t)num_clusters > cluster_degrees_capacity) {
            if (d_cluster_degrees) cudaFree(d_cluster_degrees);
            cluster_degrees_capacity = (size_t)num_clusters * 2;
...
__global__ void compress_to_u8(const int32_t* __restrict__ src, uint8_t* __restrict__ dst, int n, int nc) {
...
__global__ void compress_to_i16(const int32_t* __restrict__ src, int16_t* __restrict__ dst, int n, int nc) {
    for (int i = blockIdx.x * BLOCK_SIZE + threadIdx.x; i < n; i += gridDim.x * BLOCK_SIZE) {
        int32_t c = src[i];
        if ((uint32_t)c >= (uint32_t)nc) c = (c < 0) ? 0 : (nc - 1);
        dst[i] = (int16_t)c;
...
static void launch_modularity(
    ...
    cudaMemsetAsync(cluster_degrees, 0, (size_t)num_clusters * sizeof(float), stream);
    cudaMemsetAsync(d_intra_sum, 0, sizeof(float), stream);

    if (num_clusters <= 255 && compressed_buf) {
        int grid = (num_vertices + BLOCK_SIZE - 1) / BLOCK_SIZE;
        if (grid > 1024) grid = 1024;
        compress_to_u8<<<grid, BLOCK_SIZE, 0, stream>>>(
            cluster_assignments, (uint8_t*)compressed_buf, num_vertices, num_clusters);
        launch_main_kernels<uint8_t>(offsets, indices, edge_weights,
            (const uint8_t*)compressed_buf,
            cluster_degrees, d_intra_sum, num_vertices, num_edges, num_clusters, stream);
    } else if (num_clusters <= 32767 && compressed_buf) {
        int grid = (num_vertices + BLOCK_SIZE - 1) / BLOCK_SIZE;
        if (grid > 1024) grid = 1024;
        compress_to_i16<<<grid, BLOCK_SIZE, 0, stream>>>(
            cluster_assignments, (int16_t*)compressed_buf, num_vertices, num_clusters);
        launch_main_kernels<int16_t>(offsets, indices, edge_weights,
            (const int16_t*)compressed_buf,
            cluster_degrees, d_intra_sum, num_vertices, num_edges, num_clusters, stream);
    } else {
        launch_main_kernels<int32_t>(offsets, indices, edge_weights,
            cluster_assignments,
            cluster_degrees, d_intra_sum, num_vertices, num_edges, num_clusters, stream);
    }

    finalize_kernel_parallel<<<1, BLOCK_SIZE, 0, stream>>>(
        cluster_degrees, d_intra_sum, d_result, num_clusters);
}
