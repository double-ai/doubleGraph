...
bool local_moving_iter(Cache& ws,
...
void louvain_impl(Cache& ws,
    ...
    for (size_t lv = 0; lv < ml; lv++) {
        ...
        while (nQ > cQ + thr && mi-- > 0) {
            cQ = nQ;
            bool moved = local_moving_iter(ws, cop, cip, cwtp, cp, vwp, cwp,
                cn, ce, tew, res, ud, shift, end_bit);
            if (!moved) break;
            thrust::fill(cw.begin(), cw.begin()+cn, 0.0);
            accum_cw_kernel<<<gn, B>>>(cp, vwp, cwp, cn);
            cudaDeviceSynchronize();
        ...
        auto rk = ws.ctr_red_keys;
        auto rv = ws.ctr_red_vals;
...
ecg_result_double_t ecg(const graph32_t& graph,
    ...
    for (std::size_t i = 0; i < ensemble_size; i++) {
        ...
        int dl; double dm;
        louvain_impl(cache, d_off, d_idx, edge_weights, nv, ne,
            thrust::raw_pointer_cast(tc.data()), 1, threshold, resolution, true, seed, &dl, &dm);
    ...
    int lv; double mv;
    louvain_impl(cache, d_off, d_idx, thrust::raw_pointer_cast(mw.data()),
        nv, ne, clusters, max_level, threshold, resolution, true, 0x12345678ULL, &lv, &mv);
    ...
    compute_vw_kernel<<<(nv+255)/256, 256>>>(d_off, edge_weights,
    ...
    return ecg_result_double_t{static_cast<std::size_t>(lv), mv};
}
