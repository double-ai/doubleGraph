...
louvain_result_double_t louvain(const graph32_t& graph,
    ...
    cudaMalloc(&edge_weights_f, orig_E * sizeof(float));
    launch_convert_double_to_float(edge_weights, edge_weights_f, orig_E);

    
    float* orig_vw;
    cudaMalloc(&orig_vw, orig_N * sizeof(float));
    launch_compute_vertex_weights(graph.offsets, edge_weights_f, orig_vw, orig_N);
    ...
    if (orig_N <= 256) {
        ...
        launch_batch_sequential_louvain(graph.offsets, graph.indices,
            edge_weights_f, orig_vw, orig_N, total_weight, resolution_f,
            all_comm, all_sigma, all_mod, num_restarts);
        ...
        for (int ki = 0; ki < topK; ki++) {
            ...
            for (int level = 0; level < (int)max_level; level++) {
                int32_t* used;
                int32_t* pref;
                cudaMalloc(&used, cur_N * sizeof(int32_t));
                cudaMalloc(&pref, cur_N * sizeof(int32_t));
                int new_N = launch_renumber_communities(ml_comm, (int*)used, (int*)pref, cur_N, cache);
                cudaFree(used);
                cudaFree(pref);
                ...
                launch_map_edges(cur_off, cur_idx, ml_comm, ek, cur_N);
                ...
                cudaMalloc(&c_off2, (new_N + 1) * sizeof(int32_t));
                cudaMalloc(&c_idx2, new_E * sizeof(int32_t));
                cudaMalloc(&c_wt2, new_E * sizeof(float));
                int32_t* cnt;
                cudaMalloc(&cnt, new_N * sizeof(int32_t));
                launch_build_csr(uk, uv, new_E, new_N, (int*)cnt, c_off2, c_idx2, c_wt2, cache);
                cudaFree(cnt);

                cudaMalloc(&c_vw2, new_N * sizeof(float));
                launch_compute_new_vw(cur_vw, ml_comm, c_vw2, cur_N, new_N);
                ...
                launch_compose_communities(trial_comm, ml_comm, orig_N);
                float new_Q = launch_compute_modularity(graph.offsets, graph.indices,
                    edge_weights_f, trial_comm, orig_vw, orig_N, total_weight, resolution_f);
                if (new_Q <= level_best) break;
                level_best = new_Q;
            }

            float trial_Q = launch_compute_modularity(graph.offsets, graph.indices,
                edge_weights_f, trial_comm, orig_vw, orig_N, total_weight, resolution_f);
    ...
    {
        int32_t* par_comm;
        cudaMalloc(&par_comm, orig_N * sizeof(int32_t));
        ...
        for (int level = 0; level < (int)max_level; level++) {
            ...
            float* bm_gain;
            int32_t* best_saved;
            cudaMalloc(&comm, cur_N * sizeof(int32_t));
            cudaMalloc(&sigma, cur_N * sizeof(float));
            ...
            float new_Q = launch_compute_modularity(cur_off, cur_idx, cur_wt, comm,
            ...
            while (new_Q > cur_Q + threshold_f) {
                ...
                launch_compute_best_move(cur_off, cur_idx, cur_wt, comm, sigma,
                    cur_vw, cur_N, total_weight, resolution_f,
                    bm_comm, bm_gain);
                ...
                up_down = !up_down;
                new_Q = launch_compute_modularity(cur_off, cur_idx, cur_wt, comm,
                    cur_vw, cur_N, total_weight, resolution_f);
            ...
            if (!done) {
                ...
                cudaMalloc(&used, cur_N * sizeof(int32_t));
                cudaMalloc(&pref, cur_N * sizeof(int32_t));
                new_N = launch_renumber_communities(comm, (int*)used, (int*)pref, cur_N, cache);
                cudaFree(used);
                cudaFree(pref);
            ...
            if (!done) {
                ...
                cudaMalloc(&ek, cur_E * sizeof(int64_t));
                cudaMalloc(&ev, cur_E * sizeof(float));
                launch_map_edges(cur_off, cur_idx, comm, ek, cur_N);
                cudaMemcpy(ev, cur_wt, cur_E * sizeof(float), cudaMemcpyDeviceToDevice);
                ...
                int32_t* cnt;
                cudaMalloc(&cnt, new_N * sizeof(int32_t));
                launch_build_csr(uk, uv, new_E, new_N, (int*)cnt, c_off, c_idx, c_wt, cache);
                cudaFree(cnt);

                cudaMalloc(&c_vw, new_N * sizeof(float));
                launch_compute_new_vw(cur_vw, comm, c_vw, cur_N, new_N);
        ...
        
        {
            ...
            for (int iter = 0; iter < refine_max; iter++) {
                ...
                for (int c = 0; c < 2; c++) {
                    ...
                    launch_local_move(graph.offsets, graph.indices, edge_weights_f,
        ...
        float par_Q = launch_compute_modularity(graph.offsets, graph.indices,
    ...
    float modularity = launch_compute_modularity(graph.offsets, graph.indices,
