...
struct Cache : Cacheable {
    float* dvw = nullptr;
    int32_t* d_ca = nullptr;
    int32_t* d_cb = nullptr;
    float* dcw = nullptr;
    int32_t* dt1 = nullptr;
    int32_t* dt2 = nullptr;
    uint32_t* drk = nullptr;
    float* dco = nullptr;
    float* dmw = nullptr;
    double* d_mod = nullptr;
    ...
    void ensure(int32_t nv, int32_t ne) {
        if (nv_cap < nv) {
            ...
            if (dt2) cudaFree(dt2);
            if (drk) cudaFree(drk);
            cudaMalloc(&dvw, (size_t)nv * sizeof(float));
            cudaMalloc(&d_ca, (size_t)nv * sizeof(int32_t));
            cudaMalloc(&d_cb, (size_t)nv * sizeof(int32_t));
            cudaMalloc(&dcw, (size_t)nv * sizeof(float));
            cudaMalloc(&dt1, (size_t)nv * sizeof(int32_t));
            cudaMalloc(&dt2, (size_t)nv * sizeof(int32_t));
            cudaMalloc(&drk, (size_t)nv * sizeof(uint32_t));
            nv_cap = nv;
        }
        if (ne_cap < ne) {
            if (dco) cudaFree(dco);
            if (dmw) cudaFree(dmw);
            cudaMalloc(&dco, (size_t)ne * sizeof(float));
            cudaMalloc(&dmw, (size_t)ne * sizeof(float));
            ne_cap = ne;
        }
        if (!d_mod) {
            cudaMalloc(&d_mod, 2 * sizeof(double));
        }
    }
...
__global__ void k_affine_perm(int32_t* __restrict__ out, int32_t n, uint32_t a, uint32_t b) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        uint64_t t = (uint64_t)a * (uint64_t)(uint32_t)i + (uint64_t)b;
        out[i] = (int32_t)(t % (uint32_t)n);
    }
}

static inline int32_t gcd_i32(int32_t a, int32_t b) {
    while (b != 0) {
        int32_t t = a % b;
        a = b;
        b = t;
...
__global__ void k_fused_cw_si(const int32_t* __restrict__ off, const int32_t* __restrict__ ind,
                             ...
                             const float* __restrict__ vw, float* __restrict__ cw,
...
__global__ void k_move(const int32_t* __restrict__ off, const int32_t* __restrict__ ind,
                      const float* __restrict__ wt, const int32_t* __restrict__ c,
    ...
    for (int o = 16; o > 0; o >>= 1) {
        ...
        if (od > bd || (od == bd && oc < bc)) {
            bd = od;
            bc = oc;
        }
    }
    if (lane == 0) {
        if (bd > 0.0f && bc != mc && ((bc > mc) == ud))
            nc[v] = bc;
        else
            nc[v] = mc;
    }
...
__global__ void k_edge_keys32(const int32_t* __restrict__ off, const int32_t* __restrict__ ind,
                             const int32_t* __restrict__ c, int32_t* __restrict__ ek,
                             int32_t n, int32_t K) {
    int v = blockIdx.x * blockDim.x + threadIdx.x;
    if (v >= n) return;
    int32_t sv = c[v];
...
__global__ void k_edge_keys64(const int32_t* __restrict__ off, const int32_t* __restrict__ ind,
                             const int32_t* __restrict__ c, int64_t* __restrict__ ek,
                             int32_t n, int32_t K) {
    int v = blockIdx.x * blockDim.x + threadIdx.x;
    if (v >= n) return;
    int64_t sv = (int64_t)c[v];
    for (int32_t e = off[v]; e < off[v + 1]; e++) {
        int64_t dv = (int64_t)c[ind[e]];
        ek[e] = sv * (int64_t)K + dv;
    }
}



static float compute_mod(const int32_t* doff, const int32_t* dind, const float* dwt,
                         const int32_t* dc, const float* dvw, float* dcw, float tw, float res,
                         int32_t n, int32_t mc, double* d_mod) {
...
static float compute_mod_with_cw(const int32_t* doff, const int32_t* dind, const float* dwt,
                                 const int32_t* dc, const float* dcw, float tw, float res,
                                 int32_t n, int32_t mc, double* d_mod) {
    cudaMemset(d_mod, 0, 2 * sizeof(double));
...
static void gen_perm(int32_t* p, int32_t n, uint32_t s, uint32_t* ) {
    uint32_t a = (s * 1664525u + 1013904223u) | 1u;
    a %= (uint32_t)n;
    if (a == 0) a = 1;
    while (gcd_i32((int32_t)a, n) != 1) {
        a += 2u;
        if (a >= (uint32_t)n) a -= (uint32_t)n;
        if (a == 0) a = 1;
    }
    uint32_t b = (s * 22695477u + 1u) % (uint32_t)n;
    k_affine_perm<<<(n + BT - 1) / BT, BT>>>(p, n, a, b);
}

static void louvain_move(const int32_t* doff, const int32_t* dind, const float* dwt,
                         int32_t*& d_ca, int32_t*& d_cb, float* dcw, const float* dvw, float tw,
                         float res, float thr, int32_t n, int32_t mc, double* d_mod, int max_iter) {
    if (n == 0) return;
    int blocks = (int)(((int64_t)n * 32 + BT - 1) / BT);
...
static CG coarsen(const int32_t* doff, const int32_t* dind, const float* dwt, const int32_t* dc,
    ...
    if (use32) {
        ...
        auto ep = thrust::reduce_by_key(ekp, ekp + m, evp, thrust::device_ptr<int32_t>(uk),
                                        thrust::device_ptr<float>(uvt));
        ...
        cudaMalloc(&dst_arr, (size_t)nu * sizeof(int32_t));
        {
            ...
            int32_t Kv = K;
            thrust::transform(ukp, ukp + nu, thrust::make_zip_iterator(sp, dp),
                             [Kv] __device__(int32_t k) { return thrust::make_tuple(k / Kv, k % Kv); });
        }
        cudaFree(uk);
        uv = uvt;

        int32_t* noff;
        cudaMalloc(&noff, (K + 1) * sizeof(int32_t));
        thrust::lower_bound(thrust::device, thrust::device_ptr<int32_t>(src),
                            thrust::device_ptr<int32_t>(src) + nu, thrust::counting_iterator<int32_t>(0),
                            thrust::counting_iterator<int32_t>(K + 1), thrust::device_ptr<int32_t>(noff));
        cudaFree(src);
        ...
        cudaMalloc(&uvt, (size_t)m * sizeof(float));
        auto ep = thrust::reduce_by_key(ekp, ekp + m, evp, thrust::device_ptr<int64_t>(uk),
                                        thrust::device_ptr<float>(uvt));
...
static LR full_louvain(const int32_t* do0, const int32_t* di0, const float* dw0, int32_t* dout,
                       int32_t nv, int32_t ne, int ml, float thr, float res, float tw, bool rng,
                       uint32_t seed, int32_t* d_ca, int32_t* d_cb, float* dvw, float* dcw,
                       int32_t* dt1, int32_t* dt2, uint32_t* drk, double* d_mod) {
    int32_t cn = nv, cm = ne;
    const int32_t* co = do0;
    const int32_t* ci = di0;
    const float* cwt = dw0;
    std::vector<int32_t*> ao, ai;
    std::vector<float*> aw;
    std::vector<int32_t*> lc;
    std::vector<int32_t> ls;
    float bm = -1.0f;
    int level = 0;
    int32_t* orig_ca = d_ca;
    int32_t* orig_cb = d_cb;
    ...
    while (level < ml) {
        ...
        if (rng)
            gen_perm(d_ca, cn, seed + level * 13337, drk);
        else {
            thrust::device_ptr<int32_t> cp(d_ca);
            thrust::sequence(cp, cp + cn);
        }
        ...
        int32_t* lcomm;
        cudaMalloc(&lcomm, (size_t)cn * sizeof(int32_t));
...
ecg_result_float_t ecg_seg(const graph32_t& graph,
    ...
    LR lr = full_louvain(doff, dind, dmw, dout, nv, ne, ml, thr, res, tmw, true, 9999, d_ca,
                         d_cb, dvw, dcw, dt1, dt2, drk, d_mod);
