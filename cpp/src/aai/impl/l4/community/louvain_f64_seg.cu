...
static void launch_recompute_sigma_tot(const int32_t* community, const float* vw, float* sigma_tot, int N) {
...
static void launch_compute_best_move(
    const int32_t* offsets, const int32_t* indices, const float* weights,
...
static void launch_count_updown(
...
static void launch_apply_updown(
...
static void launch_batch_sequential_louvain(
    const int32_t* offsets, const int32_t* indices, const float* weights,
...
static int launch_renumber_communities(Cache& cache, int32_t* community, int* used, int* prefix, int N) {
    int grid = (N + BLOCK_SIZE - 1) / BLOCK_SIZE;
...
static void launch_compute_new_vw(const float* old_vw, const int32_t* community, float* new_vw, int old_N, int new_N) {
...
static void launch_compose_communities(int32_t* final_comm, const int32_t* level_comm, int orig_N) {
    compose_communities_kernel<<<(orig_N + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(final_comm, level_comm, orig_N);
}

static float launch_compute_modularity(
    const int32_t* offsets, const int32_t* indices, const float* weights,
...
static void launch_convert_double_to_float(const double* in, float* out, int N) {
    if (N <= 0) return;
...
static void coarsen_graph(
    ...
    launch_map_edges(cur_off, cur_idx, community, ek, cur_N);
    ...
    int* d_nu; cudaMalloc(&d_nu, sizeof(int));

    int new_E = launch_sort_reduce_edges(cache, ek, ev, cur_E, sk, sv, uk, uv, d_nu);

    cudaFree(ek); cudaFree(ev); cudaFree(sk); cudaFree(sv); cudaFree(d_nu);
    ...
    launch_build_csr(cache, uk, uv, new_E, new_N, cnt, c_off, c_idx, c_wt);
    cudaFree(uk); cudaFree(uv); cudaFree(cnt);

    float* c_vw; cudaMalloc(&c_vw, (size_t)new_N * sizeof(float));
    launch_compute_new_vw(cur_vw, community, c_vw, cur_N, new_N);
...
louvain_result_double_t louvain_seg(const graph32_t& graph,
    ...
    launch_reduce_sum(orig_vw, d_tw, orig_N);
    float total_weight;
    ...
    int* used; cudaMalloc(&used, (size_t)orig_N * sizeof(int));
    int* prefix; cudaMalloc(&prefix, (size_t)orig_N * sizeof(int));
    ...
    if (orig_N <= 256) {
        ...
        launch_batch_sequential_louvain(offsets, indices, edge_weights_f, orig_vw,
            orig_N, total_weight, resolution_f, all_comm, all_sigma, all_mod, num_restarts);
        ...
        for (int ki = 0; ki < topK; ki++) {
            ...
            for (int level = 0; level < (int)max_level; level++) {
                int new_N = launch_renumber_communities(cache, ml_comm, used, prefix, cur_N);
                ...
                int new_E;
                coarsen_graph(cache, cur_off, cur_idx2, cur_wt, cur_vw, ml_comm,
                              cur_N, cur_E, new_N, &c_off, &c_idx, &c_wt, &c_vw, &new_E);
                ...
                for (int iter = 0; iter < 200; iter++) {
                    ...
                    launch_sequential_local_move(cur_off, cur_idx2, cur_wt,
                ...
                launch_compose_communities(trial_comm, ml_comm, orig_N);
                float new_Q = launch_compute_modularity(offsets, indices, edge_weights_f,
            ...
            float trial_Q = launch_compute_modularity(offsets, indices, edge_weights_f,
                trial_comm, orig_vw, orig_N, total_weight, resolution_f);
    ...
    
    {
        ...
        for (int level = 0; level < (int)max_level; level++) {
            ...
            launch_init_communities(comm, cur_N);
            float new_Q = launch_compute_modularity(cur_off, cur_idx2, cur_wt, comm,
            ...
            while (new_Q > cur_Q + threshold_f) {
                cur_Q = new_Q;
                launch_recompute_sigma_tot(comm, cur_vw, sigma, cur_N);
                launch_compute_best_move(cur_off, cur_idx2, cur_wt, comm, sigma,
                    cur_vw, cur_N, total_weight, resolution_f, bm_comm, bm_gain);
                cudaMemset(d_count, 0, sizeof(int));
                launch_count_updown(comm, bm_comm, bm_gain, cur_N, up_down, d_count);
                int h_count;
                cudaMemcpy(&h_count, d_count, sizeof(int), cudaMemcpyDeviceToHost);
                bool eff = up_down; if (h_count == 0) eff = !up_down;
                launch_apply_updown(comm, bm_comm, bm_gain, cur_N, eff);
                up_down = !up_down;
                new_Q = launch_compute_modularity(cur_off, cur_idx2, cur_wt, comm,
                    cur_vw, cur_N, total_weight, resolution_f);
            ...
            cudaFree(sigma); sigma = nullptr;
            cudaFree(bm_comm); bm_comm = nullptr;
            cudaFree(bm_gain); bm_gain = nullptr;
            cudaFree(best_saved); best_saved = nullptr;

            if (cur_Q <= best_modularity) {
                cudaFree(comm); comm = nullptr;
                break;
            }
            best_modularity = cur_Q;

            int new_N = launch_renumber_communities(cache, comm, used, prefix, cur_N);
            if (new_N >= cur_N) {
                cudaFree(comm); comm = nullptr;
                break;
            }

            if (level == 0) cudaMemcpy(par_comm, comm, orig_N * sizeof(int32_t), cudaMemcpyDeviceToDevice);
            else launch_compose_communities(par_comm, comm, orig_N);
            ...
            int new_E;
            coarsen_graph(cache, cur_off, cur_idx2, cur_wt, cur_vw, comm,
                          cur_N, cur_E, new_N, &c_off, &c_idx, &c_wt, &c_vw, &new_E);
        ...
        float par_Q = launch_compute_modularity(offsets, indices, edge_weights_f,
    ...
    launch_renumber_communities(cache, clusters, used, prefix, orig_N);

    
    float modularity = launch_compute_modularity(offsets, indices, edge_weights_f,
