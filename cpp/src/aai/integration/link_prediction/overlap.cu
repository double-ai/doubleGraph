...
// =============================================================================
// Specialized overlap_coefficients to route to AAI
// =============================================================================

// Float specialization (handles both unweighted and float-weighted cases)
template <>
rmm::device_uvector<float> overlap_coefficients<int32_t, int32_t, float, false>(
  ...
  bool do_expensive_check)
{
  ...
  // Preconditions (from overlap_impl.cuh:39 + similarity_impl.cuh:45-48)
  CUGRAPH_EXPECTS(!graph_view.has_edge_mask(), "Edge masks are not supported");
  CUGRAPH_EXPECTS(!compact_graph.is_csc, "Overlap similarity requires CSR format (is_csc=false)");
  ...
  if (edge_weight_view.has_value()) {
    // Weighted Overlap with float weights
    const float* weights = edge_weight_view->value_firsts()[0];
    if (compact_graph.segment_offsets.has_value()) {
      aai::overlap_similarity_seg(compact_graph, weights, first_vertices, second_vertices,
                                  num_pairs, similarity_scores.data());
    } else {
      aai::overlap_similarity(compact_graph, weights, first_vertices, second_vertices,
                              num_pairs, similarity_scores.data());
    }
  } else {
    // Unweighted Overlap
    if (compact_graph.segment_offsets.has_value()) {
      aai::overlap_similarity_seg(compact_graph, first_vertices, second_vertices,
                                  num_pairs, similarity_scores.data());
    } else {
      aai::overlap_similarity(compact_graph, first_vertices, second_vertices,
                              num_pairs, similarity_scores.data());
    }
  }
...
// unweighted (calls float AAI kernel + widening copy) Overlap.
template <>
rmm::device_uvector<double> overlap_coefficients<int32_t, int32_t, double, false>(
  raft::handle_t const& handle,
  graph_view_t<int32_t, int32_t, false, false> const& graph_view,
  ...
  bool do_expensive_check)
{
  ...
  if (edge_weight_view.has_value()) {
    // Weighted Overlap with double weights
    ...
    // Unweighted: AAI only has float output, so compute in float and widen.
...
// =============================================================================
// Specialized overlap_all_pairs_coefficients to route to AAI
// =============================================================================

// Float specialization (handles both unweighted and float-weighted cases)
template <>
std::tuple<rmm::device_uvector<int32_t>, rmm::device_uvector<int32_t>, rmm::device_uvector<float>>
overlap_all_pairs_coefficients<int32_t, int32_t, float, false>(
  raft::handle_t const& handle,
  graph_view_t<int32_t, int32_t, false, false> const& graph_view,
  std::optional<edge_property_view_t<int32_t, float const*>> edge_weight_view,
  ...
  bool do_expensive_check)
{
  ...
  // Preconditions (from overlap_impl.cuh:61 + similarity_impl.cuh:213-219)
  CUGRAPH_EXPECTS(!graph_view.has_edge_mask(), "Edge masks are not supported");
  CUGRAPH_EXPECTS(!compact_graph.is_csc, "Overlap all-pairs similarity requires CSR format (is_csc=false)");
  CUGRAPH_EXPECTS(graph_view.is_symmetric(),
                  "similarity algorithms require an undirected(symmetric) graph");
  ...
  if (edge_weight_view.has_value()) {
    const float* weights = edge_weight_view->value_firsts()[0];
    if (compact_graph.segment_offsets.has_value()) {
      result = aai::overlap_all_pairs_similarity_seg(
          compact_graph, weights, vertices_ptr, num_vertices, topk);
    } else {
      result = aai::overlap_all_pairs_similarity(
          compact_graph, weights, vertices_ptr, num_vertices, topk);
    }
  } else {
    if (compact_graph.segment_offsets.has_value()) {
...
overlap_all_pairs_coefficients<int32_t, int32_t, double, false>(
  raft::handle_t const& handle,
  graph_view_t<int32_t, int32_t, false, false> const& graph_view,
  std::optional<edge_property_view_t<int32_t, double const*>> edge_weight_view,
  std::optional<raft::device_span<int32_t const>> vertices,
  ...
  bool do_expensive_check)
{
  ...
  if (edge_weight_view.has_value()) {
    // Weighted all-pairs Overlap with double weights
    const double* weights = edge_weight_view->value_firsts()[0];
    ...
    // Unweighted all-pairs Overlap: AAI only has float output, so compute in float and widen.
    aai::similarity_result_float_t result;
    if (compact_graph.segment_offsets.has_value()) {
      result = aai::overlap_all_pairs_similarity_seg(
          compact_graph, vertices_ptr, num_vertices, topk);
...
#else  // !AAI_ROUTE_OVERLAP

namespace cugraph {
template rmm::device_uvector<float> overlap_coefficients<int32_t, int32_t, float, false>(raft::handle_t const&, graph_view_t<int32_t, int32_t, false, false> const&, std::optional<edge_property_view_t<int32_t, float const*>>, std::tuple<raft::device_span<int32_t const>, raft::device_span<int32_t const>>, bool);
template rmm::device_uvector<double> overlap_coefficients<int32_t, int32_t, double, false>(raft::handle_t const&, graph_view_t<int32_t, int32_t, false, false> const&, std::optional<edge_property_view_t<int32_t, double const*>>, std::tuple<raft::device_span<int32_t const>, raft::device_span<int32_t const>>, bool);
template std::tuple<rmm::device_uvector<int32_t>, rmm::device_uvector<int32_t>, rmm::device_uvector<float>> overlap_all_pairs_coefficients<int32_t, int32_t, float, false>(raft::handle_t const&, graph_view_t<int32_t, int32_t, false, false> const&, std::optional<edge_property_view_t<int32_t, float const*>>, std::optional<raft::device_span<int32_t const>>, std::optional<size_t>, bool);
template std::tuple<rmm::device_uvector<int32_t>, rmm::device_uvector<int32_t>, rmm::device_uvector<double>> overlap_all_pairs_coefficients<int32_t, int32_t, double, false>(raft::handle_t const&, graph_view_t<int32_t, int32_t, false, false> const&, std::optional<edge_property_view_t<int32_t, double const*>>, std::optional<raft::device_span<int32_t const>>, std::optional<size_t>, bool);
}  // namespace cugraph

#endif  // AAI_ROUTE_OVERLAP
