...
struct scc_functor : public cugraph::c_api::abstract_functor {
  ...
  void operator()()
  {
    if constexpr (!cugraph::is_candidate<vertex_t, edge_t, weight_t>::value) {
    ...
    } else if constexpr (!std::is_same_v<vertex_t, int32_t>) {
      // SCC only supports int32 vertex/edge types
      // (is_candidate already ensures vertex_t == edge_t for same-size types)
      unsupported();
    } else {
      ...
      CUGRAPH_EXPECTS(number_map != nullptr,
                      "number_map is required for SCC (graph must be renumbered)");

      auto graph_view = graph->view();
      cugraph::c_api::attach_edge_mask_if_present(graph_view, graph_);
...

      {
        ...
        cugraph::legacy::GraphCSRView<vertex_t, edge_t, float> csr_view(
        ...
        cugraph::connected_components(csr_view, cugraph::cugraph_cc_t::CUGRAPH_STRONG, components.data());
      }
