...
struct destroy_graph_functor : public cugraph::c_api::abstract_functor {
  ...
  void* edge_ids_;
  void* edge_types_;
  cugraph::c_api::cugraph_data_mask_t* data_mask_;
  ...
  destroy_graph_functor(void* graph,
                        ...
                        void* edge_ids,
                        void* edge_types,
                        cugraph::c_api::cugraph_data_mask_t* data_mask)
    : abstract_functor(),
      graph_(graph),
  ...
  void operator()()
  {
    ...
    if (data_mask_ != nullptr) {
      if (data_mask_->edge_mask_ != nullptr) {
        auto edge_mask_ptr =
          reinterpret_cast<cugraph::edge_property_t<edge_t, bool>*>(data_mask_->edge_mask_);
        delete edge_mask_ptr;
      }
      if (data_mask_->raw_edge_bit_mask_ != nullptr) {
        auto raw_ptr =
          reinterpret_cast<rmm::device_uvector<uint32_t>*>(data_mask_->raw_edge_bit_mask_);
        delete raw_ptr;
      }
      delete data_mask_;
    }
...
extern "C" cugraph_error_code_t cugraph_data_mask_create(
  const cugraph_resource_handle_t* handle,
  ...
  cugraph_error_t** error)
{
  ...
  *error = nullptr;
  ...
  try {
    auto p_handle = reinterpret_cast<cugraph::c_api::cugraph_resource_handle_t const*>(handle);
    auto p_edge_bit_mask =
      reinterpret_cast<cugraph::c_api::cugraph_type_erased_device_array_view_t const*>(
        edge_bit_mask);

    auto internal_mask        = new cugraph::c_api::cugraph_data_mask_t{};
    internal_mask->complement_ = (complement != FALSE);

    if (p_edge_bit_mask != nullptr) {
      auto raw_data =
        new rmm::device_uvector<uint32_t>(p_edge_bit_mask->size_, p_handle->handle_->get_stream());
      raft::copy(raw_data->data(),
                 p_edge_bit_mask->as_type<uint32_t>(),
                 p_edge_bit_mask->size_,
                 p_handle->handle_->get_stream());
      internal_mask->raw_edge_bit_mask_ = raw_data;
    }

    *mask = reinterpret_cast<cugraph_data_mask_t*>(internal_mask);
  } catch (std::exception const& ex) {
    *error = reinterpret_cast<cugraph_error_t*>(new cugraph::c_api::cugraph_error_t{ex.what()});
    return CUGRAPH_UNKNOWN_ERROR;
  }
...
struct add_data_mask_functor : public cugraph::c_api::abstract_functor {
  raft::handle_t const& handle_;
  cugraph::c_api::cugraph_graph_t* graph_;
  cugraph::c_api::cugraph_data_mask_t* mask_;

  add_data_mask_functor(raft::handle_t const& handle,
                        cugraph::c_api::cugraph_graph_t* graph,
                        cugraph::c_api::cugraph_data_mask_t* mask)
    : abstract_functor(), handle_(handle), graph_(graph), mask_(mask)
  {
  }
  ...
  void operator()()
  {
    if constexpr (!cugraph::is_candidate<vertex_t, edge_t, weight_t>::value) {
      ...
      auto edge_mask =
        new cugraph::edge_property_t<edge_t, bool>(handle_, graph_view);

      if (mask_->raw_edge_bit_mask_ != nullptr) {
        auto raw_data =
          reinterpret_cast<rmm::device_uvector<uint32_t>*>(mask_->raw_edge_bit_mask_);

        auto mask_view = edge_mask->mutable_view();
        auto edge_partition_value_firsts = mask_view.value_firsts();
        auto edge_partition_edge_counts  = mask_view.edge_counts();

        size_t raw_offset = 0;
        for (size_t i = 0; i < graph_view.number_of_local_edge_partitions(); ++i) {
          size_t num_packed =
            cugraph::packed_bool_size(static_cast<size_t>(edge_partition_edge_counts[i]));
          raft::copy(edge_partition_value_firsts[i],
                     raw_data->data() + raw_offset,
                     num_packed,
                     handle_.get_stream());
          raw_offset += num_packed;
        }
      }

      mask_->edge_mask_  = edge_mask;
      mask_->edge_type_  = cugraph::c_api::data_type_id<edge_t>::id;
      graph_->data_mask_ = mask_;
    }
  }
};

struct destroy_data_mask_functor : public cugraph::c_api::abstract_functor {
  cugraph::c_api::cugraph_data_mask_t* mask_;

  destroy_data_mask_functor(cugraph::c_api::cugraph_data_mask_t* mask)
    : abstract_functor(), mask_(mask)
  {
  }
  ...
  void operator()()
  {
    if (mask_->edge_mask_ != nullptr) {
      auto edge_mask_ptr =
        reinterpret_cast<cugraph::edge_property_t<edge_t, bool>*>(mask_->edge_mask_);
      delete edge_mask_ptr;
      mask_->edge_mask_ = nullptr;
    }
  }
};
...
extern "C" cugraph_error_code_t cugraph_graph_add_data_mask(
  cugraph_graph_t* graph,
  cugraph_data_mask_t* mask,
  cugraph_error_t** error)
{
  *error = nullptr;
  ...
  auto internal_mask  = reinterpret_cast<cugraph::c_api::cugraph_data_mask_t*>(mask);

  if (internal_graph->data_mask_ != nullptr) {
    *error = reinterpret_cast<cugraph_error_t*>(
      new cugraph::c_api::cugraph_error_t{"Graph already has a data mask attached."});
  ...
  try {
    ...
    add_data_mask_functor functor(handle, internal_graph, internal_mask);

    cugraph::c_api::vertex_dispatcher(internal_graph->vertex_type_,
                                      internal_graph->edge_type_,
                                      internal_graph->weight_type_,
                                      internal_graph->edge_type_id_type_,
                                      internal_graph->edge_time_type_,
                                      internal_graph->store_transposed_,
                                      internal_graph->multi_gpu_,
                                      functor);
...
extern "C" cugraph_error_code_t cugraph_graph_release_data_mask(
  cugraph_graph_t* graph,
  cugraph_data_mask_t** mask,
  cugraph_error_t** error)
{
  ...
  *mask = reinterpret_cast<cugraph_data_mask_t*>(internal_graph->data_mask_);
  internal_graph->data_mask_ = nullptr;

  return CUGRAPH_SUCCESS;
}

extern "C" cugraph_error_code_t cugraph_graph_get_data_mask(
  cugraph_graph_t* graph,
  cugraph_data_mask_t** mask,
  cugraph_error_t** error)
{
...
extern "C" void cugraph_data_mask_destroy(cugraph_data_mask_t* mask)
{
  if (mask != nullptr) {
    auto internal_mask = reinterpret_cast<cugraph::c_api::cugraph_data_mask_t*>(mask);

    if (internal_mask->edge_mask_ != nullptr && internal_mask->edge_type_ != NTYPES) {
      // Type-dispatched delete of edge_property_t.
      // We use INT32 for vertex_t and weight_t since they don't affect edge_property_t<edge_t,bool>
      if (internal_mask->edge_type_ == INT32) {
        auto edge_mask_ptr =
          reinterpret_cast<cugraph::edge_property_t<int32_t, bool>*>(internal_mask->edge_mask_);
        delete edge_mask_ptr;
      } else {
        auto edge_mask_ptr =
          reinterpret_cast<cugraph::edge_property_t<int64_t, bool>*>(internal_mask->edge_mask_);
        delete edge_mask_ptr;
      }
    }

    if (internal_mask->raw_edge_bit_mask_ != nullptr) {
      auto raw_ptr =
        reinterpret_cast<rmm::device_uvector<uint32_t>*>(internal_mask->raw_edge_bit_mask_);
      delete raw_ptr;
    }

    delete internal_mask;
  }
}
