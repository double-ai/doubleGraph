...
struct balanced_cut_clustering_functor : public cugraph::c_api::abstract_functor {
  ...
  void operator()()
  {
    if constexpr (!cugraph::is_candidate<vertex_t, edge_t, weight_t>::value) {
      ...
      CUGRAPH_EXPECTS(number_map != nullptr,
                      "number_map is required (graph must be renumbered)");
      ...
      static_assert(std::is_same_v<vertex_t, int32_t>,
                    "When edge_t == int32_t, vertex_t must also be int32_t per is_candidate");
      {
        ...
        if (edge_weights == nullptr) {
          tmp_weights.resize(edge_partition_view.number_of_edges(), handle_.get_stream());
          cugraph::detail::scalar_fill(handle_, tmp_weights.data(), tmp_weights.size(), weight_t{1});
        }
        cugraph::legacy::GraphCSRView<vertex_t, edge_t, weight_t> csr_view(
  ...
  void operator()()
  {
    if constexpr (!cugraph::is_candidate<vertex_t, edge_t, weight_t>::value) {
      ...
      static_assert(std::is_same_v<vertex_t, int32_t>,
                    "When edge_t == int32_t, vertex_t must also be int32_t per is_candidate");
      {
        // Prepare cluster assignments - need to handle renumbering
        ...
        if (cugraph::detail::is_equal(
          ...
          raft::copy(cluster_assignments.data(),
          ...
          raft::copy(
            cluster_assignments.data(), clusters_->as_type<vertex_t>(), clusters_->size_, handle_.get_stream());
          ...
          cugraph::c_api::detail::sort_by_key(
            ...
            raft::device_span<vertex_t>{cluster_assignments.data(), cluster_assignments.size()});
        }

        {
          ...
          auto seg_opt = graph_view.local_edge_partition_segment_offsets(0);
          std::vector<vertex_t> const* seg_ptr = nullptr;
          if (seg_opt.has_value() && seg_opt->size() == 5) {
            seg_ptr = &seg_opt.value();
          }
          ...
          cugraph::legacy::GraphCSRView<vertex_t, edge_t, weight_t> csr_view(
              ...
              edge_partition_view.number_of_edges());
          weight_t score;
          cugraph::ext_raft::analyzeClustering_ratio_cut(handle_,
                                                          csr_view,
                                                          static_cast<int>(n_clusters_),
                    ...
                    "When edge_t == int32_t, vertex_t must also be int32_t per is_candidate");
      {
...

        {
          ...
          cugraph::legacy::GraphCSRView<vertex_t, edge_t, weight_t> csr_view(
              ...
              edge_partition_view.number_of_edges());
          weight_t score;
          cugraph::ext_raft::analyzeClustering_edge_cut(handle_,
                                                         csr_view,
                                                         static_cast<int>(n_clusters_),
                                                         cluster_assignments.data(),
                                                         &score,
                    ...
                    "When edge_t == int32_t, vertex_t must also be int32_t per is_candidate");
      {
...

        {
          ...
          cugraph::legacy::GraphCSRView<vertex_t, edge_t, weight_t> csr_view(
              ...
              edge_partition_view.number_of_edges());
          weight_t score;
          cugraph::ext_raft::analyzeClustering_modularity(handle_,
                                                           csr_view,
                                                           static_cast<int>(n_clusters_),
                                                           cluster_assignments.data(),
                                                           &score,
