...
struct cugraph_data_mask_t {
  cugraph_data_type_id_t edge_type_{NTYPES};  // needed for type-dispatched cleanup
  bool complement_{false};
  void* edge_mask_{nullptr};           // edge_property_t<edge_t, bool>* (after add_data_mask)
  void* raw_edge_bit_mask_{nullptr};   // rmm::device_uvector<uint32_t>* (from create)
};
...
struct cugraph_graph_t {
  ...
  cugraph_data_mask_t* data_mask_{nullptr};
};
                                       ...
                                       cugraph_error_t* error)
{
  if (store_transposed == graph->store_transposed_) {
    ...
    // Mask is invalidated by transpose; caller must re-attach
    if (graph->data_mask_ != nullptr) {
      if (graph->data_mask_->edge_mask_ != nullptr) {
        auto edge_mask_ptr =
          reinterpret_cast<cugraph::edge_property_t<edge_t, bool>*>(graph->data_mask_->edge_mask_);
        delete edge_mask_ptr;
      }
      if (graph->data_mask_->raw_edge_bit_mask_ != nullptr) {
        auto raw_ptr = reinterpret_cast<rmm::device_uvector<uint32_t>*>(
          graph->data_mask_->raw_edge_bit_mask_);
        delete raw_ptr;
      }
      delete graph->data_mask_;
      graph->data_mask_ = nullptr;
    }
    ...
    auto optional_edge_weights = std::optional<edge_property_t<edge_t, weight_t>>(std::nullopt);

    if (graph->edge_weights_ != nullptr) {
      auto edge_weights =
        reinterpret_cast<edge_property_t<edge_t, weight_t>*>(graph->edge_weights_);
    ...
    auto new_optional_edge_weights = std::optional<edge_property_t<edge_t, weight_t>>(std::nullopt);
    ...
    *number_map = std::move(new_number_map.value());
    ...
    if (new_optional_edge_weights) {
      auto new_edge_weights = new cugraph::edge_property_t<edge_t, weight_t>(handle);
