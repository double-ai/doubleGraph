...
class Dataset:
    ...
    def _find_binary_path(self):
        """Look for a pre-converted binary file (.parquet or .npz) next to the CSV."""
        csv_path = self.get_path()
    ...
    def _load_binary(self, path, reader):
        """Load edgelist from a .parquet or .npz file, return a DataFrame.

        Binary files always store columns as ``src`` and ``dst``.  If the
        dataset metadata uses different column names (e.g. ``FromNodeId``),
        the returned DataFrame is renamed to match.
        """
        col_names = self.metadata["col_names"]
        src_col, dst_col = col_names[0], col_names[1]
        use_cudf = (reader == "cudf") or (reader is None and _has_cuda_gpu)

        if path.suffix == ".parquet":
            if use_cudf:
                df = cudf.read_parquet(path, columns=["src", "dst"])
            else:
                df = pd.read_parquet(path, columns=["src", "dst"])
        else:
            # .npz path
            import numpy as np
            ...
            data = np.load(path)
            if use_cudf:
                df = cudf.DataFrame({"src": data["src"], "dst": data["dst"]})
            else:
                df = pd.DataFrame({"src": data["src"], "dst": data["dst"]})

        # Rename to match dataset metadata column names
        if src_col != "src" or dst_col != "dst":
            df = df.rename(columns={"src": src_col, "dst": dst_col})
    ...
    def get_edgelist(self, download=False, reader=None):
        ...
        if self._edgelist is None or not isinstance(self._edgelist, cudf.DataFrame):
            # Prefer binary formats: parquet > npz > csv
            binary_path = self._find_binary_path()
            if binary_path is not None:
                self._edgelist = self._load_binary(binary_path, reader)
    ...
    def get_graph(
        ...
        renumber : Boolean (default=True)
            If True, renumber vertices to contiguous integers starting from 0
            and sort by degree. This also computes segment offsets used by
            optimized algorithm implementations.

        process_edgelist_fn : callable or None (default=None)
            Optional function called with (df, src_col, dst_col, weight_col)
            before graph creation. weight_col is None when the dataset has no
            weight column. Must return the (possibly modified) DataFrame.
        """
        ...
        src_col = col_names[0]
        dst_col = col_names[1]
        weight_col = col_names[2] if len(col_names) > 2 else None

        edgelist = self._edgelist
        if process_edgelist_fn is not None:
            edgelist = process_edgelist_fn(edgelist.copy(), src_col, dst_col, weight_col)
        ...
        if not ignore_weights:
            if weight_col is not None and weight_col in edgelist.columns:
                edge_attr = weight_col
            else:
                extra = [c for c in edgelist.columns if c not in (src_col, dst_col)]
                if extra:
                    edge_attr = extra[0]
