...
def test_sssp_csr_graph(graph_file):
    ...
    if len(predecessor_diffs) > 0:
        # When multiple shortest paths exist, different predecessors can both be
        # valid. Validate that each differing predecessor is correct by checking:
        # 1. An edge exists from the predecessor to the destination vertex
        # 2. dist(src, pred) + weight(pred, dst) == dist(src, dst)
        edges = df.to_pandas()
        edge_weights = {}
        for u, v, w in zip(edges["src"], edges["dst"], edges["wgt"]):
            edge_weights[(int(u), int(v))] = float(w)
            edge_weights[(int(v), int(u))] = float(w)

        sssp = result_sssp.to_pandas()
        dist_of = dict(zip(sssp["vertex"].astype(int), sssp["distance_coo"].astype(float)))

        diffs = predecessor_diffs.to_pandas()
        for dst, pred_coo, pred_csr in zip(
            diffs["vertex"], diffs["predecessor_coo"], diffs["predecessor_csr"]
        ):
            dst, pred_coo, pred_csr = int(dst), int(pred_coo), int(pred_csr)
            w_coo = edge_weights.get((pred_coo, dst))
            w_csr = edge_weights.get((pred_csr, dst))

            for label, pred in [("COO", pred_coo), ("CSR", pred_csr)]:
                assert (pred, dst) in edge_weights, (
                    f"{label} predecessor {pred} has no edge to vertex {dst}"
                )
                actual = dist_of[pred] + edge_weights[(pred, dst)]
                assert np.isclose(actual, dist_of[dst], rtol=1e-5), (
                    f"{label} predecessor {pred} for vertex {dst}: "
                    f"dist({pred}) + weight({pred},{dst}) = {actual} != {dist_of[dst]}"
                )
