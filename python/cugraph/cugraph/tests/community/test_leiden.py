...
def _compute_modularity(G, partition, resolution=1.0):
    """Compute modularity of a partition directly from the graph edges."""
    edges = G.view_edge_list()
    src = edges["src"].to_pandas().values
    dst = edges["dst"].to_pandas().values
    wgt = edges["weight"].to_pandas().values if "weight" in edges.columns else None

    cluster = partition.to_pandas().values
    m = wgt.sum() if wgt is not None else len(src)

    import numpy as np
    strength = np.zeros(len(cluster), dtype=np.float64)
    for i in range(len(src)):
        w = wgt[i] if wgt is not None else 1.0
        strength[src[i]] += w
        strength[dst[i]] += w

    # Modularity: sum over edges of [w * (c_i == c_j)] / m
    #           - resolution * sum over clusters of (S_c / (2m))^2
    intra = 0.0
    for i in range(len(src)):
        if cluster[src[i]] == cluster[dst[i]]:
            w = wgt[i] if wgt is not None else 1.0
            intra += w
    ...
    for v in range(len(cluster)):
        c = cluster[v]
        cluster_strength[c] = cluster_strength.get(c, 0.0) + strength[v]

    Q = intra / m
    for s in cluster_strength.values():
        Q -= resolution * (s / (2.0 * m)) ** 2
...
def test_leiden_golden_results(input_and_expected_output):
    ...
    G = input_and_expected_output["graph"]
    resolution = input_and_expected_output["resolution"]

    # The reported modularity must meet the expected threshold.
    assert result_mod >= expected_mod - 0.0001

    # Verify the reported modularity matches the actual partition.
    actual_mod = _compute_modularity(G, result_partition, resolution)
    assert abs(actual_mod - result_mod) < 0.001, (
        f"Reported modularity {result_mod} does not match "
        f"computed modularity {actual_mod}"
    )
