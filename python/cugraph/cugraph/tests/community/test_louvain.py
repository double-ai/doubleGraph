...
def test_louvain_csr_graph(is_weighted):
    ...
    # COO and CSR paths may use different PLC renumber flags, which can
    # cause louvain to converge to different (but equally valid) partitions.
    # Verify algorithm quality (modularity) and partition count instead of
    # exact partition equality.
    assert louvain_csr["partition"].nunique() == louvain_coo["partition"].nunique()
    assert mod_csr == pytest.approx(mod_coo, rel=1e-2)
