...
def test_directed_graph_check(read_csv, use_weight):
    ...
    with pytest.raises(ValueError):
        cugraph.sorensen(G1, vertex_pair, use_weight=use_weight)
...
def test_all_pairs_sorensen():
    ...
    # Sort both by (first, second) since output order is unspecified
    sorensen_results = sorensen_results.sort_values(["first", "second"]).reset_index(drop=True)
    all_pairs_sorensen_results = all_pairs_sorensen_results.sort_values(["first", "second"]).reset_index(drop=True)
