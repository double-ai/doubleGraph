...
def test_all_pairs_jaccard():
    ...
    jaccard_results = jaccard_results.sort_values(["first", "second"]).reset_index(drop=True)
    all_pairs_jaccard_results = all_pairs_jaccard_results.sort_values(["first", "second"]).reset_index(drop=True)
