...
def test_directed_graph_check(graph_file, use_weight):
    ...
    with pytest.raises(ValueError):
        cugraph.overlap(G1, vertex_pair, use_weight=use_weight)
...
def test_all_pairs_overlap():
    ...
    overlap_results = overlap_results.sort_values(["first", "second"]).reset_index(drop=True)
    all_pairs_overlap_results = all_pairs_overlap_results.sort_values(["first", "second"]).reset_index(drop=True)
