...
class simpleGraphImpl:
    ...
    def __from_edgelist(
        ...
        if vertices is not None:
            if self.properties.renumbered is True:
                vertices = self.renumber_map.to_internal_vertex_id(
                    vertices if isinstance(vertices, cudf.Series)
                    else cudf.Series(vertices)
                )
        ...
        # If the Python layer already renumbered the edgelist (properties.renumbered
        # is True), tell PLC not to renumber again — the IDs are already dense 0..N-1.
        # Passing renumber=True to PLC after Python renumbering would create a second,
        # independent renumber map, causing mismatches in vertex ID conversion.
        plc_renumber = renumber and not self.properties.renumbered

        # When Python already renumbered, PLC expects renumber=False with vertices
        # strictly ordered [0, 1, ..., N-1].  The renumber map may assign internal
        # IDs in a different order than the original vertex list, producing a valid
        # permutation of 0..N-1 that PLC rejects.  Since all vertices are already
        # represented in the dense edgelist, the separate vertices array is
        # redundant and can be dropped.
        if not plc_renumber and self.properties.renumbered:
            vertices = None

        self._make_plc_graph(
            value_col=value_col,
            store_transposed=store_transposed,
            renumber=plc_renumber,
    ...
    def to_numpy_array(self):
        ...
        # If the graph is renumbered, the edgelist stores internal IDs but
        # self.nodes() returns external IDs.  Unrenumber so lookups match.
        if self.properties.renumbered:
            df = df.copy()
            df = self.renumber_map.unrenumber(df, simpleGraphImpl.srcCol)
            df = self.renumber_map.unrenumber(df, simpleGraphImpl.dstCol)
