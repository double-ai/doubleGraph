...
class NumberMap:
    ...
    def to_internal_vertex_id(self, df, col_names=None):
        ...
        if reply.isnull().any():
            missing = df[reply.isnull().values] if hasattr(df, '__getitem__') else df
            raise ValueError(
                f"vertex IDs not found in renumber map: {missing.values_host if hasattr(missing, 'values_host') else missing}"
            )
        return reply
    ...
    def renumber_and_segment(
        ...
        # Always renumber: this function is only called when renumber=True
        # was requested. Previously, single-column integer vertices skipped
        # Python-level renumbering, but the C++ PLC graph still renumbered,
        # causing is_renumbered=False while IDs were actually remapped.
        # This led to OOB crashes when external vertex IDs (from sparse
        # graphs like RMAT) were passed directly to kernels expecting
        # internal (dense) IDs.
        renumbered = True
