...
class Graph:
    ...
    def attach_edge_mask(self, edge_mask):
        """
        Attach a boolean edge mask. Edges where mask[i]=True are included,
        mask[i]=False are excluded. Order matches the internal edge order.

        Parameters
        ----------
        edge_mask : cupy.ndarray (bool), length = num_edges
        """

        if not hasattr(edge_mask, '__cuda_array_interface__'):
            edge_mask = cupy.asarray(edge_mask)

        # Pack booleans into uint32 (1 bit per edge)
        mask_u8 = edge_mask.astype(cupy.uint8)
        packed = cupy.packbits(mask_u8, bitorder='little')
        pad = (4 - len(packed) % 4) % 4
        if pad:
            packed = cupy.pad(packed, (0, pad))
        packed_u32 = packed.view(cupy.uint32)

        self._plc_graph.attach_edge_mask(ResourceHandle(), packed_u32)

    def detach_edge_mask(self):
        """Remove the currently attached edge mask."""
        self._plc_graph.detach_edge_mask()

    def has_edge_mask(self):
        """Return True if an edge mask is currently attached."""
        return self._plc_graph.has_edge_mask()
