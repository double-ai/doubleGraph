...
from pylibcugraph._cugraph_c.graph cimport (
    ...
    cugraph_graph_free,
    cugraph_data_mask_t,
    cugraph_data_mask_create,
    cugraph_graph_add_data_mask,
    cugraph_graph_release_data_mask,
    cugraph_graph_get_data_mask,
    cugraph_data_mask_destroy,
)
from pylibcugraph._cugraph_c.array cimport (
...
cdef class SGGraph(_GPUGraph):
    ...
    def attach_edge_mask(self, ResourceHandle resource_handle, edge_mask):
        """Attach a packed uint32 edge bit mask to the graph."""
        cdef cugraph_error_t* error_ptr
        cdef cugraph_error_code_t error_code
        cdef cugraph_data_mask_t* mask_ptr

        cdef cugraph_type_erased_device_array_view_t* mask_view_ptr = \
            create_cugraph_type_erased_device_array_view_from_py_obj(edge_mask)

        # Create mask (NULL vertex mask, edge mask provided, no complement)
        error_code = cugraph_data_mask_create(
            resource_handle.c_resource_handle_ptr,
            NULL,  # no vertex mask
            mask_view_ptr,
            <bool_t>0,  # complement=false
            &mask_ptr,
            &error_ptr)
        cugraph_type_erased_device_array_view_free(mask_view_ptr)
        assert_success(error_code, error_ptr, "cugraph_data_mask_create()")

        # Attach mask to graph
        error_code = cugraph_graph_add_data_mask(
            self.c_graph_ptr, mask_ptr, &error_ptr)
        if error_code != cugraph_error_code_t.CUGRAPH_SUCCESS:
            cugraph_data_mask_destroy(mask_ptr)
        assert_success(error_code, error_ptr, "cugraph_graph_add_data_mask()")

    def detach_edge_mask(self):
        """Remove the edge mask from the graph and destroy it."""
        cdef cugraph_error_t* error_ptr
        cdef cugraph_data_mask_t* mask_ptr
        cdef cugraph_error_code_t error_code

        error_code = cugraph_graph_release_data_mask(
            self.c_graph_ptr, &mask_ptr, &error_ptr)
        assert_success(error_code, error_ptr, "cugraph_graph_release_data_mask()")
        if mask_ptr is not NULL:
            cugraph_data_mask_destroy(mask_ptr)

    def has_edge_mask(self):
        """Return True if an edge mask is currently attached."""
        cdef cugraph_error_t* error_ptr
        cdef cugraph_data_mask_t* mask_ptr
        cugraph_graph_get_data_mask(self.c_graph_ptr, &mask_ptr, &error_ptr)
        return mask_ptr is not NULL
