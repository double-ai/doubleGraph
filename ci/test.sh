...
if hasArg "--run-cpp-tests"; then
    # Test binaries are built to cpp/build/gtests/ (flat directory, no subdirs).
    # Fall back to conda prefix paths for compatibility with installed layouts.
    GTESTS_DIR="${CUGRAPH_ROOT}/cpp/build/gtests"

    echo "C++ gtests for cuGraph (single-GPU only)..."
    if compgen -G "${GTESTS_DIR}/"*_TEST > /dev/null 2>&1; then
        for gt in "${GTESTS_DIR}/"*_TEST; do
            test_name=$(basename "$gt")
            # Skip C API tests (handled separately below)
            if [[ "$test_name" == CAPI_* ]]; then
                continue
            fi
    ...
    elif compgen -G "${CONDA_PREFIX}/bin/gtests/libcugraph/"*_TEST > /dev/null 2>&1; then
        ...
        echo "WARNING: No C++ test binaries found in ${GTESTS_DIR} or ${CONDA_PREFIX}/bin/gtests/libcugraph/"
    fi
    ...
    if compgen -G "${GTESTS_DIR}/"CAPI_*_TEST > /dev/null 2>&1; then
        for ct in "${GTESTS_DIR}/"CAPI_*_TEST; do
    ...
    elif compgen -G "${CONDA_PREFIX}/bin/gtests/libcugraph_c/"CAPI_*_TEST > /dev/null 2>&1; then
        ...
        echo "WARNING: No C API test binaries found in ${GTESTS_DIR} or ${CONDA_PREFIX}/bin/gtests/libcugraph_c/"
    fi
fi
